<?php
/**
 * Omniverse slider responsive param.
 *
 * @package Omniverse
 */
...
if ( ! function_exists( 'omniverse_get_slider_responsive_param' ) ) {
	/**
	 ...
	 * @return string
	 */
	function omniverse_get_slider_responsive_param( $settings, $value ) {
		$param_name   = $settings['param_name'];
		$devices_maps = array(
			'desktop'         => esc_html__( 'Desktop', 'omniverse' ),
			'tablet'          => esc_html__( 'Tablet', 'omniverse' ),
			'tablet_vertical' => esc_html__( 'Tablet', 'omniverse' ),
			'mobile'          => esc_html__( 'Mobile', 'omniverse' ),
		);

		if ( ! empty( $value ) ) {
			if ( omniverse_is_compressed_data( $value ) ) {
				$data = json_decode( omniverse_decompress( $value ), true );
			} elseif ( ! omniverse_is_compressed_data( $value ) ) {
				$data = array(
					'devices' => array(
						'desktop' => array(
							'unit'  => '-',
		...
		<div class="wd-sliders">
				...
				<div class="wd-field-devices">
					<?php foreach ( $settings['devices'] as $device => $device_settings ) : ?>
						...
						<span class="wd-device<?php echo esc_attr( $device_classes ); ?>" data-value="<?php echo esc_attr( $device ); ?>" title="<?php echo esc_attr( ucfirst( $devices_maps[ $device ] ) ); ?>">
							<span><?php echo esc_attr( $devices_maps[ $device ] ); ?></span>
						</span>
					<?php endforeach; ?>
				</div>
			<?php endif; ?>
			...
			<?php foreach ( $settings['devices'] as $device => $device_settings ) : ?>
				<?php echo omniverse_render_slider_responsive( $device, $settings, $device_settings ); // phpcs:ignore ?>
			<?php endforeach; ?>
...
if ( ! function_exists( 'omniverse_render_slider_responsive' ) ) {
	/**
	 ...
	 * @return false|string
	 */
	function omniverse_render_slider_responsive( $device, $settings, $device_settings ) {
		ob_start();
		...
		<div class="wd-slider<?php echo esc_attr( $slider_classes ); ?>" data-device="<?php echo esc_attr( $device ); ?>" data-value="<?php echo esc_attr( $value ); ?>" data-unit="<?php echo esc_attr( $device_settings['unit'] ); ?>">
			...
			<span class="dn-range-field-value-input">
				<input type="number" class="wd-slider-value-preview" aria-label="<?php esc_attr_e( 'Preview', 'omniverse' ); ?>">
			</span>
