...
if ( ! function_exists( 'omniverse_get_typography_map' ) ) {
	 ...
	 * @return array
	 */
	function omniverse_get_typography_map( $args ) {
		$fields = array();

		if ( ! isset( $args['title'] ) ) {
			$args['title'] = esc_html__( 'Typography', 'omniverse' );
		}

		$fields['font_family'] = array(
			'heading'      => esc_html__( 'Family', 'omniverse' ),
			'type'         => 'wd_fonts',
		...
		$fields['font_weight'] = array(
			'heading'    => esc_html__( 'Weight', 'omniverse' ),
			'type'       => 'wd_select',
			...
			'value'      => array(
				...
				esc_html__( 'Thin 100', 'omniverse' )       => '100',
				esc_html__( 'Light 200', 'omniverse' )      => '200',
				esc_html__( 'Regular 300', 'omniverse' )    => '300',
				...
				esc_html__( 'Black 900', 'omniverse' )      => '900',
			),
		);

		$fields['text_transform'] = array(
			'heading'    => esc_html__( 'Transform', 'omniverse' ),
			'type'       => 'wd_select',
			...
			'value'      => array(
				esc_html__( 'Default', 'omniverse' )    => '',
				esc_html__( 'Uppercase', 'omniverse' )  => 'uppercase',
				esc_html__( 'Lowercase', 'omniverse' )  => 'lowercase',
				esc_html__( 'Capitalize', 'omniverse' ) => 'capitalize',
				esc_html__( 'Normal', 'omniverse' )     => 'none',
			),
		);
		...
		$fields['font_style'] = array(
			...
			'value'      => array(
				...
				esc_html__( 'Italic', 'omniverse' )  => 'italic',
				esc_html__( 'Oblique', 'omniverse' ) => 'oblique',
			),
		);
...
if ( ! function_exists( 'omniverse_vc_get_control_data' ) ) {
	/**
	 * Get control data.
	 *
	 ...
	 * @return string
	 */
	function omniverse_vc_get_control_data( $data, $device ) {
		$data_decoded = json_decode( omniverse_decompress( $data ), true );
...
if ( ! function_exists( 'omniverse_parse_shortcodes_css_data_new' ) ) {
	/**
	 ...
	 * @throws Exception .
	 */
	function omniverse_parse_shortcodes_css_data_new( $content ) {
		$css_data = array();
		...
		foreach ( $shortcodes[2] as $index => $tag ) {
			...
			if ( isset( $shortcode['params'] ) && ! empty( $shortcode['params'] ) ) {
				foreach ( $shortcode['params'] as $param ) {
					if ( isset( $param['type'] ) && in_array( $param['type'], $omniverse_fields, true ) && isset( $attr_array[ $param['param_name'] ] ) ) {
						$css_data[] = array(
							'selector_id' => $attr_array['omniverse_css_id'],
							'tag'         => $tag,
		...
		foreach ( $shortcodes[5] as $shortcode_content ) {
			$css_data = array_merge( $css_data, omniverse_parse_shortcodes_css_data_new( $shortcode_content ) );
		}
...
if ( ! function_exists( 'omniverse_get_fields_css' ) ) {
	/**
	 ...
	 * @throws Exception .
	 */
	function omniverse_get_fields_css( $post_id ) {
		$post       = get_post( $post_id );
		$data_array = omniverse_parse_shortcodes_css_data_new( $post->post_content );

		return omniverse_fields_css_data_to_css( $data_array, $post_id );
	}
}

if ( ! function_exists( 'omniverse_save_fields_css' ) ) {
	/**
	 ...
	 * @throws Exception .
	 */
	function omniverse_save_fields_css( $post_id ) {
		$css  = omniverse_get_fields_css( $post_id );
		$css .= omniverse_get_fields_css_old( $post_id );

		if ( empty( $css ) ) {
			delete_post_meta( $post_id, 'omniverse_shortcodes_custom_css' );
		} else {
			update_post_meta( $post_id, 'omniverse_shortcodes_custom_css', $css );
		}
	}

	add_action( 'save_post', 'omniverse_save_fields_css' );
}

if ( ! function_exists( 'omniverse_fields_css_data_to_css' ) ) {
	/**
	 ...
	 */
	function omniverse_fields_css_data_to_css( $data_array, $post_id ) {
		...
		foreach ( $data_array as $data ) {
			...
			if ( omniverse_is_compressed_data( $data['value'] ) && function_exists( 'omniverse_decompress' ) ) {
				$decompressed_data = json_decode( omniverse_decompress( $data['value'] ), true );
			} elseif ( ! omniverse_is_compressed_data( $data['value'] ) ) {
						...
						foreach ( $properties as $property ) {
							...
							if ( false !== stripos( $property, 'box-shadow' ) ) {
								...
								if ( false !== stripos( $property, 'font-family' ) ) {
									$standard_fonts = omniverse_get_config( 'standard-fonts' );
									$backup_fonts   = apply_filters( 'omniverse_backup_fonts', ', Arial, Helvetica, sans-serif' );
		...
		omniverse_save_fields_fonts( $fonts, $post_id );
...
if ( ! function_exists( 'omniverse_save_fields_fonts' ) ) {
	/**
	 ...
	 * @return void
	 */
	function omniverse_save_fields_fonts( $fonts, $post_id ) {
		if ( empty( $fonts ) ) {
			delete_post_meta( $post_id, 'omniverse_shortcodes_fonts' );
		} else {
			update_post_meta( $post_id, 'omniverse_shortcodes_fonts', $fonts );
		}
	}
}

if ( ! function_exists( 'omniverse_load_fields_fonts' ) ) {
	/**
	 * Load fields fonts.
	 */
	function omniverse_load_fields_fonts() {
		$id = get_the_ID();

		if ( Main::get_instance()->has_custom_layout( 'single_product' ) && ! is_singular( 'omniverse_layout' )
		) {
			$id = Main::get_instance()->get_layout_id( 'single_product' );
		}

		$fonts = get_post_meta( $id, 'omniverse_shortcodes_fonts', true );
	...
	add_action( 'wp', 'omniverse_load_fields_fonts' );
}
