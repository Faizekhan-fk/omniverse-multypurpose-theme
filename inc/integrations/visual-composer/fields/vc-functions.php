 ...
 */
if ( ! function_exists( 'omniverse_parse_shortcodes_css_data' ) ) {
	function omniverse_parse_shortcodes_css_data( $content ) {
		...
		foreach ( $shortcodes[2] as $index => $tag ) {
			...
			$omniverse_fields = array(
				'omniverse_responsive_size',
				'omniverse_responsive_spacing',
				'omniverse_colorpicker',
				'omniverse_slider',
			);

			if ( isset( $shortcode['params'] ) && ! empty( $shortcode['params'] ) ) {
				foreach ( $shortcode['params'] as $param ) {
					if ( isset( $param['type'] ) && in_array( $param['type'], $omniverse_fields ) && isset( $attr_array[ $param['param_name'] ] ) ) {
		...
		foreach ( $shortcodes[5] as $shortcode_content ) {
			$css_data .= omniverse_parse_shortcodes_css_data( $shortcode_content );
		}
...
if ( ! function_exists( 'omniverse_get_fields_css_old' ) ) {
	/**
	 ...
	 * @throws Exception .
	 */
	function omniverse_get_fields_css_old( $post_id ) {
		$post       = get_post( $post_id );
		$css_data   = omniverse_parse_shortcodes_css_data( $post->post_content );
		$data_array = explode( '[|]', $css_data );
		return omniverse_shortcodes_css_data_to_css( $data_array );
	}
}

if ( ! function_exists( 'omniverse_print_shortcodes_css' ) ) {
	function omniverse_print_shortcodes_css( $id = null ) {
		...
		if ( $id ) {
			$css = get_post_meta( $id, 'omniverse_shortcodes_custom_css', true );
			if ( ! empty( $css ) ) {
				echo '<style data-type="omniverse_shortcodes-custom-css">' . $css . '</style>';
			}
		}
	}

	add_action( 'wp_head', 'omniverse_print_shortcodes_css', 1000 );
}


if ( ! function_exists( 'omniverse_shortcodes_css_data_to_css' ) ) {
	function omniverse_shortcodes_css_data_to_css( $css_data ) {
		$results         = '';
		...
		foreach ( $css_data as $value ) {
			$decompressed_data = function_exists( 'omniverse_decompress' ) ? json_decode( omniverse_decompress( $value ) ) : '';
