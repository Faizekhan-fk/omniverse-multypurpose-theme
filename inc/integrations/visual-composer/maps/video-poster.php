...
if( ! function_exists( 'omniverse_add_field_to_video' ) ) { 
	function omniverse_add_field_to_video() {
	    ...
	    $vc_video_new_params = array(
	        ...
	        array(
				...
				'heading' => esc_html__( 'Add poster to video', 'omniverse' ),
		    ...
		    array(
			    ...
			    'heading'          => esc_html__( 'Play icon size', 'omniverse' ),
			    'group'            => esc_html__( 'Advanced', 'omniverse' ),
			    'devices'          => array(
				    'desktop' => array(
					    'unit' => 'px',
	...
	add_action( 'vc_after_init', 'omniverse_add_field_to_video' ); 
}

// **********************************************************************//
//  Function return vc_video with image mask.
// **********************************************************************//
if( ! function_exists( 'omniverse_add_video_poster' ) ) {
	function omniverse_add_video_poster( $output, $obj, $attr ) {
		if ( isset( $attr['image_poster_switch'] ) && $attr['image_poster_switch'] === 'yes' && isset( $attr['poster_image'] ) ) {
			omniverse_enqueue_js_script( 'video-poster-element' );
			...
			$id         = 'wd-rs-' . $attr['omniverse_css_id'];
			$image_id   = $attr['poster_image'];
			$image_size = $attr['img_size'];
			$image      = omniverse_otf_get_image_url( $image_id, $image_size );
...
add_filter( 'vc_shortcode_output', 'omniverse_add_video_poster', 10, 3 );
