 ...
 * ------------------------------------------------------------------------------------------------
 */
if ( ! function_exists( 'omniverse_get_vc_map_wishlist' ) ) {
	function omniverse_get_vc_map_wishlist() {
		return array(
			'name'        => esc_html__( 'Wishlist', 'omniverse' ),
			'base'        => 'omniverse_wishlist',
			'category'    => omniverse_get_tab_title_category_for_wpb( esc_html__( 'Theme elements', 'omniverse' ) ),
			'description' => esc_html__( 'Required for the wishlist page.', 'omniverse' ),
			'icon'        => OMNIVERSE_ASSETS . '/images/vc-icon/wishlist.svg',
			'params' => array(
				array(
					'type' => 'wd_notice',
					'param_name' => 'notice',
