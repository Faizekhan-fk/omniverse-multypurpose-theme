...
class Vctemplates {
	...
	public function library( $data ) {
		// $templates = omniverse_get_config('vc-templates');

		if ( omniverse_get_opt( 'white_label' ) ) {
			$title = 'Templates library';
		} else {
			$title = 'OmniVerse templates library';
		}

		$data[] = array(
			'category'             => 'omniverse_templates',
			'category_name'        => $title,
			'category_weight'      => 5,
			'category_description' => 'WPBakery predefined template parts and layouts from Zynxsol Studio. Designed for OmniVerse WordPress template.',
			'templates'            => array(),
		);
	...
	public function render_template( $category ) {
		...
		$category['output'] .= '<div class="dn-box dn-wpb-templates dn-theme-style">';
		$category['output'] .= '<div class="dn-box-header dn-wpb-templates-heading">';
		$category['output'] .= '<div class="dn-row">';
		$category['output'] .= '<div class="dn-col"><div>';
		...
		$category['output'] .= '<div class="dn-col-auto"><div class="dn-import-search dn-search dn-i-search"><input type="text" class="omniverse-templates-search" placeholder="Start typing to search..." /></div></div>';

		$category['output'] .= '</div>';
		$category['output'] .= '</div>';
		$category['output'] .= '
			<div class="dn-box-content dn-wpb-templates-content omniverse-templates-list dn-loading" data-vc-action="collapseAll"></div>';
		$category['output'] .= '</div>';
		$category['output'] .= '';
	...
	public function render_item( $template ) {
		$name                = isset( $template['name'] ) ? esc_html( $template['name'] ) : esc_html( __( 'No title', 'omniverse' ) );
		...
		$output  = <<<HTML
					<div class="omniverse-template-item $custom_class"
...
HTML;
	...
	public function render_template_window( $template_name, $template_data ) {
		...
		$add_template_title = esc_attr__( 'Add template', 'omniverse' );
		$image_src          = OMNIVERSE_THEME_DIR . '/inc/configs/templates-library/' . $template_id . '/preview.jpg';

		echo <<<HTML
			<div class="omniverse-template-image">
			...
			<h3 class="omniverse-template-title">$template_name</h3>
HTML;
	...
	private function _config() {
		$this->folder = OMNIVERSE_CONFIGS . '/templates-library/';
	...
	private function _load_template( $id ) {
		$response = wp_remote_get( OMNIVERSE_DEMO_URL . '?omniverse_action=omniverse_get_template&id=' . $id );
		$body     = '';
	...
	private function _media_exists( $src ) {
		$media = get_option( 'omniverse-vc-imported-media' );
	...
	private function _save_media_id( $src, $id ) {
		...
		if ( $exists_id = array_search( $src, $media ) ) {
			unset( $media[ $exists_id ] );
		}

		$media[ $id ] = $src;

		return update_option( 'omniverse-vc-imported-media', $media, false );
	...
	private function _load_importers() {
		...
		if ( ! function_exists( 'OMNIVERSE_Theme_Plugin' ) ) {
			return false;
		}
		...
		$plugin_dir = OMNIVERSE_Theme_Plugin()->plugin_path();
