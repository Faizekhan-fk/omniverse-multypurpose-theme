 ...
 * ------------------------------------------------------------------------------------------------
 */
if ( ! function_exists( 'omniverse_dokan_edit_product_wrap_start' ) ) {
	function omniverse_dokan_edit_product_wrap_start() {
		echo '<div class="site-content col-12" role="main">';
	}

	add_action( 'dokan_dashboard_wrap_before', 'omniverse_dokan_edit_product_wrap_start', 10 );
}

if ( ! function_exists( 'omniverse_dokan_edit_product_wrap_end' ) ) {
	function omniverse_dokan_edit_product_wrap_end() {
		echo '</div>';
	}

	add_action( 'dokan_dashboard_wrap_after', 'omniverse_dokan_edit_product_wrap_end', 10 );
}

if ( ! function_exists( 'omniverse_dokan_lazy_load_fix' ) ) {
	function omniverse_dokan_lazy_load_fix() {
		return array(
			'img' => array(
				'alt'           => array(),
				'class'         => array(),
	...
	add_filter( 'dokan_product_image_attributes', 'omniverse_dokan_lazy_load_fix', 10 );
}

if ( ! function_exists( 'omniverse_dokan_remove_map_from_shop_page' ) ) {
	function omniverse_dokan_remove_map_from_shop_page() {
	...
	add_action( 'init', 'omniverse_dokan_remove_map_from_shop_page' );
}

if ( ! function_exists( 'omniverse_dokan_add_map_before_main_content' ) ) {
	function omniverse_dokan_add_map_before_main_content() {
	...
	add_action( 'woocommerce_before_main_content', 'omniverse_dokan_add_map_before_main_content' );
}