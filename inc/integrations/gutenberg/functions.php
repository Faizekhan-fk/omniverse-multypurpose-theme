...
if ( ! function_exists( 'omniverse_gutenberg_disable_svg' ) ) {
	/**
	 * Gutenberg disable svg.
	 */
	function omniverse_gutenberg_disable_svg() {
	...
	add_action( 'init', 'omniverse_gutenberg_disable_svg' );
}

if ( ! function_exists( 'omniverse_gutenberg_show_widgets' ) ) {
	/**
	 * Gutenberg show widgets.
	 *
	 * @return array
	 */
	function omniverse_gutenberg_show_widgets() {
		return array();
	}

	add_action( 'widget_types_to_hide_from_legacy_widget_block', 'omniverse_gutenberg_show_widgets', 100 );
}

if ( ! function_exists( 'omniverse_gutenberg_enqueue_editor_styles' ) ) {
	/**
	 * Gutenberg styles.
	 *
	 * @since 1.0.0
	 */
	function omniverse_gutenberg_enqueue_editor_styles() {
		if ( omniverse_get_opt( 'disable_gutenberg_backend_css' ) ) {
			return;
		}

		$rtl = is_rtl() ? '-rtl' : '';
		wp_enqueue_style( 'wd-gutenberg-editor-style', OMNIVERSE_THEME_DIR . '/css/parts/wp-gutenberg-editor' . $rtl . '.min.css', array(), omniverse_get_theme_info( 'Version' ) );
	}

	add_action( 'admin_print_styles-post.php', 'omniverse_gutenberg_enqueue_editor_styles', 10 );
	add_action( 'admin_print_styles-post-new.php', 'omniverse_gutenberg_enqueue_editor_styles', 10 );
	add_action( 'admin_print_styles-widgets.php', 'omniverse_gutenberg_enqueue_editor_styles', 10 );
}

if ( ! function_exists( 'omniverse_gutenberg_editor_styles' ) ) {
	/**
	 * Gutenberg styles.
	 *
	 * @since 1.0.0
	 */
	function omniverse_gutenberg_editor_styles() {
		add_theme_support( 'editor-styles' );
		add_editor_style( 'style-editor.css' );
		add_theme_support( 'align-wide' );
	}

	add_action( 'after_setup_theme', 'omniverse_gutenberg_editor_styles', 10 );
}

if ( ! function_exists( 'omniverse_gutenberg_editor_custom_styles' ) ) {
	/**
	 * Gutenberg styles.
	 *
	 * @since 1.0.0
	 */
	function omniverse_gutenberg_editor_custom_styles() {
		if ( omniverse_get_opt( 'disable_gutenberg_backend_css' ) ) {
			return;
		}

		$all_pages_bg          = omniverse_get_opt( 'pages-background' );
		...
		$form_width = omniverse_get_opt( 'form_border_width' );

		// Default button.
		$default_btn_style          = omniverse_get_opt( 'btns_default_style' );
		$default_btn_color          = 'light' === omniverse_get_opt( 'btns_default_color_scheme' ) ? '#fff' :
			'#333';
		$default_btn_color_hover    = 'light' === omniverse_get_opt( 'btns_default_color_scheme_hover' ) ? '#fff' : '#333';
		$default_btn_bg_color       = omniverse_get_opt( 'btns_default_bg' );
		$default_btn_bg_color_hover = omniverse_get_opt( 'btns_default_bg_hover' );

		// Accent button.
		$accent_btn_style          = omniverse_get_opt( 'btns_shop_style' );
		$accent_btn_color          = '#333';
		$accent_btn_color_hover    = '#333';
		$accent_btn_bg_color       = omniverse_get_opt( 'btns_shop_bg' );
		$accent_btn_bg_color_hover = omniverse_get_opt( 'btns_shop_bg_hover' );

		if ( 'light' === omniverse_get_opt( 'btns_shop_color_scheme' ) ) {
			$accent_btn_color = '#fff';
		} elseif ( 'custom' === omniverse_get_opt( 'btns_shop_color_scheme' ) ) {
			$accent_btn_color_custom = omniverse_get_opt( 'btns_shop_color_scheme_custom' );
		...
		if ( 'light' === omniverse_get_opt( 'btns_shop_color_scheme_hover' ) ) {
			$accent_btn_color_hover = '#fff';
		} elseif ( 'custom' === omniverse_get_opt( 'btns_shop_color_scheme_hover' ) ) {
			$accent_btn_color_hover_custom = omniverse_get_opt( 'btns_shop_color_scheme_hover_custom' );
		...
		// Link.
		$link_color = omniverse_get_opt( 'link-color' );
		$primary_color = omniverse_get_opt( 'primary-color' );
		$alternative_color = omniverse_get_opt( 'secondary-color' );

		// Typography.
		$text_font = omniverse_get_opt( 'text-font' );
		$title_font = omniverse_get_opt( 'primary-font' );
		$entities_font = omniverse_get_opt( 'post-titles-font' );
		$alternative_font = omniverse_get_opt( 'secondary-font' );
		$widget_title_font = omniverse_get_opt( 'widget-titles-font' );
		$header_el_font = omniverse_get_opt( 'navigation-font' );

		// Icon font.
		$icon_font      = omniverse_get_opt( 'icon_font', array( 'font' => '1', 'weight' => '400' ) );
		$font_display   = omniverse_get_opt( 'icons_font_display' );
		...
		if ( 'none' === $rounding ) {
			...
			$custom_rounding = json_decode( omniverse_decompress( omniverse_get_opt( 'custom_rounding_size' ) ), true );
	...
	add_action( 'admin_print_styles-post.php', 'omniverse_gutenberg_editor_custom_styles', 20 );
	add_action( 'admin_print_styles-post-new.php', 'omniverse_gutenberg_editor_custom_styles', 20 );
	add_action( 'admin_print_styles-widgets.php', 'omniverse_gutenberg_editor_custom_styles', 20 );
}
