...
class Timeline extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Timeline', 'omniverse' );
	}
	...
	protected function register_controls() {
		...
		$repeater->add_control(
			'item_type',
			[
				'label'   => esc_html__( 'Item type', 'omniverse' ),
				'type'    => Controls_Manager::SELECT,
				'options' => [
					'item'       => esc_html__( 'Timeline items', 'omniverse' ),
					'breakpoint' => esc_html__( 'Breakpoint', 'omniverse' ),
				],
				'default' => 'item',
			]
		);
		...
		$repeater->add_control(
			...
			[
				...
				'default'   => '#333333',
				'selectors' => [
					'{{WRAPPER}} {{CURRENT_ITEM}} .omniverse-timeline-breakpoint-title' => 'background-color: {{VALUE}}',
				],
				'condition' => [
					'item_type' => 'breakpoint',
				],
		...
		$repeater->start_controls_tab(
			'secondary_tab',
			[
				'label' => esc_html__( 'Secondary', 'omniverse' ),
			]
		);
		...
		$this->add_control(
			'line_style',
			[
				'label'   => esc_html__( 'Line style', 'omniverse' ),
				'type'    => Controls_Manager::SELECT,
				'options' => [
					'default' => esc_html__( 'Default', 'omniverse' ),
					'dashed'  => esc_html__( 'Dashed', 'omniverse' ),
				],
				'default' => 'default',
			]
		);
		...
		$this->add_control(
			'item_style',
			[
				'label'   => esc_html__( 'Item style', 'omniverse' ),
				'type'    => Controls_Manager::SELECT,
				'options' => [
					'default' => esc_html__( 'Default', 'omniverse' ),
					'shadow'  => esc_html__( 'With shadow', 'omniverse' ),
				],
				'default' => 'default',
			]
		);
		...
		$this->add_control(
			'line_color',
			[
				'label'     => esc_html__( 'Color of line', 'omniverse' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '#e1e1e1',
				'selectors' => [
					'{{WRAPPER}} .dot-start, {{WRAPPER}} .dot-end' => 'background-color: {{VALUE}}',
					'{{WRAPPER}} .omniverse-timeline-line' => 'border-color: {{VALUE}}',
				],
			]
		);
		...
		$this->add_control(
			'dots_color',
			[
				'label'     => esc_html__( 'Color of dots', 'omniverse' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '#1e73be',
				'selectors' => [
					'{{WRAPPER}} .omniverse-timeline-dot' => 'background-color: {{VALUE}}',
				],
			]
		);
			...
			<div class="wd-timeline">
				<?php foreach ( $settings['items'] as $index => $item ) : ?>
					...
					if ( isset( $settings['image_primary']['id'] ) && $settings['image_primary']['id'] ) {
						$image_primary = omniverse_otf_get_image_html( $settings['image_primary']['id'], $settings['image_primary_size'], $settings['image_primary_custom_dimension'] );
					...
					if ( isset( $settings['image_secondary']['id'] ) && $settings['image_secondary']['id'] ) {
						$image_secondary = omniverse_otf_get_image_html( $settings['image_secondary']['id'], $settings['image_secondary_size'], $settings['image_secondary_custom_dimension'] );
					...
					<?php if ( 'breakpoint' === $settings['item_type'] ) : ?>
						<div <?php echo $this->get_render_attribute_string( $repeater_breakpoint_key ); ?>>
							<span class="omniverse-timeline-breakpoint-title">
