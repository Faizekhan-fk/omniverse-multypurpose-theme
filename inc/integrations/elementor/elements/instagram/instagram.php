...
if ( ! function_exists( 'omniverse_elementor_instagram_template' ) ) {
	function omniverse_elementor_instagram_template( $settings ) {
		...
		if ( 'slider' === $settings['design'] ) {
			...
			$carousel_atts   .= ' style="' . omniverse_get_grid_attrs( $settings ) . '"';
		...
		if ( 'images' === $settings['data_source'] ) {
			...
			$media_array = omniverse_get_instagram_custom_images( implode( ',', $images ), $settings['images_size'], $settings['images_link'], $settings['images_likes'], $settings['images_comments'] );
		} else {
			$media_array = omniverse_scrape_instagram( $settings['username'], $settings['number']['size'], $settings['ajax_body'], $settings['data_source'] );
		}
		...
		if ( is_wp_error( $media_array ) && ( $media_array->get_error_code() === 'invalid_response_429' || apply_filters( 'omniverse_intagram_user_ajax_load', false ) || 'ajax' === $settings['data_source'] ) ) {
			...
			for ( $i = 0; $i < $settings['number']['size']; $i++ ) {
				$media_array[] = array(
					$settings['size'] => OMNIVERSE_ASSETS . '/images/settings/instagram/insta-placeholder.jpg',
					'link'            => '#',
					'likes'           => '0',
					'comments'        => '0',
				);
				...
				<div class="<?php echo esc_attr( $pics_classes ); ?>" <?php echo $carousel_atts; ?>>
					...
					<?php foreach ( $media_array as $item ) : ?>
						...
						<div class="wd-insta-item<?php echo esc_attr( $picture_classes ); ?>">
							<a href="<?php echo esc_url( $item['link'] ); ?>" target="<?php echo esc_attr( $settings['target'] ); ?>" aria-label="<?php esc_attr_e( 'Instagram picture', 'omniverse' ); ?>"></a>
