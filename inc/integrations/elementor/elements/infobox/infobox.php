...
if ( ! function_exists( 'omniverse_elementor_infobox_carousel_template' ) ) {
	function omniverse_elementor_infobox_carousel_template( $settings, $element ) {
		...
		<div class="wd-carousel-container info-box-carousel-wrapper<?php echo esc_attr( $extra_classes ); ?>">
			<div class="wd-carousel-inner">
				<div class="wd-carousel wd-grid info-box-carousel<?php echo esc_attr( $wrapper_classes ); ?>" <?php echo omniverse_get_carousel_attributes( $settings ); ?>>
					<div class="wd-carousel-wrap">
						<?php foreach ( $settings['content_repeater'] as $index => $infobox ) : ?>
							...
							?>
							<?php omniverse_elementor_infobox_template( $infobox, $element ); ?>
						<?php endforeach; ?>
					</div>
				</div>
...
if ( ! function_exists( 'omniverse_elementor_infobox_template' ) ) {
	function omniverse_elementor_infobox_template( $settings, $element ) {
		$default_settings = array(
			'link'                        => '',
			'alignment'                   => 'left',
			'image_alignment'             => 'top',
		...
		if ( 'bg-hover' === $settings['style'] ) {
			$wrapper_classes .= ' color-scheme-hover-' . $settings['omniverse_hover_color_scheme'];
		}
		if ( 'yes' === $settings['svg_animation'] ) {
			omniverse_enqueue_js_library( 'vivus' );
			omniverse_enqueue_js_script( 'infobox-element' );
		...
		$title_classes   .= ' ' . omniverse_get_new_size_classes( 'infobox', $settings['title_size'], 'title' );
		$wrapper_classes .= omniverse_get_old_classes( ' box-title-' . $settings['title_size'] );
		$wrapper_classes .= omniverse_get_old_classes( ' omniverse-info-box' );
		...
		$subtitle_classes .= ' ' . omniverse_get_new_size_classes( 'infobox', $settings['title_size'], 'subtitle' );
		...
		if ( $settings['link'] && $settings['link']['url'] && ! $settings['btn_text'] ) {
			...
			$element->add_render_attribute( 'link', 'aria-label', esc_html__( 'Infobox link', 'omniverse' ) );
		}
		...
		if ( isset( $settings['image']['id'] ) && $settings['image']['id'] ) {
			...
			if ( omniverse_is_svg( $settings['image']['url'] ) && apply_filters( 'omniverse_show_infobox_svg_by_tag', true ) ) {
				$image_output = '<span class="info-svg-wrapper info-icon" style="width:' . esc_attr( $custom_image_size['width'] ) . 'px; height:' . esc_attr( $custom_image_size['height'] ) . 'px;">' . omniverse_get_any_svg( $settings['image']['url'], $rand ) . '</span>';
			}
		}
