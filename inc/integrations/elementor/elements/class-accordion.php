...
class Accordion extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Accordion', 'omniverse' );
	}
	...
	protected function register_controls() {
		...
		$repeater->add_control(
			'html_block_id',
			array(
				'label'       => esc_html__( 'HTML Block', 'omniverse' ),
				'type'        => Controls_Manager::SELECT,
			...
			<?php foreach ( $settings['items'] as $index => $item ) : ?>
				...
				if ( 'image' === $item['icon_type'] && isset( $item['image']['id'] ) && $item['image']['id'] ) {
					...
					if ( omniverse_is_svg( $item['image']['url'] ) ) {
						$icon_output = omniverse_get_svg_html( $item['image']['id'], $image_size );
					}
				...
				<div class="wd-accordion-item">
					...
					<div class="wd-accordion-content reset-last-child<?php echo esc_attr( $loop_content_classes_wrapper ); ?>" data-accordion-index="<?php echo esc_attr( $index ); ?>">
						<?php if ( 'html_block' === $item['content_type'] ) : ?>
							<?php echo omniverse_get_html_block( $item['html_block_id'] ); // phpcs:ignore ?>
