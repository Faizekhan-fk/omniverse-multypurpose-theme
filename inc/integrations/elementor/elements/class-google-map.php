...
class Google_Map extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Google map', 'omniverse' );
	}
	...
	protected function register_controls() {
		...
		$repeater->add_control(
			'marker_lat',
			array(
				'label'       => esc_html__( 'Latitude (required)', 'omniverse' ),
				'type'        => Controls_Manager::TEXT,
		...
		$repeater->add_control(
			'marker_lon',
			array(
				'label'   => esc_html__( 'Longitude (required)', 'omniverse' ),
				'type'    => Controls_Manager::TEXT,
				'default' => '-0.12776',
			)
		);
		...
		$this->add_control(
			'google_key',
			array(
				'label'       => esc_html__( 'Google API key', 'omniverse' ),
				'type'        => Controls_Manager::TEXT,
				'description' => wp_kses(
		...
		$this->add_control(
			'multiple_markers',
			array(
				'label'        => esc_html__( 'Multiple markers', 'omniverse' ),
		...
		$this->add_control(
			'style_json',
			array(
				'label'       => esc_html__( 'Styles (JSON)', 'omniverse' ),
				'type'        => 'wd_google_json',
				'description' => sprintf(
					__( 'Styled maps allow you to customize the presentation of the standard Google base maps, changing the visual display of such elements as roads, parks, and built-up areas.%3$s You can find more Google maps styles on the website: %1$s Snazzy Maps %2$s 3$s Just copy JSON code and paste it here.', 'omniverse' ),
		...
		$this->add_control(
			...
			array(
				...
				'description' => esc_html__( 'For a better performance you can initialize the Google map only when you scroll down the page or when you click on it.', 'omniverse' ),
				'type'        => Controls_Manager::SELECT,
				'options'     => array(
	...
	protected function render() {
		...
		$default_settings = array(
			...
			'content_width'             => 300,
			'google_key'                => omniverse_get_opt( 'google_map_api_key' ),
			'marker_icon'               => '',
			'multiple_markers'          => 'no',
		...
		wp_enqueue_script( 'wd-google-map-api', 'https://maps.google.com/maps/api/js?libraries=geometry&callback=omniverseThemeModule.googleMapsCallback&v=weekly&key=' . $settings['google_key'], array( 'omniverse-theme' ), $version, true );
		wp_enqueue_script( 'wd-maplace', OMNIVERSE_THEME_DIR . '/js/libs/maplace' . $minified . '.js', array( 'wd-google-map-api' ), $version, true );

		omniverse_enqueue_js_script( 'google-map-element' );
		...
		if ( is_array( $markers ) && ! empty( $markers ) ) {
			foreach ( $markers as $marker_key => $marker ) {
				if ( isset( $marker['image']['id'] ) && $marker['image']['id'] ) {
					$markers[ $marker_key ]['marker_icon']      = omniverse_otf_get_image_url( $marker['image']['id'], $marker['image_size'], $marker['image_custom_dimension'] );
		...
		if ( isset( $settings['marker_icon']['id'] ) && $settings['marker_icon']['id'] ) {
			$marker_icon = omniverse_otf_get_image_url( $settings['marker_icon']['id'], $settings['marker_icon_size'], $settings['marker_icon_custom_dimension'] );
		} else {
			$marker_icon = OMNIVERSE_ASSETS_IMAGES . '/google-icon.png';
		}
		...
		omniverse_enqueue_inline_style( 'el-google-map' );
