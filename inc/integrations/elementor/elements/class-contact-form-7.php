...
class Contact_Form_7 extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Contact form 7', 'omniverse' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'form_id',
			[
				'label'       => esc_html__( 'Select contact form', 'omniverse' ),
		...
		$this->add_control(
			'style',
			[
				'label'   => esc_html__( 'Color presets', 'omniverse' ),
				'type'    => Controls_Manager::SELECT,
				'options' => [
					''                 => esc_html__( 'Default', 'omniverse' ),
					'wd-style-with-bg' => esc_html__( 'With background', 'omniverse' ),
				],
				'default' => '',
			]
		);
	...
	protected function render() {
		...
		if ( ! $settings['form_id'] || ! defined( 'WPCF7_PLUGIN' ) ) {
			echo '<div class="wd-notice wd-info">' . esc_html__( 'You need to create a form using Contact form 7 plugin to be able to display it using this element.', 'omniverse' ) . '</div>';
			return;
		}
