...
if ( ! function_exists( 'omniverse_elementor_products_tabs_template' ) ) {
	function omniverse_elementor_products_tabs_template( $settings ) {
		$default_settings = [
			// General.
			'title'                         => '',
			'description'                   => '',
		...
		// Title classes.
		if ( omniverse_elementor_is_edit_mode() ) {
		...
		if ( isset( $settings['image']['id'] ) && $settings['image']['id'] ) {
			$image_output = '<span class="img-wrapper">' . omniverse_otf_get_image_html( $settings['image']['id'], $settings['image_size'], $settings['image_custom_dimension'] ) . '</span>';

			if ( omniverse_is_svg( $settings['image']['url'] ) ) {
				$image_output = '<span class="svg-icon img-wrapper" style="width:' . esc_attr( $custom_image_size['width'] ) . 'px; height:' . esc_attr( $custom_image_size['height'] ) . 'px;">' . omniverse_get_any_svg( $settings['image']['url'], rand( 999, 9999 ) ) . '</span>';
			}
		}
		...
		omniverse_enqueue_js_script( 'products-tabs' );
		omniverse_enqueue_inline_style( 'tabs' );
		omniverse_enqueue_inline_style( 'product-tabs' );

		$wrapper_classes .= omniverse_get_old_classes( ' omniverse-products-tabs' );
		...
		<div class="wd-tabs wd-products-tabs<?php echo esc_attr( $wrapper_classes ); ?>">
			<div class="wd-tabs-header<?php echo esc_attr( $header_classes ); ?>">
				...
				<?php if ( $settings['description'] ) : ?>
					<div class="wd-tabs-desc">
						<?php echo wp_kses( $settings['description'], omniverse_get_allowed_html() ); ?>
					</div>
				<?php endif; ?>
				...
				<div class="wd-nav-wrapper wd-nav-tabs-wrapper tabs-navigation-wrapper<?php echo esc_attr( $nav_tabs_wrapper_classes ); ?>">
					...
					<ul class="wd-nav wd-nav-tabs products-tabs-title<?php echo esc_attr( $wd_nav_classes ); ?>">
						<?php foreach ( $settings['tabs_items'] as $key => $item ) : ?>
							...
							$icon_output       = '';
							$item['elementor'] = true;
							$encoded_settings  = wp_json_encode( array_intersect_key( $settings + $item, omniverse_get_elementor_products_config() ) );
							...
							if ( ( ! $item['icon_type'] || 'image' === $item['icon_type'] ) && isset( $item['image']['id'] ) && $item['image']['id'] ) {
								$icon_output = omniverse_otf_get_image_html( $item['image']['id'], $item['image_size'], $item['image_custom_dimension'] );

								if ( omniverse_is_svg( $item['image']['url'] ) ) {
									$icon_output = omniverse_get_svg_html( $item['image']['id'], $custom_icon_size, array( 'class' => 'svg-icon' ) );
								}
							} elseif ( 'icon' === $item['icon_type'] && isset( $item['icon'] ) && $item['icon'] ) {
								$icon_output = omniverse_elementor_get_render_icon( $item['icon'] );
							}
							?>
			...
			<?php if ( isset( $settings['tabs_items'][0] ) ) : ?>
				<?php echo omniverse_elementor_products_tab_template( $settings + $settings['tabs_items'][0] ); ?>
			<?php endif; ?>
		</div>
		<?php
	}
...
if ( ! function_exists( 'omniverse_elementor_products_tab_template' ) ) {
	function omniverse_elementor_products_tab_template( $settings ) {
		...
		$settings = wp_parse_args(
			...
			array_merge(
				...
				omniverse_get_elementor_products_config()
			)
		);
		...
		?>
		<?php if ( ! $is_ajax ) : ?>
			<div class="wd-tab-content-wrapper<?php echo omniverse_get_old_classes( ' omniverse-tab-content' ); ?>">
			<?php omniverse_sticky_loader(); ?>
		<?php endif; ?>

		<?php echo omniverse_elementor_products_template( $settings ); ?>
