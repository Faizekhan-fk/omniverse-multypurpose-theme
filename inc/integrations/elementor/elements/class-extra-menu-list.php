...
class Extra_Menu_List extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Extra menu list', 'omniverse' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'label',
			[
				'label' => esc_html__( 'Label text (optional)', 'omniverse' ),
				'type'  => Controls_Manager::TEXT,
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'secondary' => esc_html__( 'Secondary', 'omniverse' ),
					'red'       => esc_html__( 'Red', 'omniverse' ),
					'green'     => esc_html__( 'Green', 'omniverse' ),
					'blue'      => esc_html__( 'Blue', 'omniverse' ),
					'orange'    => esc_html__( 'Orange', 'omniverse' ),
					'grey'      => esc_html__( 'Grey', 'omniverse' ),
		...
		$this->add_control(
			...
			[
				...
				'label'       => esc_html__( 'List items', 'omniverse' ),
				'separator'   => 'before',
				'title_field' => '{{{ title }}}',
	...
	protected function render() {
		...
		$this->add_render_attribute(
			[
				'parent_ul'    => [
					'class' => [
						'wd-sub-menu',
						omniverse_get_old_classes( 'sub-menu' ),
						'mega-menu-list',
					],
				],
				'parent_li'    => [
		...
		$link_attrs = omniverse_get_link_attrs( $settings['link'] );
				...
				<li <?php echo $this->get_render_attribute_string( 'parent_li' ); ?>>
					<?php if ( $settings['title'] ) : ?>
						<a <?php echo $link_attrs; ?>>
							...
							<?php if ( omniverse_elementor_is_edit_mode() ) : ?>
					...
					<ul class="sub-sub-menu">
						<?php foreach ( $settings['menu_items_repeater'] as $index => $item ) : ?>
							...
							<li <?php echo $this->get_render_attribute_string( $repeater_li_key ); ?>>
								<a <?php echo $link_attrs; ?>>
									<?php if ( $item['image'] ) : ?>
										<?php echo omniverse_otf_get_image_html( $item['image']['id'], $item['image_size'], $item['image_custom_dimension'] );
										?>
									<?php endif; ?>
									...
									<?php endif; ?>
										<?php echo wp_kses( $item['title'], omniverse_get_allowed_html() ); ?>
									<?php if ( omniverse_elementor_is_edit_mode() ) : ?>
										</span>
									<?php endif; ?>
									...
									<?php if ( $item['label'] ) : ?>
										<span <?php echo $this->get_render_attribute_string( $repeater_label_key ); ?>>
											<?php echo wp_kses( $item['label'], omniverse_get_allowed_html() ); ?>
										</span>
									<?php endif; ?>
								</a>
							</li>
