...
if ( ! function_exists( 'omniverse_elementor_blog_template' ) ) {
	function omniverse_elementor_blog_template( $settings ) {
		$default_settings = [
			// General.
			'post_type'               => 'post',
			...
			'blog_spacing'            => omniverse_get_opt( 'blog_spacing' ),
			'blog_spacing_tablet'     => omniverse_get_opt( 'blog_spacing_tablet', '' ),
			'blog_spacing_mobile'     => omniverse_get_opt( 'blog_spacing_mobile', '' ),
			'pagination'              => '',
			...
			'ajax_page'               => '',
			'custom_sizes'            => apply_filters( 'omniverse_blog_shortcode_custom_sizes', false ),
			'elementor'               => true,
		];
		...
		// Loop.
		omniverse_set_loop_prop( 'blog_type', 'shortcode' );
		omniverse_set_loop_prop( 'blog_design', $settings['blog_design'] );
		omniverse_set_loop_prop( 'img_size', $settings['img_size'] );
		omniverse_set_loop_prop( 'blog_columns', $settings['blog_columns'] );
		omniverse_set_loop_prop( 'omniverse_loop', 0 );
		omniverse_set_loop_prop( 'parts_title', $settings['parts_title'] );
		omniverse_set_loop_prop( 'parts_meta', $settings['parts_meta'] );
		omniverse_set_loop_prop( 'parts_text', $settings['parts_text'] );
		omniverse_set_loop_prop( 'parts_btn', $settings['parts_btn'] );
		omniverse_set_loop_prop( 'parts_media', $settings['parts_media'] );
		...
		if ( isset( $settings['blog_columns_tablet']['size'] ) && $settings['blog_columns_tablet']['size'] ) {
			omniverse_set_loop_prop( 'blog_columns_tablet', $settings['blog_columns_tablet']['size'] );
		}
		if ( isset( $settings['blog_columns_mobile']['size'] ) && $settings['blog_columns_mobile']['size'] ) {
			omniverse_set_loop_prop( 'blog_columns_mobile', $settings['blog_columns_mobile']['size'] );
		}

		if ( 'carousel' === $settings['blog_design'] ) {
			omniverse_set_loop_prop( 'blog_design', $settings['blog_carousel_design'] );
		}
		if ( '' === $settings['blog_spacing'] ) {
			$settings['blog_spacing'] = omniverse_get_opt( 'blog_spacing' );

			if ( '' === $settings['blog_spacing_tablet'] ) {
				$settings['blog_spacing_tablet'] = omniverse_get_opt( 'blog_spacing_tablet' );
			}
			if ( '' === $settings['blog_spacing_mobile'] ) {
				$settings['blog_spacing_mobile'] = omniverse_get_opt( 'blog_spacing_mobile' );
			}
		}
//		if ( ! $settings['parts_btn'] ) {
//			omniverse_set_loop_prop( 'parts_btn', false );
//		}
		...
		if ( 'carousel' === $settings['blog_design'] ) {
			...
			return omniverse_generate_posts_slider( $settings, $blog_query );
		} else {
			$attributes      = '';
			$wrapper_classes = '';
			...
			<?php endif; ?>
				...
				<?php while ( $blog_query->have_posts() ) : ?>
					...
					<?php $name = omniverse_is_blog_design_new( $settings['blog_design'] ) ? $settings['blog_design'] : ''; ?>
			...
			<?php if ( $blog_query->max_num_pages > 1 && ! $is_ajax && $settings['pagination'] ) : ?>
				<div class="wd-loop-footer blog-footer">
					<?php if ( 'infinit' === $settings['pagination'] || 'more-btn' === $settings['pagination'] ) : ?>
						...
						<a href="#" data-holder-id="<?php echo esc_attr( $id ); ?>" rel="nofollow noopener" class="btn wd-load-more wd-blog-load-more load-on-<?php echo 'more-btn' === $settings['pagination'] ? 'click' : 'scroll'; ?>"><span class="load-more-label"><?php esc_html_e( 'Load more posts', 'omniverse' ); ?></span></a>
