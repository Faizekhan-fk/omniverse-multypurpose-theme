...
if ( ! function_exists( 'omniverse_get_elementor_products_config' ) ) {
	/**
	 ...
	 * @return array
	 */
	function omniverse_get_elementor_products_config() {
		return array_merge(
			...
			array(
				...
				'custom_sizes'                 => apply_filters( 'omniverse_products_shortcode_custom_sizes', false ),
				'elementor'                    => true,
				'wrapper_classes'              => '',
			)
		);
...
if ( ! function_exists( 'omniverse_elementor_products_template' ) ) {
	function omniverse_elementor_products_template( $settings ) {
		if ( ! omniverse_woocommerce_installed() ) {
			return '';
		}
		global $product;

		$default_settings = omniverse_get_elementor_products_config();
		...
		$is_ajax                    = omniverse_is_woo_ajax() && 'yes' !== $settings['force_not_ajax'] && isset( $_POST['action'] ) && $_POST['action'] !== 'omniverse_load_full_search_html' && $_POST['action'] !== 'omniverse_load_html_dropdowns';
		...
		if ( 'yes' === $settings['hide_out_of_stock'] || apply_filters( 'omniverse_hide_out_of_stock_items', false ) && 'yes' === get_option( 'woocommerce_hide_out_of_stock_items' ) ) {
			$query_args['meta_query'][] = array(
				'key'     => '_stock_status',
				'value'   => 'outofstock',
				'compare' => 'NOT IN',
		...
		if ( 'top_rated_products' === $settings['post_type'] ) {
			...
			$products = new WP_Query( apply_filters( 'omniverse_product_element_query_args', $query_args ) );
		...
		if ( 'inherit' === $settings['product_hover'] ) {
			$settings['product_hover'] = omniverse_get_opt( 'products_hover' );
			$settings['stretch_product'] = omniverse_get_opt( 'stretch_product_desktop' );
			$settings['stretch_product_tablet'] = omniverse_get_opt( 'stretch_product_tablet' );
			$settings['stretch_product_mobile'] = omniverse_get_opt( 'stretch_product_mobile' );
		}

		// Loop settings.
		omniverse_set_loop_prop( 'timer', $settings['sale_countdown'] );
		omniverse_set_loop_prop( 'progress_bar', $settings['stock_progress_bar'] );
		omniverse_set_loop_prop( 'stretch_product_desktop', $settings['stretch_product'] );
		omniverse_set_loop_prop( 'stretch_product_tablet', $settings['stretch_product_tablet'] );
		omniverse_set_loop_prop( 'stretch_product_mobile', $settings['stretch_product_mobile'] );
		omniverse_set_loop_prop( 'product_hover', $settings['product_hover'] );
		omniverse_set_loop_prop( 'products_view', $settings['layout'] );
		...
		if ( ! empty( $settings['grid_gallery'] ) ) {
			omniverse_set_loop_prop( 'grid_gallery', $settings['grid_gallery'] );

			if ( ! empty( $settings['grid_gallery_control'] ) ) {
				omniverse_set_loop_prop( 'grid_gallery_control', $settings['grid_gallery_control'] );
			}
			if ( ! empty( $settings['grid_gallery_enable_arrows'] ) ) {
				omniverse_set_loop_prop( 'grid_gallery_enable_arrows', $settings['grid_gallery_enable_arrows'] );
			}
		}
		...
		if ( isset( $settings['columns']['size'] ) ) {
			omniverse_set_loop_prop( 'products_columns', $settings['columns']['size'] );
		}
		...
		if ( $settings['products_masonry'] ) {
			omniverse_set_loop_prop( 'products_masonry', 'enable' === $settings['products_masonry'] );
		}
		if ( $settings['products_different_sizes'] ) {
			omniverse_set_loop_prop( 'products_different_sizes', 'enable' === $settings['products_different_sizes'] );
		}
		if ( $settings['product_quantity'] ) {
			omniverse_set_loop_prop( 'product_quantity', 'enable' === $settings['product_quantity'] );
		}
		if ( 'arrows' !== $settings['pagination'] && $settings['items_per_page'] ) {
			omniverse_set_loop_prop( 'woocommerce_loop', $settings['items_per_page'] * ( $paged - 1 ) );
		}
		...
		if ( 'carousel' === $settings['layout'] ) {
			...
			omniverse_enqueue_product_loop_styles( $settings['product_hover'] );
			...
			$output = omniverse_generate_posts_slider(
				wp_parse_args(
					array(
		...
		if ( 'none' !== omniverse_get_opt( 'product_title_lines_limit' ) && 'list' !== $settings['layout'] ) {
		...
		$products_element_classes .= $settings['highlighted_products'] ? omniverse_get_old_classes( ' omniverse-highlighted-products' ) : '';
		...
		if ( ( omniverse_loop_prop( 'stretch_product_desktop' ) || omniverse_loop_prop( 'stretch_product_tablet' ) || omniverse_loop_prop( 'stretch_product_mobile' ) ) && in_array( $settings['product_hover'],
		...
		<?php if ( ! $is_ajax || 'yes' === $settings['ajax_recently_viewed'] ) : ?>
			...
			<?php if ( 'arrows' === $settings['pagination'] ) : ?>
				<?php omniverse_enqueue_inline_style( 'product-arrows' ); ?>
				<?php omniverse_sticky_loader(); ?>
			<?php endif; ?>
	...
	<?php endif; ?>
		...
		<?php omniverse_set_loop_prop( 'shop_pagination', $settings['pagination'] ); ?>
		...
		<?php if ( $products->max_num_pages > 1 && ! $is_ajax && $settings['pagination'] ) : ?>
			...
			<?php if ( 'more-btn' === $settings['pagination'] || 'infinit' === $settings['pagination'] ) : ?>
				<div class="wd-loop-footer products-footer">
					...
					<a href="#" rel="nofollow noopener" class="btn wd-load-more wd-products-load-more load-on-<?php echo 'more-btn' === $settings['pagination'] ? 'click' : 'scroll'; ?>"><span class="load-more-label"><?php esc_html_e( 'Load more products', 'omniverse' ); ?></span></a>
