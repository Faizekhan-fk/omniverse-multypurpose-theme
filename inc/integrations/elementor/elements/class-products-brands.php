...
class Products_Brands extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Brands', 'omniverse' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'options' => array(
					...
					'term_id' => esc_html__( 'ID', 'omniverse' ),
					'slug'    => esc_html__( 'Slug', 'omniverse' ),
					'random'  => esc_html__( 'Random order', 'omniverse' ),
				),
			)
		);
		...
		$this->add_control(
			'ids',
			array(
				'label'       => esc_html__( 'Brands', 'omniverse' ),
				'description' => esc_html__( 'List of product brands to show. Leave empty to show all.', 'omniverse' ),
				'type'        => 'wd_autocomplete',
				'search'      => 'omniverse_get_taxonomies_by_query',
				'render'      => 'omniverse_get_taxonomies_title_by_id',
				'taxonomy'    => omniverse_get_opt( 'brands_attribute' ),
				'multiple'    => true,
				'label_block' => true,
			)
		);
		...
		$this->add_control(
			...
			array(
				'label'        => esc_html__( 'Filter in current category', 'omniverse' ),
				'description'  => esc_html__( ' Enable this option and all brand links will work inside the current category page. Or it will lead to the shop page if you are not on the category page.', 'omniverse' ),
				'type'         => Controls_Manager::SWITCHER,
				'default'      => 'no',
				'label_on'     => esc_html__( 'Yes', 'omniverse' ),
				'label_off'    => esc_html__( 'No', 'omniverse' ),
		...
		$this->add_control(
			'disable_link',
			array(
				'label'        => esc_html__( 'Disable links', 'omniverse' ),
				'type'         => Controls_Manager::SWITCHER,
				'default'      => 'no',
				'label_on'     => esc_html__( 'Yes', 'omniverse' ),
				'label_off'    => esc_html__( 'No', 'omniverse' ),
		...
		$this->add_control(
			...
			array(
				...
				'options' => array(
					...
					'list'     => esc_html__( 'Links List', 'omniverse' ),
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'options' => array(
					...
					'alt'     => esc_html__( 'Alternate', 'omniverse' ),
				),
			)
		);
	...
	protected function render() {
		$default_settings = array(
			...
			'disable_link'               => 'no',
			'custom_sizes'               => apply_filters( 'omniverse_brands_shortcode_custom_sizes', false ),
			'with_bg_color'              => 'no',
		);
		...
		if ( empty( $attribute ) || ! taxonomy_exists( $attribute ) ) {
			echo '<div class="wd-notice wd-info">' . esc_html__( 'You must select your brand attribute in Theme Settings -> Shop -> Brands', 'omniverse' ) . '</div>';
			return;
		}
		...
		if ( 'carousel' === $settings['style'] ) {
			...
			$this->add_render_attribute( 'items_wrapper', 'style', omniverse_get_grid_attrs( $settings ) );
		...
		if ( omniverse_is_shop_on_front() ) {
			...
			$link = omniverse_get_current_url();
		} else {
			$link = get_post_type_archive_link( 'product' );
		}

		omniverse_enqueue_inline_style( 'brands' );

		if ( 'bordered' === $settings['brand_style'] ) {
			omniverse_enqueue_inline_style( 'brands-style-bordered' );
		}
