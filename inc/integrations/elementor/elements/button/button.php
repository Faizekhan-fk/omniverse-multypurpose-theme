...
if ( ! function_exists( 'omniverse_elementor_button_template' ) ) {
	function omniverse_elementor_button_template( $settings ) {
		$default_settings = [
			'text'                        => 'Read more',
			'link'                        => '',
		...
		$wrapper_classes .= omniverse_get_old_classes( ' omniverse-button-wrapper' );
		...
		if ( 'icon' === $settings['icon_type'] && $settings['icon'] ) {
			...
			$icon_output   = omniverse_elementor_get_render_icon(
				$settings['icon'],
				[
					'class' => 'wd-icon',
				],
		...
		// Text classes.
		if ( omniverse_elementor_is_edit_mode() && $settings['inline_edit'] ) {
