...
if ( ! function_exists( 'omniverse_elementor_banner_template' ) ) {
	function omniverse_elementor_banner_carousel_template( $settings, $element ) {
		...
		$settings           = wp_parse_args( $settings, array_merge( omniverse_get_carousel_atts(), $default_settings ) );
		$carousel_classes   = '';
		$wrapper_classes    = '';
		...
		<div class="wd-carousel-container banners-carousel-wrapper<?php echo esc_attr( $wrapper_classes ); ?>">
			<div class="wd-carousel-inner">
				<div class="wd-carousel wd-grid banners-carousel<?php echo esc_attr( $carousel_classes ); ?>" <?php echo omniverse_get_carousel_attributes( $settings ); ?>>
					<div class="wd-carousel-wrap">
						<?php foreach ( $settings['content_repeater'] as $index => $banner ) : ?>
							...
							?>
							<?php omniverse_elementor_banner_template( $banner, $element ); ?>
						<?php endforeach; ?>
					</div>
				</div>
...
if ( ! function_exists( 'omniverse_elementor_banner_template' ) ) {
	function omniverse_elementor_banner_template( $settings, $element ) {
		$default_settings = array(
			'source_type'                => 'image',
			'video'                      => array( 'id' => '' ),
		...
		$banner_classes .= ' color-scheme-' . $settings['omniverse_color_scheme'];
		...
		// Subtitle classes.
		if ( omniverse_elementor_is_edit_mode() && ! strstr( $settings['wrapper_classes'], 'elementor-repeater-item' ) ) {
		...
		$subtitle_classes .= ' ' . omniverse_get_new_size_classes( 'banner', $settings['title_size'], 'subtitle' );
		...
		$banner_classes          .= omniverse_get_old_classes( ' banner-vr-align-' . $settings['vertical_alignment'] );
		$banner_classes          .= omniverse_get_old_classes( ' banner-hr-align-' . $settings['horizontal_alignment'] );
		...
		$title_classes .= ' ' . omniverse_get_new_size_classes( 'banner', $settings['title_size'], 'title' );

		// Content classes.
		if ( omniverse_elementor_is_edit_mode() && ! strstr( $settings['wrapper_classes'], 'elementor-repeater-item' ) ) {
			$inner_classes .= ' elementor-inline-editing';
		}
		$inner_classes .= ' ' . omniverse_get_new_size_classes( 'banner', $settings['content_text_size'], 'content' );
		...
		if ( $settings['link'] && $settings['link']['url'] ) {
			...
			$element->add_render_attribute( 'link', 'aria-label', esc_html__( 'Banner link', 'omniverse' ) );
		...
		if ( 'image' === $settings['source_type'] ) {
			if ( $settings['image']['id'] ) {
				$image_url = omniverse_otf_get_image_url( $settings['image']['id'], $settings['image_size'], $settings['image_custom_dimension'] );
		...
		if ( 'video' === $settings['source_type'] && ! empty( $settings['video_poster']['id'] ) ) {
			$video_attrs .= ' poster="' . omniverse_otf_get_image_url( $settings['video_poster']['id'], $settings['video_poster_size'], $settings['video_poster_custom_dimension'] ) . '"';
		}
					...
					<div class="banner-image<?php echo esc_attr( $banner_image_classes ); ?>">
						<?php if ( 'image' === $settings['source_type'] ) : ?>
							<?php if ( 'parallax' !== $settings['hover'] && $settings['image']['id'] ) : ?>
								<?php echo omniverse_otf_get_image_html( $settings['image']['id'], $settings['image_size'], $settings['image_custom_dimension'] ); ?>
							<?php elseif ( $image_url ) : ?>
								<?php echo apply_filters( 'omniverse_image', '<img src="' . esc_url( $image_url ) . '" class="promo-banner-image" alt="promo-banner-image">' ); ?>
							<?php endif; ?>
					...
					<div class="content-banner <?php echo esc_attr( $content_classes ); ?>">
						...
						<?php if ( $settings['btn_text'] ) : ?>
							<div class="banner-btn-wrapper<?php echo esc_attr( $btn_wrapper_classes ); ?>">
								...
								omniverse_elementor_button_template(
									array(
