...
class Pricing_Tables extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Pricing tables', 'omniverse' );
	}
	...
	protected function register_controls() {
		...
		$repeater->add_control(
			'name',
			[
				'label'   => esc_html__( 'Pricing plan name', 'omniverse' ),
		...
		$repeater->add_control(
			'features_list',
			[
				'label'       => esc_html__( 'Featured list', 'omniverse' ),
				'description' => esc_html__( 'Start each feature text from a new line', 'omniverse' ),
...
Feature 3',
		...
		$repeater->add_control(
			'price_value',
			[
				'label'   => esc_html__( 'Price value', 'omniverse' ),
				'type'    => Controls_Manager::TEXT,
				'default' => '199',
			]
		);
		...
		$repeater->add_control(
			'price_suffix',
			[
				'label'   => esc_html__( 'Price suffix', 'omniverse' ),
				'type'    => Controls_Manager::TEXT,
				'default' => 'per month',
			]
		);
		...
		$repeater->add_control(
			'currency',
			[
				'label'   => esc_html__( 'Price currency', 'omniverse' ),
				'type'    => Controls_Manager::TEXT,
				'default' => '$',
			]
		);
		...
		$repeater->add_control(
			...
			[
				...
				'options' => [
					...
					'product' => esc_html__( 'Product add to cart', 'omniverse' ),
				],
				'default' => 'custom',
			]
		);
		...
		$repeater->add_control(
			'product_id',
			[
				'label'       => esc_html__( 'Select identificator', 'omniverse' ),
				'description' => esc_html__( 'Input product ID or product SKU or product title to see suggestions.', 'omniverse' ),
		...
		$repeater->add_control(
			...
			[
				...
				'options' => [
					''       => '',
					'red'    => esc_html__( 'Red', 'omniverse' ),
					'green'  => esc_html__( 'Green', 'omniverse' ),
					'blue'   => esc_html__( 'Blue', 'omniverse' ),
					'yellow' => esc_html__( 'Yellow', 'omniverse' ),
				],
				'default' => 'red',
			]
		);
		...
		$repeater->add_control(
			'best_option',
			[
				'label'        => esc_html__( 'Best option', 'omniverse' ),
				'description'  => esc_html__( 'Highlight this price plan as best value with extra styles.', 'omniverse' ),
		...
		$repeater->add_control(
			'with_bg_image',
			[
				'label'        => esc_html__( 'With background image', 'omniverse' ),
				'type'         => Controls_Manager::SWITCHER,
				'default'      => 'no',
				'label_on'     => esc_html__( 'Yes', 'omniverse' ),
				'label_off'    => esc_html__( 'No', 'omniverse' ),
...
21.16 megapixel',
	...
	protected function render() {
		...
		omniverse_enqueue_inline_style( 'pricing-table' );
			...
			<div <?php echo $this->get_render_attribute_string( 'wrapper' ); ?>>
				<?php foreach ( $settings['items'] as $index => $item ) : ?>
					...
					if ( 'yes' === $settings['with_bg_image'] && $settings['bg_image'] ) {
						...
						$bg_style = 'background-image:url(' . esc_url( omniverse_otf_get_image_url( $settings['bg_image']['id'], $settings['bg_image_size'], $settings['bg_image_custom_dimension'] ) ) . ')';
					}
						...
						<div class="wd-plan">
							<div class="wd-plan-name">
								<span <?php echo $this->get_render_attribute_string( $repeater_name_key ); ?>>
									<?php echo wp_kses( $settings['name'], omniverse_get_allowed_html() ); ?>
								</span>
							</div>
						</div>
						...
						<div class="wd-plan-inner">
							<?php if ( $settings['label'] ) : ?>
								<div class="wd-plan-label price-label">
									<span>
										<?php echo wp_kses( $settings['label'], omniverse_get_allowed_html() ); ?>
									</span>
								</div>
							<?php endif ?>
							...
							<div class="wd-plan-price" style="<?php echo esc_attr( $bg_style ); ?>">
								<?php if ( $settings['currency'] ) : ?>
									<span <?php echo $this->get_render_attribute_string( $repeater_currency_key ); ?>>
										<?php echo wp_kses( $settings['currency'], omniverse_get_allowed_html() ); ?>
									</span>
								<?php endif ?>
								
								<?php if ( $settings['price_value'] ) : ?>
									<span <?php echo $this->get_render_attribute_string( $repeater_value_key ); ?>>
										<?php echo wp_kses( $settings['price_value'], omniverse_get_allowed_html() ); ?>
									</span>
								<?php endif ?>
								
								<?php if ( $settings['price_suffix'] ) : ?>
									<span <?php echo $this->get_render_attribute_string( $repeater_suffix_key ); ?>>
										<?php echo wp_kses( $settings['price_suffix'], omniverse_get_allowed_html() ); ?>
									</span>
								<?php endif ?>
							</div>
								...
								<div class="wd-plan-features">
									<?php foreach ( $features as $value ) : ?>
										<div class="wd-plan-feature">
											<?php echo wp_kses( $value, omniverse_get_allowed_html() ); ?>
										</div>
									<?php endforeach; ?>
								</div>
							<?php endif ?>
							...
							<div <?php echo $this->get_render_attribute_string( $repeater_footer_key ); ?>>
								...
								<?php else : ?>
									<?php if ( $settings['button_label'] ) : ?>
										<a <?php echo omniverse_get_link_attrs( $settings['link'] ); ?> <?php echo $this->get_render_attribute_string( $repeater_btn_key ); ?>>
											<?php echo wp_kses( $settings['button_label'], omniverse_get_allowed_html() ); ?>
										</a>
									<?php endif ?>
								<?php endif ?>
							</div>
