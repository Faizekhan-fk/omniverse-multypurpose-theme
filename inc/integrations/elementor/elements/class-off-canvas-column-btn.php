...
class Off_Canvas_Column_Btn extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Off canvas column button', 'omniverse' );
	}
	...
	protected function register_controls() {
		$sticky_key = omniverse_is_elementor_pro_installed() ? 'wd_sticky' : 'sticky';
		/**
		 * Content tab
		 */
		...
		$this->add_control(
			$sticky_key,
			array(
				'label'        => esc_html__( 'Sticky', 'omniverse' ),
				'description'  => esc_html__( 'Make the off canvas sidebar button sticky.', 'omniverse' ),
		...
		$this->add_control(
			...
			array(
				...
				'options' => array(
					...
					'custom'  => esc_html__( 'Custom image', 'omniverse' ),
				),
				'default' => 'default',
			)
		);
	...
	protected function render() {
		...
		omniverse_enqueue_js_script( 'off-canvas-colum-btn' );
		omniverse_enqueue_inline_style( 'el-off-canvas-column-btn' );
		...
		$off_canvas_classes .= omniverse_get_old_classes( ' omniverse-show-sidebar-btn' );

		if ( 'custom' === $settings['icon_type'] && ! empty( $settings['icon']['id'] ) ) {
			if ( omniverse_is_svg( $settings['icon']['url'] ) ) {
				$icon_output = omniverse_get_svg_html(
					$settings['icon']['id'],
					$icon_size
				);
			} else {
				$icon_output = omniverse_otf_get_image_html( $settings['icon']['id'], $settings['icon_size'], $settings['icon_custom_dimension'] );
			}
		}

		if ( 'yes' === $settings['sticky'] || 'yes' === $settings['wd_sticky'] ) {
			omniverse_enqueue_inline_style( 'mod-sticky-sidebar-opener' );
		}

		omniverse_enqueue_inline_style( 'off-canvas-sidebar' );
		?>
