...
class Icon_List extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'List', 'omniverse' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'options' => array(
					...
					'image'     => esc_html__( 'With image', 'omniverse' ),
					'ordered'   => esc_html__( 'Ordered', 'omniverse' ),
					'unordered' => esc_html__( 'Unordered', 'omniverse' ),
		...
		$this->add_control(
			...
			array(
				...
				'options' => array(
					...
					'extra-large' => esc_html__( 'Extra Large (22px)', 'omniverse' ),
				),
				'default' => 'default',
			)
		);
		...
		$this->add_control(
			'text_color_hover',
			array(
				'label'     => esc_html__( 'Text color hover', 'omniverse' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '',
				'selectors' => array(
		...
		$this->add_responsive_control(
			'list_items_gap',
			array(
				'label'          => esc_html__( 'List items gap', 'omniverse' ),
		...
		$this->add_control(
			'icons_color',
			array(
				'label'     => esc_html__( 'Icons color', 'omniverse' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '#2C2C2C',
				'selectors' => array(
		...
		$this->add_control(
			'icons_color_hover',
			array(
				'label'     => esc_html__( 'Icons color hover', 'omniverse' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '',
				'selectors' => array(
		...
		$this->add_control(
			'icons_bg_color',
			array(
				'label'     => esc_html__( 'Icons background color', 'omniverse' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '#EAEAEA',
				'selectors' => array(
		...
		$this->add_control(
			...
			array(
				'label'     => esc_html__( 'Icons background color hover', 'omniverse' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '',
				'selectors' => array(
	...
	protected function render() {
			...
			array(
				'list' => array(
					'class' => array(
						...
						omniverse_get_new_size_classes( 'list', $settings['size'], 'text' ),
		...
		if ( 'image' === $settings['list_type'] && isset( $settings['image']['id'] ) && $settings['image']['id'] ) {
			...
			$icon_output = omniverse_elementor_get_render_icon( $settings['icon'], array( 'class' => 'list-icon wd-icon' ), 'span' );
		}
		...
		<ul <?php echo $this->get_render_attribute_string( 'list' ); ?>>
			<?php foreach ( $settings['list_items'] as $index => $item ) : ?>
				...
				if ( 'image' === $item['item_type'] && isset( $item['image']['id'] ) && $item['image']['id'] ) {
					...
					if ( omniverse_is_svg( wp_get_attachment_image_url( $item['image']['id'] ) ) ) {
						if ( 'custom' === $item['image_size'] && ! empty( $item['image_custom_dimension'] ) ) {
							$item_icon_output = omniverse_get_svg_html( $item['image']['id'], $item['image_custom_dimension'] );
						} else {
							$item_icon_output = omniverse_get_svg_html( $item['image']['id'], $item['image_size'] );
						}
					}
				}
				?>
				<li class="elementor-repeater-item-<?php echo esc_attr( $item['_id'] ); ?>">
					...
					<?php if ( isset( $item['link']['url'] ) && $item['link']['url'] ) : ?>
						...
						<a <?php echo $link_attrs ?> aria-label="<?php esc_attr_e( 'List item link', 'omniverse' ); ?>"></a>
					<?php endif; ?>
				</li>
			<?php endforeach ?>
		</ul>
