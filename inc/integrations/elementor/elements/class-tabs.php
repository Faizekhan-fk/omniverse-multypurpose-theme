...
class Tabs extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Tabs', 'omniverse' );
	}
	...
	protected function register_controls() {
		...
		$repeater->add_control(
			...
			array(
				...
				'placeholder' => esc_html__( 'Tab Title', 'omniverse' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => 'Tab Title',
			)
		);
		...
		$repeater->add_control(
			...
			array(
				...
				'description' => esc_html__( 'You can display content as a simple text or if you need more complex structure you can create an HTML Block with Elementor builder and place it here.', 'omniverse' ),
		...
		$this->add_control(
			'tabs',
			array(
				'label'   => esc_html__( 'Tabs items', 'omniverse' ),
		...
		$this->add_control(
			...
			array(
				'label'     => esc_html__( 'Custom color', 'omniverse' ),
				'type'      => Controls_Manager::COLOR,
				'selectors' => array(
	...
	protected function render() {
		...
		omniverse_enqueue_js_script( 'tabs-element' );
		...
		if ( isset( $tabs_parent_settings['image']['id'] ) && $tabs_parent_settings['image']['id'] ) {
			$image_output = '<span class="img-wrapper">' . omniverse_otf_get_image_html( $tabs_parent_settings['image']['id'], $tabs_parent_settings['image_size'], $tabs_parent_settings['image_custom_dimension'] ) . '</span>';

			if ( omniverse_is_svg( $tabs_parent_settings['image']['url'] ) ) {
				$image_output = '<span class="svg-icon img-wrapper" style="width:' . esc_attr( $tabs_parent_settings['width'] ) . 'px; height:' . esc_attr( $custom_image_size['height'] ) . 'px;">' . omniverse_get_any_svg( $tabs_parent_settings['image']['url'], wp_rand( 999, 9999 ) ) . '</span>';
			}
		}
			...
			<div class="wd-tabs-header<?php echo esc_attr( $nav_tabs_wrapper_classes ); ?>">
				<?php if ( $tabs_parent_settings['title'] ) : ?>
					<div class="tabs-name title">
						...
						<span class="tabs-text" data-elementor-setting-key="title">
							<?php echo wp_kses( $tabs_parent_settings['title'], omniverse_get_allowed_html() ); ?>
						</span>
					</div>
				<?php endif; ?>
				...
				<?php if ( $tabs_parent_settings['description'] ) : ?>
					<div class="wd-tabs-desc">
						<?php echo wp_kses( $tabs_parent_settings['description'], omniverse_get_allowed_html() ); ?>
					</div>
				<?php endif; ?>
					...
					<ul class="wd-nav wd-nav-tabs<?php echo esc_attr( $nav_tabs_title_classes ); ?>">
						<?php foreach ( $tabs_child_settings as $tab ) : ?>
							...
							if ( 'image' === $tab['icon_type'] && isset( $tab['image']['id'] ) && $tab['image']['id'] ) {
								$icon_output = omniverse_otf_get_image_html( $tab['image']['id'], $tab['image_size'], $tab['image_custom_dimension'] );

								if ( omniverse_is_svg( $tab['image']['url'] ) ) {
									$icon_output = omniverse_get_svg_html( $tab['image']['id'], $image_size, array( 'class' => 'svg-icon' ) );
								}
							} elseif ( 'icon' === $tab['icon_type'] && $tab['icon'] ) {
								$icon_output = omniverse_elementor_get_render_icon( $tab['icon'] );
							}
							?>
				...
				<?php foreach ( $tabs_child_settings as $index => $tab ) : ?>
					...
					<div class="wd-tab-content<?php echo esc_attr( $content_classes_active ); ?>">
						<?php if ( 'html_block' === $tab['content_type'] ) : ?>
							<?php echo omniverse_get_html_block( $tab['content_html_block'] ); // phpcs:ignore ?>
