...
if ( ! function_exists( 'omniverse_elementor_get_content' ) ) {
	/**
	 ...
	 * @return string
	 */
	function omniverse_elementor_get_content( $id, $inline_css = false ) {
		ob_start();

		$post = new Elementor\Core\Files\CSS\Post( $id );
		$meta = $post->get_meta();

		if ( ! $inline_css && $post::CSS_STATUS_FILE === $meta['status'] && apply_filters( 'omniverse_elementor_content_file_css', true ) && ! omniverse_is_woo_ajax() ) {
			?>
			...
			<?php
		} elseif ( ! $inline_css && ( ! apply_filters( 'omniverse_elementor_content_file_css', true ) || omniverse_is_woo_ajax() ) ) {
...
if ( ! function_exists( 'omniverse_get_link_attrs' ) ) {
	/**
	 * Get image url
	 *
	 * @since 1.0.0
	 ...
	 * @return string
	 */
	function omniverse_get_link_attrs( $link ) {
		$link_attrs = '';
...
if ( ! function_exists( 'omniverse_elementor_get_render_icon' ) ) {
	/**
	 * Render Icon
	 *
	 * @since 1.0.0
	 ...
	 * @return mixed|string
	 */
	function omniverse_elementor_get_render_icon( $icon, $attributes = [], $tag = 'i' ) {
		ob_start();
...
if ( ! function_exists( 'omniverse_elementor_has_location' ) ) {
	/**
	 * Custom shapes.
	 *
	 * @since 1.0.0
	 ...
	 * @return bool
	 */
	function omniverse_elementor_has_location( $location ) {
...
if ( ! function_exists( 'omniverse_get_image_html' ) ) {
	/**
	 * Get image url
	 *
	 * @since 1.0.0
	 ...
	 * @return string
	 */
	function omniverse_get_image_html( $settings, $image_size_key = '' ) {
...
if ( ! function_exists( 'omniverse_elementor_is_edit_mode' ) ) {
	/**
	 * Whether the edit mode is active.
	 *
	 * @since 1.0.0
	 */
	function omniverse_elementor_is_edit_mode() {
		if ( ! omniverse_is_elementor_installed() ) {
			return false;
		}
...
if ( ! function_exists( 'omniverse_elementor_is_preview_mode' ) ) {
	/**
	 * Whether the preview mode is active.
	 *
	 * @since 1.0.0
	 */
	function omniverse_elementor_is_preview_mode() {
		return Plugin::$instance->preview->is_preview_mode();
	}
}

if ( ! function_exists( 'omniverse_elementor_is_preview_page' ) ) {
	/**
	 * Whether the preview page.
	 *
	 * @since 1.0.0
	 */
	function omniverse_elementor_is_preview_page() {
		return isset( $_GET['preview_id'] ); // phpcs:ignore
	}
}

if ( ! function_exists( 'omniverse_get_image_url' ) ) {
	/**
	 * Get image url
	 *
	 * @since 1.0.0
	 ...
	 * @return string
	 */
	function omniverse_get_image_url( $id, $image_size_key, $settings ) {
		if ( ! omniverse_is_elementor_installed() ) {
...
if ( ! function_exists( 'omniverse_get_all_image_sizes_names' ) ) {
	/**
	 ...
	 * @return array
	 */
	function omniverse_get_all_image_sizes_names( $style = 'default' ) {
		$available_sizes = omniverse_get_all_image_sizes();
		$image_sizes     = array();
		...
		if ( 'elementor' === $style ) {
			$image_sizes['custom'] = esc_html__( 'Custom', 'omniverse' );
		} elseif ( 'header_builder' === $style ) {
			...
			$image_sizes[ esc_html__( 'Custom', 'omniverse' ) ] = 'custom';
		}
