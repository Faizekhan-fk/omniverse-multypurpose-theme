...
if ( ! function_exists( 'omniverse_add_container_full_width_control' ) ) {
	/**
	 ...
	 */
	function omniverse_add_container_full_width_control( $element ) {
		$element->start_controls_section(
			'wd_extra_layout',
			array(
				'label' => esc_html__( '[Zynxsol] Layout', 'omniverse' ),
		...
		$element->add_control(
			'wd_section_stretch',
			array(
				'label'        => esc_html__( 'Container stretch', 'omniverse' ),
				'description'  => esc_html__( 'Enable this option to stretch container with CSS.', 'omniverse' ),
				'type'         => Controls_Manager::SELECT,
				'default'      => '',
				'options'      => array(
					''                => esc_html__( 'Disabled', 'omniverse' ),
					'stretch'         => esc_html__( 'Stretch container', 'omniverse' ),
					'stretch-content' => esc_html__( 'Stretch container and content', 'omniverse' ),
				),
	...
	add_action( 'elementor/element/container/section_layout_container/after_section_end', 'omniverse_add_container_full_width_control' );
}

if ( ! function_exists( 'omniverse_add_container_color_scheme_control' ) ) {
	/**
	 ...
	 * @param object $element The control.
	 */
	function omniverse_add_container_color_scheme_control( $element ) {
		$element->start_controls_section(
			'wd_extra_style',
			array(
	...
	add_action( 'elementor/element/container/section_background_overlay/after_section_end', 'omniverse_add_container_color_scheme_control' );
}

if ( ! function_exists( 'omniverse_add_container_custom_controls' ) ) {
	/**
	 ...
	 */
	function omniverse_add_container_custom_controls( $element ) {
		$element->start_controls_section(
			'wd_extra_advanced',
			array(
				'label' => esc_html__( '[Zynxsol] Extra', 'omniverse' ),
		...
		$element->add_responsive_control(
			'container_sticky',
			array(
				'label'        => esc_html__( 'Sticky container', 'omniverse' ),
				'type'         => Controls_Manager::SWITCHER,
		...
		$element->add_control(
			...
			array(
				'label'        => esc_html__( 'Sticky container offset (px)', 'omniverse' ),
		...
		$element->add_control(
			...
			array(
				...
				'description'  => esc_html__('Limit the column height and add the "Read more" button. IMPORTANT: you need to add our "Button" element to the end of this container and enable an appropriate option there as well.', 'omniverse'),
				'type'         => Controls_Manager::SWITCHER,
				'default'      => '',
				'label_on'     => esc_html__('Yes', 'omniverse'),
				'label_off'    => esc_html__('No', 'omniverse'),
	...
	add_action( 'elementor/element/container/section_layout/after_section_end', 'omniverse_add_container_custom_controls' );
}

if ( ! function_exists( 'omniverse_container_before_render' ) ) {
	/**
	 ...
	 */
	function omniverse_container_before_render( $widget ) {
		$settings = $widget->get_settings_for_display();

		if ( 'enabled' === omniverse_get_opt( 'negative_gap', 'enabled' ) && isset( $settings['content_width'] ) && 'boxed' === $settings['content_width'] && isset( $settings['boxed_width']['size'] ) && ! $settings['boxed_width']['size'] ) {
		...
		if ( ! empty( $settings['container_sticky'] ) || ! empty( $settings['container_sticky_tablet'] ) || ! empty( $settings['container_sticky_mobile'] ) ) {
			omniverse_enqueue_js_script( 'sticky-container' );
		}
	}

	add_action( 'elementor/frontend/container/before_render', 'omniverse_container_before_render', 10 );
}

if ( ! function_exists( 'omniverse_add_container_class_if_content_width' ) ) {
	/**
	 ...
	 * @param object $widget Element.
	 */
	function omniverse_add_container_class_if_content_width( $widget ) {
		$settings = $widget->get_settings_for_display();
