...
namespace DN\Elementor\Controls;
...
class Buttons extends Base_Data_Control {
	...
	public function enqueue() {
		wp_enqueue_script( 'dn-buttons-control', OMNIVERSE_THEME_DIR . '/inc/integrations/elementor/assets/js/buttons.js', [ 'jquery' ], omniverse_get_theme_info( 'Version' ), false );
	}
	...
	public function content_template() {
		...
		var type  = has_image ? ' dn-images-set' : '';
		var style = has_style ? ' dn-style-' + options[first_key].style : '';
		#>
		<div class="elementor-control-field">
			<label for="<?php echo esc_attr( $control_uid ); ?>" class="elementor-control-title">{{{ data.label
				}}}</label>
			<div class="elementor-control-input-wrapper">
				<input type="hidden" id="<?php echo esc_attr( $control_uid ); ?>" class="dn-buttons-input" data-setting="{{ data.name }}">
				<div class="dn-btns-set{{ type }}{{ style }}">
					<# _.each( options, function( option_settings, option_value ) { #>
					<#
					var type = has_image ? ' dn-set-btn-image' : ' dn-set-btn';
					var value = data.controlValue;
					if ( typeof value == 'string' ) {
					var selected = ( option_value === value ) ? ' dn-active' : '';
					} else if ( null !== value ) {
					var value = _.values( value );
					var selected = ( -1 !== value.indexOf( option_value ) ) ? ' dn-active' : '';
					}
					#>
					<div class="dn-set-item{{ selected }}{{ type }}" data-value="{{ option_value }}">
						<# if ( option_settings['image'] ) { #>
						<img src="{{{ option_settings['image'] }}}">
						<span class="dn-set-tooltip">{{{ option_settings['title'] }}}</span>
						<# } else { #>
