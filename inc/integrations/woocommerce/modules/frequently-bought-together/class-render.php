...
class Render extends Singleton {
	...
	public function init() {
		...
		add_action( 'wp_ajax_omniverse_purchasable_fbt_products', array( $this, 'purchasable_fbt_products' ) );
		add_action( 'wp_ajax_nopriv_omniverse_purchasable_fbt_products', array( $this, 'purchasable_fbt_products' ) );
		...
		add_filter( 'omniverse_show_widget_cart_item_quantity', array( $this, 'widget_cart_item_quantity' ), 10, 2 );
	...
	public function enqueue_style() {
		...
		foreach ( WC()->cart->cart_contents as $product_cart ) {
			if ( ! empty( $product_cart['wd_fbt_bundle_id'] ) ) {
				omniverse_enqueue_inline_style( 'woo-opt-fbt-cart' );
	...
	public function purchasable_fbt_products() {
		...
		$main_product_discount     = get_post_meta( $bundle_id, '_omniverse_main_products_discount', true );
		$item_keys                 = array();
	...
	public function before_calculate_totals( $cart_object ) {
		...
		foreach ( $cart_object->cart_contents as $cart_item ) {
			if ( isset( $cart_item['data'], $cart_item['wd_fbt_discount'] ) && $cart_item['wd_fbt_discount'] && $cart_item['data'] instanceof WC_Product ) {
				...
				$price = apply_filters( 'omniverse_fbt_set_product_cart_price', $price, $cart_item );
	...
	private function update_data_bundle_product( $cart_item ) {
		...
		$fbt_products       = get_post_meta( $cart_item['wd_fbt_bundle_id'], '_omniverse_fbt_products', true );
		$main_discount      = get_post_meta( $cart_item['wd_fbt_bundle_id'], '_omniverse_main_products_discount', true );
		$show_checkbox      = get_post_meta( $cart_item['wd_fbt_bundle_id'], '_omniverse_show_checkbox', true );
		...
		$bundles_id = get_post_meta( $cart_item['wd_fbt_parent_id'], 'omniverse_fbt_bundles_id', true );
	...
	public function cart_item_price( $price, $cart_item ) {
		if ( isset( $cart_item['wd_fbt_parent_id'], $cart_item['wd_fbt_discount'] ) && $cart_item['wd_fbt_discount'] ) {
			...
			$new_price = apply_filters( 'omniverse_fbt_product_cart_price', $new_price, $cart_item );
	...
	public function cart_item_subtotal( $price, $cart_item ) {
		if ( isset( $cart_item['wd_fbt_parent_id'], $cart_item['wd_fbt_discount'] ) && $cart_item['wd_fbt_discount'] ) {
			...
			$new_price = apply_filters( 'omniverse_fbt_product_cart_subtotal', $new_price, $cart_item );
	...
	public function update_cart_item_quantity( $cart_item_key, $quantity, $old_quantity, $cart ) {
		if ( isset( $_REQUEST['action'] ) && 'omniverse_purchasable_fbt_products' === $_REQUEST['action'] ) { //phpcs:ignore
			return;
		}
	...
	public function cart_item_name( $item_name, $item ) {
		if ( ! empty( $item['wd_fbt_parent_id'] ) ) {
			...
			<span class="wd-fbt-label wd-tooltip">
				<?php esc_html_e( 'Bundled product', 'omniverse' ); ?>
			</span>
			<?php
