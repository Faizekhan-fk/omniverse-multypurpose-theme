...
class Frontend extends Singleton {
	...
	public function init() {
		add_action( 'omniverse_after_product_tabs', array( $this, 'get_bought_together_products' ) );

		add_action( 'wp_ajax_omniverse_update_frequently_bought_price', array( $this, 'update_frequently_bought_price' ) );
		add_action( 'wp_ajax_nopriv_omniverse_update_frequently_bought_price', array( $this, 'update_frequently_bought_price' ) );
	}
	...
	public function update_frequently_bought_price() {
		...
		$products_id  = omniverse_clean( $_POST['products_id'] ); //phpcs:ignore
		$fbt_products = get_post_meta( $bundle_id, '_omniverse_fbt_products', true );
		$fragments    = array();
		...
		$fragments[ 'div.wd-fbt-bundle-' . $this->bundle_id . ' .wd-fbt-purchase .wd-fbt-desc' ] = '<div class="wd-fbt-desc">' . sprintf( _n( 'For %s item', 'For %s items', $fbt_count, 'omniverse' ), $fbt_count ) . '</div>';
	...
	public function get_bought_together_products( $element_settings = array() ) {
		...
		$settings = array(
			'title'                   => '',
			'slides_per_view'         => omniverse_get_opt( 'bought_together_column', 3 ),
			'slides_per_view_tablet'  => omniverse_get_opt( 'bought_together_column_tablet', 'auto' ),
			'slides_per_view_mobile'  => omniverse_get_opt( 'bought_together_column_mobile', 'auto' ),
			'hide_pagination_control' => '',
			'hide_prev_next_buttons'  => '',
			'form_width'              => omniverse_get_opt( 'bought_together_form_width' ),
			'is_builder'              => false,
		);
		...
		$bundles_id = get_post_meta( $main_product, 'omniverse_fbt_bundles_id', true );
		...
		foreach ( $bundles_id as $bundle_id ) {
			...
			$wfbt_products = get_post_meta( $bundle->ID, '_omniverse_fbt_products', true );
		...
		omniverse_enqueue_inline_style( 'woo-opt-fbt' );
		omniverse_enqueue_js_script( 'frequently-bought-together' );

		add_filter( 'woocommerce_get_price_html', array( $this, 'update_product_price' ), 10, 2 );
		add_filter( 'omniverse_product_label_output', array( $this, 'added_sale_label' ) );
		...
		omniverse_set_loop_prop( 'show_quick_shop', false );
		...
		foreach ( $bundles_data as $bundle_id => $wfbt_products ) {
			...
			foreach ( $wfbt_products as $wfbt_product ) {
				...
				if ( get_post_meta( $bundle_id, '_omniverse_show_checkbox', true ) && get_post_meta( $bundle_id, '_omniverse_hide_out_of_stock_product', true ) && ! $current_product->is_in_stock() ) {
					continue;
				}
		...
		remove_filter( 'omniverse_product_label_output', array( $this, 'added_sale_label' ) );
	...
	protected function get_heading( $title = '', $is_builder = false ) {
		...
		<h4 class="wd-el-title title<?php echo esc_attr( $class ); ?>">
			...
			<?php else : ?>
				<?php esc_html_e( 'Frequently bought together', 'omniverse' ); ?>
			<?php endif; ?>
		</h4>
		<?php
	}
	...
	public function get_form_content( $settings ) {
		...
		?>
			<div class="wd-fbt wd-design-side wd-fbt-bundle-<?php echo esc_attr( $this->bundle_id ); ?>">
				<?php
				if ( 'elementor' === omniverse_get_current_page_builder() ) {
					echo omniverse_elementor_products_template( $atts ); //phpcs:ignore
				} else {
					$atts['include'] = implode( ',', $atts['include'] );

					echo omniverse_shortcode_products( $atts ); //phpcs:ignore
				}
	...
	protected function get_products_purchase() {
		...
		$show_checkbox  = get_post_meta( $this->bundle_id, '_omniverse_show_checkbox', true );
		$state_checkbox = get_post_meta( $this->bundle_id, '_omniverse_default_checkbox_state', true );
		$classes        = '';
		$button_classes = '';
	...
	public function added_sale_label( $content ) {
		...
		$label = '<span class="onsale product-label wd-fbt-sale-label">' . sprintf( _x( '-%d%%', 'sale percentage', 'omniverse' ), $discount ) . '</span>';
	...
	private function get_discount_product_bundle( $product_id ) {
		if ( $this->main_product_id === $product_id ) {
			$discount = (float) get_post_meta( $this->bundle_id, '_omniverse_main_products_discount', true );
