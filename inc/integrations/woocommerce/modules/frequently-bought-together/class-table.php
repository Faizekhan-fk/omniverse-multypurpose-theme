...
class Bundles_Table extends \WP_List_Table {
	...
	public function __construct( $product_id = '' ) {
		parent::__construct(
			array(
				'singular' => __( 'types', 'omniverse' ),
				'plural'   => __( 'type', 'omniverse' ),
				'ajax'     => true,
			)
		);
	...
	protected function column_name( $item ) {
		...
		<div class="row-actions">
			...
			<span class="delete">
				<a href="#" class="dn-delete-bundle" data-id="<?php echo esc_attr( $item['id'] ); ?>" data-link="ajax-delete">
					<?php esc_html_e( 'Delete', 'omniverse' ); ?>
				</a>
			</span>
		</div>
		<?php
	...
	public function get_columns() {
		$data['name']     = esc_html__( 'Bundles name', 'omniverse' );
		$data['products'] = esc_html__( 'Products', 'omniverse' );
	...
	public function prepare_items( $product_id = '' ) {
		...
		$bundles_id = get_post_meta( $product_id, 'omniverse_fbt_bundles_id', true );
	...
	public function no_items() {
		esc_html_e( 'No bundles found.', 'omniverse' );
	}
}
