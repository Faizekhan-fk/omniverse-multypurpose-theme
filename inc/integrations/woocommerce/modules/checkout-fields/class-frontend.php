...
class Frontend extends Singleton {
	...
	public function enqueue_scripts() {
		if ( ! omniverse_get_opt( 'checkout_fields_enabled' ) || ! is_checkout() ) {
			return;
		}

		wp_enqueue_script( 'checkout-fields', OMNIVERSE_THEME_DIR . '/js/scripts/wc/checkoutFields.js', array( 'jquery' ), OMNIVERSE_VERSION, true );
		wp_localize_script( 'checkout-fields', 'omniverse_checkout_fields', $this->add_localized_settings() );
	}
	...
	public function change_locale_fields( $fields ) {
		if ( ! omniverse_get_opt( 'checkout_fields_enabled' ) ) {
			return $fields;
		}
