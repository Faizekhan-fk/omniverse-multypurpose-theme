...
namespace DN\Modules\Checkout_Fields\List_Table;

use WP_List_Table;
use DN\Modules\Checkout_Fields\Helper;
use DN\Modules\Checkout_Fields\Admin;
...
class Fields_Table extends WP_List_Table {
	...
	public function column_sort( $item ) {
		...
		<div class="dn-ui-sortable-handle"></div>
	...
	public function column_position( $item ) {
		...
		$this->helper->get_template(
			...
			array(
				...
				'options' => array(
					'form-row-first' => esc_html__( 'Left', 'omniverse' ),
					'form-row-wide'  => esc_html__( 'Wide', 'omniverse' ),
					'form-row-last'  => esc_html__( 'Right', 'omniverse' ),
				),
				'current' => $current,
			)
		);
	...
	public function column_required( $item ) {
		...
		$this->helper->get_template(
			...
			array(
				...
				'text_on'  => esc_html__( 'Yes', 'omniverse' ),
				'text_off' => esc_html__( 'No', 'omniverse' ),
			)
		);
	...
	public function get_columns() {
		return array(
			'sort'       => esc_html__( 'Sort', 'omniverse' ),
			'field_name' => esc_html__( 'Name', 'omniverse' ),
			'label'      => esc_html__( 'Label', 'omniverse' ),
			'position'   => esc_html__( 'Position', 'omniverse' ),
			'required'   => esc_html__( 'Required', 'omniverse' ),
			'status'     => esc_html__( 'Status', 'omniverse' ),
		);
	}
	...
	public function table_data() {
		...
		$change_options          = get_option( 'zs_checkout_fields_manager_options', array() );
	...
	protected function extra_tablenav( $which ) {
		...
		?>
		<a href="<?php echo esc_attr( add_query_arg( 'reset-all-fields', true, $this->admin->get_base_url() ) ); ?>" class="dn-reset-all-fields dn-btn dn-color-primary">
			<?php esc_html_e( 'Reset all', 'omniverse' ); ?>
		</a>
		<?php
	}
