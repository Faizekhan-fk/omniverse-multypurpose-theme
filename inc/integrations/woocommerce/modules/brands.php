...
if( ! function_exists( 'omniverse_product_brand' ) ) {
	function omniverse_product_brand() {
		global $product;

		$attr = omniverse_get_opt( 'brands_attribute' );

		if ( ! $attr || ( ! omniverse_get_opt( 'product_page_brand' ) && ( omniverse_loop_prop( 'is_quick_view' ) || ! Main::get_instance()->has_custom_layout( 'single_product' ) ) ) ) {
			return;
		}
		...
		$classes = ( omniverse_get_opt( 'product_brand_location' ) == 'sidebar' && ! omniverse_loop_prop( 'is_quick_view' ) ) ? ' widget sidebar-widget' : '';

		$classes .= omniverse_get_old_classes( ' omniverse-product-brands' );
		...
		foreach ($brands as $brand) {
			...
			$link_wrapper_classes .= omniverse_get_old_classes( ' omniverse-product-brand' );
...
if( ! function_exists( 'omniverse_product_brands_links' ) ) {
	function omniverse_product_brands_links() {
		global $product;
		$brand_option = omniverse_get_opt( 'brands_attribute' );
		$brands = wc_get_product_terms( $product->get_id(), $brand_option, array( 'fields' => 'all' ) );
		$taxonomy = get_taxonomy( $brand_option );

		if( !omniverse_get_opt( 'brands_under_title' ) || empty( $brands ) ) return;

		$link = ( omniverse_is_shop_on_front() ) ? home_url() : get_post_type_archive_link( 'product' );

		echo '<div class="wd-product-brands-links' . omniverse_get_old_classes( ' omniverse-product-brands-links' ) . '">';
...
if( ! function_exists( 'omniverse_product_brand_tab' ) ) {
	function omniverse_product_brand_tab( $tabs ) {
		global $product;
		
		$show_tab = false;
		$brand_title = esc_html__( 'About brand', 'omniverse' );
		$brand_info = wc_get_product_terms( $product->get_id(), omniverse_get_opt( 'brands_attribute' ), array( 'fields' => 'all' ) );
		if ( !isset( $brand_info[0] ) ) return $tabs;
		
		if ( $brand_info[0]->description ) $show_tab = true;
		if ( omniverse_get_opt( 'brand_tab_name' ) ) $brand_title = sprintf( esc_html__( 'About %s', 'omniverse' ), $brand_info[0]->name );
		
		if ( $show_tab ) {
			$tabs['brand_tab'] = array(
				'title' 	=> $brand_title,
				'priority' 	=> 50,
				'callback' 	=> 'omniverse_product_brand_tab_content'
			);
		}
...
if( ! function_exists( 'omniverse_product_brand_tab_content' ) ) {
	function omniverse_product_brand_tab_content() {
		global $product;
		$attr = omniverse_get_opt( 'brands_attribute' );
		if ( ! $attr ) return;
