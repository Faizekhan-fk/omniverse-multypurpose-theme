...
namespace DN\Modules\Product_Gallery_Video;
...
class Main extends Singleton {
	...
	public function hooks() {
		if ( ! omniverse_get_opt( 'single_product_main_gallery_video', true ) ) {
			return;
		}
		...
		add_filter( 'omniverse_single_product_thumbnail_classes', array( $this, 'product_thumbnails_classes' ), 10, 2 );

		add_filter( 'omniverse_get_single_product_image_data', array( $this, 'get_single_image_data' ), 10, 2 );

		add_filter( 'omniverse_admin_localized_string_array', array( $this, 'admin_localized_settings' ) );
		add_filter( 'omniverse_localized_string_array', array( $this, 'localized_settings' ) );
	}
	...
	public function add_options() {
		Options::add_field(
			array(
				...
				'name'        => esc_html__( 'Main carousel with video', 'omniverse' ),
				'hint'        => '<video data-src="' . OMNIVERSE_TOOLTIP_URL . 'video-display-type-simplified.mp4" autoplay loop muted></video>',
				'description' => esc_html__( 'Enable the ability to add videos to the main product gallery carousel.', 'omniverse' ),
				'type'        => 'switcher',
	...
	public function get_gallery_product_video_btn( $post_id, $attachment_id ) {
		$product_settings = get_post_meta( $post_id, 'omniverse_wc_video_gallery', true );
		...
		if ( $this->check_is_available_video( $settings ) ) {
			$classes = ' dn-edit-video';
		} else {
			$classes = ' dn-add-video';
		}

		?>
		<div class="dn-product-video-wrapp">
			<a href="#" class="dn-btn dn-color-primary dn-product-gallery-video dn-i-add<?php echo esc_attr( $classes ); ?>">
				<?php esc_html_e( 'Video', 'omniverse' ); ?>
			</a>
			<input type="hidden" name="dn-product-gallery-video[<?php echo esc_attr( $attachment_id ); ?>]" value='<?php echo wp_json_encode( $settings ); ?>'>
		</div>
		<?php
	}
	...
	public function get_main_image_video_btn( $content, $post_id, $attachment_id ) {
		if ( ! omniverse_woocommerce_installed() || 'product' !== get_post_type( $post_id ) ) {
			return $content;
		}
	...
	public function get_product_thumbnail_popup( $post ) {
		...
		wp_enqueue_script( 'wd-product-gallery-videos', OMNIVERSE_ASSETS . '/js/productGalleryVideo.js', array(), omniverse_get_theme_info( 'Version' ), true );
		wp_enqueue_script( 'wd-tooltips', OMNIVERSE_ASSETS . '/js/tooltip.js', array(), omniverse_get_theme_info( 'Version' ), true );

		include OMNIVERSE_THEMEROOT . '/inc/integrations/woocommerce/modules/product-gallery-video/template/popup.php';
	}
	...
	public function save_product_gallery( $product_id, $post ) {
		if ( 'product' !== $post->post_type || empty( $_POST['dn-product-gallery-video'] ) ) { //phpcs:ignore
			return;
		}

		$attachments_settings = (array) omniverse_clean( $_POST['dn-product-gallery-video'] ); //phpcs:ignore
		...
		update_post_meta( $product_id, 'omniverse_wc_video_gallery', $product_settings );
	}
	...
	public function product_image_thumbnail( $content, $attachment_id ) {
		...
		if ( empty( $product ) || ! omniverse_get_opt( 'single_product_main_gallery_video', true ) ) {
			return $content;
		}

		$attachment_id             = apply_filters( 'omniverse_single_product_image_thumbnail_id', $attachment_id, $product );
		$product_settings          = (array) get_post_meta( $product->get_id(), 'omniverse_wc_video_gallery', true );
		...
		$attributes        = array(
			...
			'class'                   => apply_filters( 'omniverse_single_product_gallery_image_class', '' ),
		);
		...
		omniverse_enqueue_js_script( 'single-product-video-gallery' );

		?>
		<div class="wd-carousel-item wd-with-video<?php echo esc_attr( $classes ); ?>">
			<?php
			omniverse_enqueue_inline_style( 'woo-single-prod-opt-gallery-video' );
			if ( omniverse_get_opt( 'photoswipe_icon' ) || 'popup' === omniverse_get_opt( 'image_action' ) ) {
				omniverse_enqueue_inline_style( 'woo-single-prod-opt-gallery-video-pswp' );
			}
	...
	public function get_video_content( $settings ) {
		...
		if ( 'mp4' === $settings['video_type'] ) {
			...
			if ( apply_filters( 'omniverse_output_origin_url_for_youtube', false ) ) {
			...
			if ( 'theme' === $settings['video_control'] && $settings['autoplay'] ) {
				omniverse_enqueue_js_library( 'vimeo_player' );
			}
	...
	public function product_thumbnails_classes( $classes, $attachment_id ) {
		...
		if ( omniverse_get_opt( 'single_product_main_gallery_video', true ) && ! empty( $this->thumbnails_settings[ $attachment_id ] ) && $this->check_is_available_video( $this->thumbnails_settings[ $attachment_id ] ) ) {
	...
	public function get_single_image_data( $image_settings, $attachment_id ) {
		...
		if ( empty( $product ) || empty( $post ) || ! omniverse_get_opt( 'single_product_main_gallery_video', true ) ) {
	...
	public function admin_localized_settings( $settings ) {
		$settings['product_gallery_video_text'] = esc_html__( 'Video', 'omniverse' );
	...
	public function localized_settings( $settings ) {
		$settings['vimeo_library_url'] = OMNIVERSE_THEME_DIR . '/js/libs/vimeo-player.min.js';
