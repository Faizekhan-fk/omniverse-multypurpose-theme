...
class Query extends Singleton {
	...
	public function init() {
		...
		add_action( 'omniverse_quick_view_posts_args', array( $this, 'add_variations_to_quickview' ), 10, 1 );
	...
	public function posts_clauses( $clauses, $query ) {
		...
		if ( ! empty( $query->query_vars['omniverse_single_variations_filter'] ) ) {
			if ( omniverse_get_opt( 'hide_variation_parent' ) && ( ! omniverse_get_opt( 'wishlist_page' ) || get_queried_object_id() !== (int) apply_filters( 'wpml_object_id', omniverse_get_opt( 'wishlist_page' ), 'page', true ) && empty( $_REQUEST['atts']['is_wishlist'] ) ) ) { //phpcs:ignore
			...
			if ( strripos( $clauses['where'], 'wp_wc_product_attributes_lookup' ) ) {
				if ( omniverse_get_opt( 'hide_variation_parent' ) ) {
	...
	public function add_variations_to_query( $query ) {
		...
		$query->set( 'omniverse_single_variations_filter', 'yes' );
	}
	...
	public function add_variations_to_product_query( $query ) {
		if ( ( is_admin() && ( ! isset( $_REQUEST['action'] ) ) ) || isset( $query->query['product'] ) || ! empty( $query->query['wd_show_variable_products'] ) || ! empty( $query->query['preview'] ) ) { //phpcs:ignore
			return;
		}
