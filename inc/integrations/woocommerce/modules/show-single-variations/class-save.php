...
class Save extends Singleton {
	...
	public function save_product( $post_id, $post, $update ) {
		...
		$taxonomies = apply_filters( 'omniverse_single_variations_taxonomies', array( 'product_cat', 'product_tag' ), $post_id );
		$post_metas = apply_filters( 'omniverse_single_variations_post_meta', array( '_omniverse_new_label_date', '_omniverse_new_label' ), $post_id );
		...
		foreach ( $variation_ids as $variation_id ) {
			...
			if ( isset( $this->request_data()['_omniverse_exclude_show_single_variation'] ) && ( $this->request_data()['_omniverse_exclude_show_single_variation'] || get_post_meta( $post_id, '_omniverse_exclude_show_single_variation', true ) ) ) {
				update_post_meta( $variation_id, '_wd_show_variation', 'on' === $this->request_data()['_omniverse_exclude_show_single_variation'] ? 'no' : '' );
			}
	...
	public function save_variation( $variation_id, $i = false ) {
		...
		if ( 'new' === omniverse_get_opt( 'variation_gallery_storage_method' ) && isset( $request_data['wd_additional_variation_images'][ $variation_id ] ) ) {
			update_post_meta( $variation_id, '_product_image_gallery', $request_data['wd_additional_variation_images'][ $variation_id ] );
		} elseif ( isset( $request_data['omniverse_variation_gallery'][ $variation_id ] ) ) {
			update_post_meta( $variation_id, '_product_image_gallery', $request_data['omniverse_variation_gallery'][ $variation_id ] );
		}
