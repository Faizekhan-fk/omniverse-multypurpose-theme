...
class Reviews extends Singleton {
	...
	public function init() {
		add_action( 'wp_ajax_omniverse_filter_review', array( $this, 'ajax_filter_reviews' ) );
		add_action( 'wp_ajax_nopriv_omniverse_filter_review', array( $this, 'ajax_filter_reviews' ) );
		...
		if ( 'style-2' === omniverse_get_opt( 'reviews_style' ) ) {
	...
	public function do_after_setup_globals_options() {
		if ( 'rating' === omniverse_get_opt( 'reviews_rating_summary_content' ) && omniverse_get_opt( 'reviews_rating_summary_filter' ) ) {
			add_action( 'omniverse_after_rating_summary_content', array( $this, 'get_ajax_loader' ) );
		}
	}
	...
	public function add_comment_class( $classes, $css_class, $comment_id, $comment, $post_id ) {
		if ( ! omniverse_woocommerce_installed() || ( ! is_singular( 'product' ) && ! Builder::get_instance()->has_custom_layout( 'single_product' ) ) && ! omniverse_is_woo_ajax() || empty( $comment ) || ! function_exists( 'wc_customer_bought_product' ) ) {
			return $classes;
		}
	...
	public function filter_comments() {
		...
		$args = apply_filters(
			'omniverse_product_reviews_args',
			array(
				'post_id'            => $product_id,
				'post_type'          => 'product',
				'status'             => 'approve',
		...
		$stars = apply_filters( 'omniverse_product_reviews_ratings', $ratings );
	...
	public function render_comments( $return = false ) {
		...
		$comment_list_classes = 'wd-grid-g';
		$comment_list_styles  = '';
		$reviews_attr         = array();

		$comment_list_classes .= ' wd-review-' . omniverse_get_opt( 'reviews_style', 'style-1' );
		...
		foreach ( array( 'desktop', 'tablet', 'mobile' ) as $device ) {
			...
			$prefix          = '';
			$reviews_columns = Global_Data::get_instance()->get_data( $key ) ? Global_Data::get_instance()->get_data( $key ) : omniverse_get_opt( $key, 1 );
		...
		?>
		<ol class="commentlist <?php echo apply_filters( 'omniverse_single_product_comment_list_classes', $comment_list_classes ); ?>" style="<?php echo esc_attr( $comment_list_styles ); ?>" data-reviews-columns="<?php echo esc_attr( wp_json_encode( $reviews_attr ) ); ?>">
			<?php
			if ( 0 !== count( $comments ) ) {
				wp_list_comments(
					apply_filters(
				...
				?>
				<li class="woocommerce-noreviews wd-col"><?php esc_html_e( 'There are no reviews matching the given conditions.', 'omniverse' ); ?></li>
				<?php
			}
			?>
		</ol>
	...
	public function render_pagination( $return = false ) {
		...
		if ( omniverse_get_opt( 'reviews_rating_summary' ) ) {
			$args['add_args'] = array(
				...
				'order_by'   => $order_by,
				'action'     => 'omniverse_filter_comments',
			);
		}
		...
		<nav class="woocommerce-pagination">
			<?php
			echo paginate_links( apply_filters( 'omniverse_comment_pagination_args', $args ) ); // phpcs:ignore;
			?>
		</nav>
