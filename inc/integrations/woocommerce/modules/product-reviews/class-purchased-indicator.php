...
class Purchased_Indicator {
	...
	public function add_author_icon( $author, $comment_ID ) {
		if ( ! omniverse_get_opt( 'show_reviews_purchased_indicator' ) || ( $this->is_parent_comment( $comment_ID ) && ! $this->is_user_purchased_product( $comment_ID ) ) || ( ! wp_doing_ajax() && is_admin() ) ) {
			return $author;
		}

		$tooltip_text = $this->is_parent_comment( $comment_ID ) ? esc_html__( 'Verified owner', 'omniverse' ) : esc_html__( 'Store manager', 'omniverse' );
	...
	protected function is_user_purchased_product( $comment_id ) {
		...
		if ( ! omniverse_woocommerce_installed() || ! is_singular( 'product' ) && ! omniverse_is_woo_ajax() || empty( $comment ) || ! function_exists( 'wc_customer_bought_product' ) || ! function_exists( 'wc_review_is_from_verified_owner' ) ) {
			return false;
		}
