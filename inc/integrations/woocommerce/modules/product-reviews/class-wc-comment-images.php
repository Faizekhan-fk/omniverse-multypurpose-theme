...
class WC_Comment_Images {
	 ...
	 */
	private $image_meta_key = '_omniverse_image_id';
	 ...
	 */
	private $upload_field_name = 'omniverse_image';
	...
	public function filter_upload_mimes() {
		...
		return apply_filters( 'omniverse_comment_images_upload_mimes', $mimes );
	}
	...
	public function get_max_upload_size( $with_format = false ) {
		$max_upload_size = (int) omniverse_get_opt( 'single_product_comment_images_upload_size', '1' ) * MB_IN_BYTES;
	...
	public function add_metaboxes() {
		if ( ! omniverse_get_opt( 'single_product_comment_images', '1' ) ) {
			return;
		}
		...
		$metaboxes = Metaboxes::add_metabox(
			array(
				'id'     => 'omniverse_comment_images_metabox',
				'title'  => esc_html__( 'Review images metabox', 'omniverse' ),
		...
		$metaboxes->add_field(
			array(
				'id'       => '_omniverse_image_id',
				'type'     => 'upload_list',
				'section'  => 'general',
				'tags'     => 'some tags for search',
				'name'     => esc_html__( 'Upload review images ', 'omniverse' ),
				'default'  => $ids,
				'priority' => 10,
			)
		);
	...
	public function display_images( $comment_content ) {
		if ( ! $this->has_images() || ! omniverse_get_opt( 'single_product_comment_images', '1' ) || ( ! wp_doing_ajax() && ( is_admin() || ! is_singular( 'product' ) ) ) ) {
			return $comment_content;
		}

		ob_start();

		echo omniverse_images_gallery_shortcode(
			array(
	...
	public function save_image( $comment_id, $comment_approved, $comment ) {
		if ( ! is_user_logged_in() || ! omniverse_get_opt( 'single_product_comment_images', '1' ) ) {
			return;
		}
	...
	public function check_images( $comment_data ) {
		...
		$field_name   = $this->get_upload_field_name();
		$images_count = omniverse_get_opt( 'single_product_comment_images_count', '3' );
		...
		if ( is_array( $names ) && count( $names ) > $images_count ) {
			wp_die( sprintf( esc_html__( 'You can upload up to %s images to your review.', 'omniverse' ), $images_count ) ); // phpcs:ignore
		}

		foreach ( $attachments['size'] as $size ) {
			if ( $size > $this->get_max_upload_size() ) {
				wp_die( sprintf( esc_html__( 'The maximum upload file size: %s.', 'omniverse' ), $this->get_max_upload_size( true ) ) ); // phpcs:ignore
			}
		}

		if ( omniverse_get_opt( 'single_product_comment_images_required' ) ) {
			$this->enable_filter_upload();
			foreach ( $names as $name ) {
				$filetype = wp_check_filetype( $name );

				if ( ! $filetype['ext'] ) {
					wp_die( sprintf( esc_html__( 'You are allowed to upload images only in %s formats.', 'omniverse' ), apply_filters( 'omniverse_comment_images_upload_mimes', 'png, jpeg' ) ) ); // phpcs:ignore
				}
			}
	...
	public function add_image_field( $submit_field ) {
		if ( ! omniverse_get_opt( 'single_product_comment_images', '1' ) || ! is_singular( 'product' ) ) {
			return $submit_field;
		}

		if ( ! is_user_logged_in() ) {
			return '<p class="comment-form-img-message">' . esc_html__( 'You have to be logged in to be able to add photos to your review.', 'omniverse' ) . '</p>' . $submit_field;
		}

		$name            = $this->get_upload_field_name();
		$max_upload_size = $this->get_max_upload_size( true );
		$required        = omniverse_get_opt( 'single_product_comment_images_required' );

		omniverse_enqueue_js_library( 'tooltips' );
		omniverse_enqueue_js_script( 'btns-tooltips' );
		omniverse_enqueue_js_script( 'comment-image' );
		omniverse_enqueue_inline_style( 'woo-single-prod-opt-review-images' );
		...
		<div class="comment-form-images">
			<div class="wd-add-img-btn-wrapper">
				<label for="wd-add-img-btn">
					<?php esc_html_e( 'Click to add images', 'omniverse' ); ?>
					<?php if ( $required ) : ?>
				...
				<div class="wd-add-img-msg wd-hint wd-tooltip">
					<div class="wd-add-img-msg-text">
						<?php printf( esc_html__( 'The maximum file size is %s and you can upload up to %s images.', 'omniverse' ), $max_upload_size, omniverse_get_opt( 'single_product_comment_images_count' ) ); // phpcs:ignore ?>
					</div>
				</div>
