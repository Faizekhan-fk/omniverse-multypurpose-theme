...
class Likes {
	...
	public function hooks() {
		...
		add_action( 'wp_ajax_omniverse_comments_likes', array( $this, 'ajax_comments_likes' ) );
	...
	public function render( $data_object ) {
		if ( ! omniverse_get_opt( 'reviews_enable_likes' ) || '0' !== $data_object->comment_parent || ( ! wp_doing_ajax() && ( is_admin() || ! is_singular( 'product' ) ) ) ) {
			return;
		}
		...
		omniverse_enqueue_js_script( 'product-reviews-likes' );
		omniverse_enqueue_inline_style( 'woo-single-prod-opt-review-likes' );
		?>
