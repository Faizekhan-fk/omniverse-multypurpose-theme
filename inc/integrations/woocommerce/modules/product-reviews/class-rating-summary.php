...
class Rating_Summary extends Singleton {
	...
	public function init() {
		if ( ! omniverse_get_opt( 'reviews_rating_summary' ) ) {
			return;
		}
	...
	public function render_filter_dashboard( $return = false ) {
		...
		$rating_summary_classes = 'rating' === omniverse_get_opt( 'reviews_rating_summary_content', 'rating' ) && omniverse_get_opt( 'reviews_rating_summary_filter' ) ? ' wd-with-filter' : '';
		$rating_format          = 'percentage';

		if ( 'criteria' === omniverse_get_opt( 'reviews_rating_summary_content', 'rating' ) && Rating_Criteria::get_instance()->is_criteria_enabled() ) {
			$rating_summary_classes .= ' wd-with-criteria';
			$ratings                 = Rating_Criteria::get_instance()->get_summary_criteria_list();
		}

		$ratings = apply_filters( 'omniverse_product_ratings', $ratings );
		...
		omniverse_enqueue_inline_style( 'woo-single-prod-opt-rating-summary' );
		?>
		<div class="wd-rating-summary wd-sticky<?php echo esc_attr( apply_filters( 'omniverse_rating_summary_classes', $rating_summary_classes ) );?>">
			<div class="wd-rating-summary-heading">
				...
				<?php endif; ?>
				<div class="star-rating" role="img" aria-label="<?php echo esc_attr( sprintf( __( 'Rated %s out of 5', 'omniverse' ), $average_rating ) ); ?>">
					<?php echo wp_kses( omniverse_get_star_rating_html( $average_rating ), true ); ?>
				</div>
				<div class="wd-rating-summary-total">
					<?php
					printf( _nx( '%d review', '%d reviews', $reviews_count, 'noun', 'omniverse' ), $reviews_count ); // phpcs:ignore.
					?>
				</div>
			</div>
				...
				foreach ( $ratings as $key => $rating ) {
					...
					<div class="wd-rating-summary-item<?php echo esc_attr( $rating_summary_item_classes ); ?>">
						<div class="wd-rating-label" data-rating="<?php echo esc_attr( $rating ); ?>">
							<?php if ( ! Rating_Criteria::get_instance()->is_criteria_enabled() || 'rating' === omniverse_get_opt( 'reviews_rating_summary_content', 'rating' ) ) : ?>
								<div class="star-rating" role="img" aria-label="<?php echo esc_attr( sprintf( __( 'Rated %s out of 5', 'omniverse' ), $rating ) ); ?>">
									<?php echo wp_kses( omniverse_get_star_rating_html( $rating ), true ); ?>
								</div>
							<?php else: ?>
						...
						<div class="wd-rating-count">
							...
							if ( Rating_Criteria::get_instance()->is_criteria_enabled() && 'criteria' === omniverse_get_opt( 'reviews_rating_summary_content', 'rating' ) ) {
								$rating_count = round( $this->get_rating_value( $key, $rating , 'average' ), 1 );
							}

							$rating_count = apply_filters( 'omniverse_rating_count', $rating_count, $key, $rating );

							?>
							<?php echo esc_html( $rating_count ); ?>

							<?php if ( Rating_Criteria::get_instance()->is_criteria_enabled() && 'criteria' === omniverse_get_opt( 'reviews_rating_summary_content', 'rating' ) ) : ?>
								<div class="star-rating" role="img" aria-label="<?php echo esc_attr( sprintf( __( 'Rated %s out of 5', 'omniverse' ), $rating_count ) ); ?>">
									<?php echo wp_kses( omniverse_get_star_rating_html( $rating_count ), true ); ?>
								</div>
							<?php endif; ?>
						</div>
					</div>
			...
			<?php do_action( 'omniverse_after_rating_summary_content' ); ?>
		</div>
		<?php
		if ( $return ) {
			return ob_get_clean();
