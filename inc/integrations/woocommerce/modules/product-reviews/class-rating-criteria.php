...
class Rating_Criteria extends Singleton {
			...
			<div class="wd-star-ratings wd-event-hover">
				...
				<div class="wd-criteria-wrap wd-dropdown">
					<?php foreach ( $summary_criteria_list as $criteria_id => $criteria_title ) : ?>
						...
						?>
						<div class="wd-star-rating-wrap">
							<div class="star-rating" role="img" aria-label="<?php echo esc_attr( sprintf( __( 'Rated %s out of 5', 'omniverse' ), $criteria_rating ) ); ?>">
								<?php echo wp_kses( omniverse_get_star_rating_html( $criteria_rating ), true ); ?>
							</div>
	...
	public function get_criteria_stars_ratings_fields() {
		...
		omniverse_enqueue_js_script( 'product-reviews-criteria' );
		?>
			...
			<?php foreach ( $summary_criteria_list as $criteria_id => $criteria_title ) : ?>
				<div class="wd-review-criteria comment-form-rating" data-criteria-id=<?php echo esc_attr( $criteria_id ); ?>>
					<label for="<?php echo esc_attr( $criteria_id ); ?>">
						...
						<?php if ( omniverse_get_opt( 'reviews_criteria_rating_required' ) ) : ?>
							<span class="required">*</span>
						<?php endif; ?>
					</label>
					<div class="stars">
					...
					<select name="<?php echo esc_attr( $criteria_id ); ?>" id="<?php echo esc_attr( $criteria_id ); ?>" required>
						<option value="">
							<?php echo esc_html__( 'Rate&hellip;', 'omniverse' ); ?>
						</option>
						<option value="5">
							<?php echo esc_html__( 'Perfect', 'omniverse' ); ?>
						</option>
						<option value="4">
							<?php echo esc_html__( 'Good', 'omniverse' ); ?>
						</option>
						<option value="3">
							<?php echo esc_html__( 'Average', 'omniverse' ); ?>
						</option>
						<option value="2">
							<?php echo esc_html__( 'Not that bad', 'omniverse' ); ?>
						</option>
						<option value="1">
							<?php echo esc_html__( 'Very poor', 'omniverse' ); ?>
						</option>
					</select>
				</div>
			<?php endforeach; ?>
	...
	/**
	 * Clear omniverse_criteria_hash transients.
	 *
	 * @param int $comment_ID Current comment id.
	 * @return void
	 */
	public function clear_transient( $comment_ID ) {
		...
		foreach ( $this->get_summary_criteria_list() as $criteria_id => $criteria_title ) {
			delete_transient( 'omniverse_criteria_hash_' . $product_id . '_' . $criteria_id );
		}
	}
	...
	public function get_criteria_rating_list() {
		...
		foreach ( $this->get_summary_criteria_list() as $criteria_id => $criteria_title ) {
			$comments = get_transient( 'omniverse_criteria_hash_' . $product_id . '_' . $criteria_id );
			...
			if ( ! $comments ) {
				...
				set_transient( 'omniverse_criteria_hash_' . $product_id . '_' . $criteria_id ,  $comments );
			}
	...
	private function get_summary_criteria_list_from_theme_settings() {
		...
		if ( ! omniverse_get_opt( 'reviews_rating_by_criteria' ) ) {
		...
		for ( $i = 1; $i <= 6; $i++ ) {
			...
			if ( ! omniverse_get_opt( $criteria_slug_id ) || ! omniverse_get_opt( $criteria_title_id ) ) {
				continue;
			}

			$summary_criteria_list[ omniverse_get_opt( $criteria_slug_id ) ] = omniverse_get_opt( $criteria_title_id );
		}
