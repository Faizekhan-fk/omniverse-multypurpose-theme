...
class Pros_Cons {
	...
	public function add_pros_cons_fields( $comment_form ) {
		if ( ! omniverse_get_opt( 'reviews_enable_pros_cons' ) ) {
			return $comment_form;
		}

		$comment_form['comment_field'] .= '<p class="comment-form-pros"><label for="pros">' . esc_html__( 'Pros', 'omniverse' ) . '</label><input id="pros" name="pros" type="text" value="" size="30"/></p>';

		$comment_form['comment_field'] .= '<p class="comment-form-cons"><label for="cons">' . esc_html__( 'Cons', 'omniverse' ) . '</label><input id="cons" name="cons" type="text" value="" size="30"/></p>';
	...
	public function render( $comment_content, $comment ) {
		if ( ! omniverse_get_opt( 'reviews_enable_pros_cons' ) || ( ! wp_doing_ajax() && ( is_admin() || ! is_singular( 'product' ) ) ) ) {
		...
		<div class="wd-review-arguments">
			<?php foreach ( $pros_list as $pros ) : ?>
				...
				<div class="wd-pros">
					<div class="wd-argument-label">
						<?php echo esc_html__( 'Pros:', 'omniverse' ); ?>
					</div>
					<p>
			...
			<?php foreach ( $cons_list as $cons ) : ?>
				...
				<div class="wd-cons">
					<div class="wd-argument-label">
						<?php echo esc_html__( 'Cons:', 'omniverse' ); ?>
					</div>
					<p>
	...
	public function add_metaboxes() {
		if ( ! omniverse_get_opt( 'reviews_enable_pros_cons' ) || ! isset( $_REQUEST['action'] ) || 'editcomment' !== $_REQUEST['action'] ) {
			return;
		}
		...
		$metaboxes = Metaboxes::add_metabox(
			array(
				'id'     => 'omniverse_review_pros_cons_metabox',
				'title'  => esc_html__( 'Review pros/cons metabox', 'omniverse' ),
		...
		$metaboxes->add_field(
			array(
				'id'       => 'pros',
				'name'     => esc_html__( 'Pros', 'omniverse' ),
				'type'     => 'text_input',
				'section'  => 'general',
				'default'  => $pros_text,
				'priority' => 10,
		...
		$metaboxes->add_field(
			array(
				'id'       => 'cons',
				'name'     => esc_html__( 'Cons', 'omniverse' ),
				'type'     => 'text_input',
				'section'  => 'general',
				'default'  => $cons_text,
				'priority' => 10,
