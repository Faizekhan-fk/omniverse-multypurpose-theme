...
foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
	...
	if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_checkout_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
		if ( $_product->is_sold_individually() || ! omniverse_get_opt( 'checkout_product_quantity' ) || ( ! empty( $cart_item['wd_fbt_parent_keys'] ) && isset( WC()->cart->cart_contents[ $cart_item['wd_fbt_parent_keys'] ] ) ) ) {
			ob_start();
			?>
			...
			omniverse_enqueue_js_script( 'checkout-quantity' );
		...
		if ( $product_permalink && omniverse_get_opt( 'checkout_link_to_product' ) ) {
		...
		<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
			<td class="wd-checkout-prod">
				<?php if ( omniverse_get_opt( 'checkout_remove_button' ) ) : ?>
					<div class="wd-checkout-remove-btn-wrapp"><?php
						omniverse_enqueue_js_script( 'checkout-remove-btn' );
				...
				<?php if ( omniverse_get_opt( 'checkout_show_product_image' ) ) : ?>
...
wp_nonce_field( 'omniverse_remove_product_from_checkout' );
