...
class Compare extends Singleton {
	 ...
	 * @var int
	 */
	private $cookie_name = 'omniverse_compare_list';
	...
	public function hooks() {
		...
		add_action( 'wp_ajax_omniverse_add_to_compare', array( $this, 'add_to_compare' ) );
		add_action( 'wp_ajax_nopriv_omniverse_add_to_compare', array( $this, 'add_to_compare' ) );

		add_action( 'wp_ajax_omniverse_remove_from_compare', array( $this, 'remove_from_compare' ) );
		add_action( 'wp_ajax_nopriv_omniverse_remove_from_compare', array( $this, 'remove_from_compare' ) );

		add_action( 'wp_ajax_omniverse_remove_category_from_compare', array( $this, 'remove_category_from_compare' ) );
		add_action( 'wp_ajax_nopriv_omniverse_remove_category_from_compare', array( $this, 'remove_category_from_compare' ) );

		add_action( 'wp_ajax_omniverse_get_fragment_product_category_compare', array( $this, 'get_fragment_product_category_compare' ) );
		add_action( 'wp_ajax_nopriv_omniverse_get_fragment_product_category_compare', array( $this, 'get_fragment_product_category_compare' ) );

		add_filter( 'omniverse_get_update_compare_fragments', array( $this, 'get_dropdown_compare_fragments' ) );
	...
	public function include_files() {
		...
		foreach ( $files as $file ) {
			require_once get_parent_theme_file_path( OMNIVERSE_FRAMEWORK . '/integrations/woocommerce/modules/compare/' . $file . '.php' );
		}
	}
	...
	public function add_localized_settings( $localized ) {
		$localized['compare_by_category']       = omniverse_get_opt( 'compare_by_category' ) ? 'yes' : 'no';
		$localized['compare_page_nonce']        = wp_create_nonce( 'wd-compare-page' );
		$localized['compare_save_button_state'] = omniverse_get_opt( 'compare_save_button_state' ) ? 'yes' : 'no';
	...
	public function add_to_compare() {
		...
		if ( ! in_array( $id, $products ) ) { //phpcs:ignore
			...
			setcookie( $this->cookie_name, wp_json_encode( $products ), 0, COOKIEPATH, COOKIE_DOMAIN, omniverse_cookie_secure_param(), false );
		...
		wp_send_json(
			array(
				...
				'fragments' => apply_filters( 'omniverse_get_update_compare_fragments', array() ),
			)
		);
	}
	...
	public function remove_from_compare() {
		...
		if ( isset( $_GET['category_id'] ) && omniverse_get_opt( 'compare_by_category' ) ) { //phpcs:ignore
		...
		if ( ! $products ) {
			setcookie( $this->cookie_name, false, 0, COOKIEPATH, COOKIE_DOMAIN, omniverse_cookie_secure_param(), false );
		...
		if ( 'wpb' === omniverse_get_current_page_builder() && class_exists( 'WPBMap' ) ) {
	...
	public function remove_category_from_compare() {
		...
		if ( ! $product_ids ) {
			...
			setcookie( $this->cookie_name, wp_json_encode( $product_ids ), 0, COOKIEPATH, COOKIE_DOMAIN, omniverse_cookie_secure_param(), false );
	...
	public function compare_available_fields( $new = false ) {
		...
		if ( $new ) {
			$options = array(
				'description'  => array(
					'name'  => esc_html__( 'Description', 'omniverse' ),
					'value' => 'description',
				),
				'dimensions'   => array(
					'name'  => esc_html__( 'Dimensions', 'omniverse' ),
					'value' => 'dimensions',
				),
				'weight'       => array(
					'name'  => esc_html__( 'Weight', 'omniverse' ),
					'value' => 'weight',
				),
				'availability' => array(
					'name'  => esc_html__( 'Availability', 'omniverse' ),
					'value' => 'availability',
				),
				'sku'          => array(
					'name'  => esc_html__( 'Sku', 'omniverse' ),
					'value' => 'sku',
				),
			);
		...
		$fields = array(
			'enabled'  => array(
				'description'  => esc_html__( 'Description', 'omniverse' ),
				'sku'          => esc_html__( 'Sku', 'omniverse' ),
				'availability' => esc_html__( 'Availability', 'omniverse' ),
			),
			'disabled' => array(
				'weight'     => esc_html__( 'Weight', 'omniverse' ),
				'dimensions' => esc_html__( 'Dimensions', 'omniverse' ),
			),
		);
	...
	public function remove_unnecessary_products() {
		...
		if ( ! isset( $post->ID ) || intval( omniverse_get_opt( 'compare_page' ) ) !== $post->ID || get_transient( 'wd_compare_unnecessary_products' ) ) {
			return;
		}
