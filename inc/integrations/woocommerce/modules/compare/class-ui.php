...
class Ui extends Singleton {
	...
	public function init() {
		...
		add_action( 'omniverse_sticky_atc_actions', array( $this, 'add_to_compare_sticky_atc_btn' ), 10 );
	}
	...
	public function add_to_compare_btn( $classes = '' ) {
		...
		<div class="wd-compare-btn product-compare-button <?php echo esc_attr( $classes ); ?>">
			<a href="<?php echo esc_url( $url ); ?>" data-id="<?php echo esc_attr( $product_id ); ?>" rel="nofollow" data-added-text="<?php esc_html_e( 'Compare products', 'omniverse' ); ?>">
				<span><?php esc_html_e( 'Compare', 'omniverse' ); ?></span>
			</a>
		</div>
		<?php
	}
	...
	public function compare_page( $active_category = '' ) {
		if ( ! omniverse_woocommerce_installed() || ! omniverse_get_opt( 'compare', 1 ) ) {
			return '';
		}
		...
		if ( $products ) {
			...
			if ( omniverse_get_opt( 'compare_by_category' ) && 1 < count( $products_categories ) ) {
				omniverse_enqueue_inline_style( 'page-compare-by-category' );
					...
					<div class="wd-compare-content">
						<?php foreach ( $products_categories as $category_id => $category ) : ?>
							...
							if ( intval( $active_category ) === $category_id ) {
								...
								if ( empty( $_GET['action'] ) || 'omniverse_remove_category_from_compare' !== $_GET['action'] ) { //phpcs:ignore
									$classes .= ' wd-in';
								}
							}
							?>
		...
		<div class="wd-compare-header">
			<div class="wd-compare-select-wrap">
				<label for="wd-compare-select" class="title">
					<?php esc_html_e( 'Compare by categories:', 'omniverse' ); ?>
				</label>
	...
	public function get_empty_compare_content() {
		$empty_compare_text = omniverse_get_opt( 'empty_compare_text' );

		omniverse_enqueue_inline_style( 'woo-page-empty-page' );

		?>
		<p class="wd-empty-compare wd-empty-page">
			<?php esc_html_e( 'Compare list is empty.', 'omniverse' ); ?>
		</p>
		...
		<p class="return-to-shop">
			<a class="button" href="<?php echo esc_url( apply_filters( 'omniverse_compare_return_to_shop_url', wc_get_page_permalink( 'shop' ) ) ); ?>">
	...
	public function is_products_have_field( $field_id, $products, $category ) {
		foreach ( $products as $product ) {
			if ( isset( $product[ $field_id ] ) && ( ! empty( $product[ $field_id ] ) && apply_filters( 'omniverse_compare_empty_field_symbol', '-' ) !== $product[ $field_id ] && 'N/A' !== $product[ $field_id ] ) && ( ! $category || in_array( $category, $product['category'], true ) ) ) {
				return true;
			}
		}
	...
	public function compare_display_field( $field_id, $product ) {
		...
		switch ( $type ) {
			case 'basic':
				...
				<div class="wd-compare-remove-action wd-action-btn wd-style-text wd-cross-icon">
					<a href="#" rel="nofollow" class="wd-compare-remove" data-id="<?php echo esc_attr( $product['id'] ); ?>">
						<?php echo esc_html__( 'Remove', 'omniverse' ); ?>
					</a>
				</div>
				...
				if ( ! omniverse_get_opt( 'catalog_mode' ) ) {
					echo apply_filters( 'omniverse_compare_add_to_cart_btn', $product['basic']['add_to_cart'] ); // phpcs:ignore
				}
				break;

			case 'attribute':
				if ( omniverse_get_opt( 'brands_attribute' ) === $field_id ) {
					$brands = wc_get_product_terms( $product['id'], $field_id, array( 'fields' => 'all' ) );

					if ( empty( $brands ) ) {
						echo apply_filters( 'omniverse_compare_empty_field_symbol', '-' ); //phpcs:ignore
						return;
					}
					...
					foreach ( $brands as $brand ) {
						...
						if ( ! empty( $image ) ) {
							?>
							<div class="wd-compare-brand<?php echo esc_attr( omniverse_get_old_classes( ' omniverse-compare-brand' ) ); ?>">
								<?php echo apply_filters( 'omniverse_image', '<img src="' . esc_url( $image ) . '" title="' . esc_attr( $brand->name ) . '" alt="' . esc_attr( $brand->name ) . '" />' ); //phpcs:ignore ?>
							</div>
							<?php
						} else {
			...
			case 'weight':
				if ( $product[ $field_id ] ) {
					$unit = apply_filters( 'omniverse_compare_empty_field_symbol', '-' ) !== $product[ $field_id ] ? get_option( 'woocommerce_weight_unit' ) : '';
	...
	public function get_compared_products_data() {
		...
		$divider = apply_filters( 'omniverse_compare_empty_field_symbol', '-' );
			...
			foreach ( $fields as $field_id => $field_name ) {
				if ( taxonomy_exists( $field_id ) ) {
					...
					if ( ! empty( $terms ) ) {
						...
						$products_data[ $product->get_id() ][ $field_id ][] = apply_filters( 'omniverse_compare_empty_field_symbol', '-' );
					}
	...
	public function get_compare_fields() {
		...
		$fields_settings = omniverse_get_opt( 'fields_compare' );

		if ( class_exists( 'DN\Admin\Modules\Options' ) && $fields_settings && count( $fields_settings ) > 1 ) {
			$fields_labels = omniverse_compare_available_fields();
	...
	public function compare_availability_html( $product ) {
		...
		if ( empty( $availability['availability'] ) ) {
			...
			if ( isset( $availability['class'] ) ) {
				$availability['class'] .= ' wd-style-' . omniverse_get_opt( 'stock_status_design', 'default' );
			}

			if ( 'with-bg' === omniverse_get_opt( 'stock_status_design', 'default' ) || 'bordered' === omniverse_get_opt( 'stock_status_design', 'default' ) ) {
		...
		<div class="wd-dropdown wd-dropdown-menu wd-dropdown-compare wd-design-default">
			<ul class="wd-sub-menu">
				<?php foreach ( $categories as $category_id => $category ) : ?>
					<li>
						<a href="<?php echo esc_url( add_query_arg( 'product_cat', $category_id, omniverse_get_compare_page_url() ) ); ?>">
