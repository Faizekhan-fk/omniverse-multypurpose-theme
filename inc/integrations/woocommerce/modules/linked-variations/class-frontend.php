...
class Frontend extends Singleton {
			...
			<table class="variations">
				<tbody>
					<?php foreach ( $linked_variations_data as $attr_slug => $attr_data ) : ?>
						<?php
						$index            = 0;
						$swatch_size      = omniverse_wc_get_attribute_term( $attr_slug, 'swatch_size' );
						$swatch_dis_style = omniverse_wc_get_attribute_term( $attr_slug, 'swatch_dis_style' );
						$swatch_style     = omniverse_wc_get_attribute_term( $attr_slug, 'swatch_style' );
						$swatch_shape     = omniverse_wc_get_attribute_term( $attr_slug, 'swatch_shape' );
									...
									<?php foreach ( $attr_data['terms'] as $term_slug => $term_data ) : ?>
										...
										if ( $this->linked_data['use_image'] && in_array( $attr_slug, $this->linked_data['use_image'], true ) ) {
											...
											if ( is_array( $term_meta['image'] ) ) {
												...
												$image = '<img src="' . $term_meta['image'] . '" alt="' . esc_attr__( 'Swatch image', 'omniverse' ) . '">';
											}
										} else {
	...
	private function set_linked_data( $product_id ) {
		$post = new WP_Query(
			[
				'post_type'   => 'omniverse_woo_lv',
				'numberposts' => 1,
				'meta_query'  => [ // phpcs:ignore
					[
						'key'     => '_omniverse_linked_products',
		...
		$this->linked_data = [
			'products'  => get_post_meta( $post->posts[0]->ID, '_omniverse_linked_products', true ),
			'attrs'     => get_post_meta( $post->posts[0]->ID, '_omniverse_linked_attrs', true ),
			'use_image' => get_post_meta( $post->posts[0]->ID, '_omniverse_linked_use_product_image', true ),
		];
	}
