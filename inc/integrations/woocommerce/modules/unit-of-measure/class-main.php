...
namespace DN\Modules\Unit_Of_Measure;
...
class Main extends Singleton {
	...
	public function output_control() {
		...
		woocommerce_wp_text_input(
			array(
				'id'          => 'omniverse_price_unit_of_measure',
				'label'       => esc_html__( 'Unit of measurement', 'omniverse' ),
				'desc_tip'    => 'true',
				'description' => esc_html__( 'Unit of measurement for the product quantity. For example KG, ML, meters, liters, etc.', 'omniverse' ),
				'type'        => 'text',
			)
		);
		echo '</div>';
	...
	public function output_control_in_bulk_edit() {
		...
		<div class="inline-edit-group dimensions">
			<label class="alignleft">
				<span class="title">
					<?php esc_html_e( 'Unit of measurement', 'omniverse' ); ?>
				</span>
			...
			<label class="change-input">
				<input type="text" name="omniverse_price_unit_of_measure" class="text stock omniverse_price_unit_of_measure" placeholder="<?php esc_attr_e( 'Unit of measurement', 'omniverse' ); ?>" value="">
			</label>
		</div>
		<?php
	}
	...
	public function save_control_product_meta() {
		if ( ! isset( $_POST['omniverse_price_unit_of_measure'] ) ) { //phpcs:ignore
			return;
		}

		$this->save_option_for_product( get_the_ID(), wp_kses( $_POST['omniverse_price_unit_of_measure'], true ) ); //phpcs:ignore
	}
	...
	public function save_control_in_bulk_edit() {
		if ( ! isset( $_GET['post'] ) || ! isset( $_GET['change_unit_of_measure'] ) || ! $_GET['change_unit_of_measure'] || ! isset( $_GET['omniverse_price_unit_of_measure'] ) ) { //phpcs:ignore
			return;
		}

		$posts_id = omniverse_clean( $_GET['post'] ); //phpcs:ignore
		$option   = wp_kses( $_GET['omniverse_price_unit_of_measure'], true ); //phpcs:ignore
	...
	public function save_option_for_product( $product_id, $option ) {
		update_post_meta( $product_id, 'omniverse_price_unit_of_measure', $option );
	}
	...
	public function get_unit_of_measure_db( $wc_product ) {
		...
		return get_post_meta( $id, 'omniverse_price_unit_of_measure', true );
	}
}
