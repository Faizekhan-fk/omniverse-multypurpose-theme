...
class WC_Wishlist {
	...
	public function init() {
		...
		$this->products_table  = $wpdb->prefix . 'omniverse_wishlist_products';
		$this->wishlists_table = $wpdb->prefix . 'omniverse_wishlists';
		...
		$wpdb->omniverse_products_table  = $this->products_table;
		$wpdb->omniverse_wishlists_table = $this->wishlists_table;

		add_action( 'after_switch_theme', array( $this, 'install' ) );
		add_action( 'admin_init', array( $this, 'theme_settings_install' ), 100 );

		add_action( 'wp_ajax_omniverse_add_to_wishlist', array( $this, 'add_to_wishlist_action' ) );
		add_action( 'wp_ajax_nopriv_omniverse_add_to_wishlist', array( $this, 'add_to_wishlist_action' ) );

		add_action( 'wp_ajax_omniverse_remove_from_wishlist', array( $this, 'remove_from_wishlist_action' ) );
		add_action( 'wp_ajax_nopriv_omniverse_remove_from_wishlist', array( $this, 'remove_from_wishlist_action' ) );

		add_filter( 'omniverse_localized_string_array', array( $this, 'update_localized_settings' ) );
	...
	public function custom_rewrite_rule() {
		$id   = (int) omniverse_get_opt( 'wishlist_page' );
	...
	public function add_to_wishlist_action() {
		check_ajax_referer( 'omniverse-wishlist-add', 'key' );

		if ( ! is_user_logged_in() && omniverse_get_opt( 'wishlist_logged' ) ) {
			return false;
		}
		...
		if ( isset( $_GET['group'] ) ) { //phpcs:ignore
			$group = omniverse_clean( $_GET['group'] ); //phpcs:ignore
		}

		if ( $group && 'disable' !== omniverse_get_opt( 'wishlist_show_popup', 'disable' ) && is_user_logged_in() ) {
	...
	public function remove_from_wishlist_action() {
		...
		$product_id   = omniverse_clean( $_GET['product_id'] ); //phpcs:ignore
		$group_id     = '';
		$product_atts = array();

		if ( isset( $_GET['atts'] ) ) {
			$product_atts = omniverse_clean( $_GET['atts'] ); //phpcs:ignore
		}
		...
		if ( $products ) {
			...
			$content = omniverse_shortcode_products( $product_atts );
	...
	public function remove_product_from_wishlist( $wishlist, $product_id, $group_id ) {
		...
		do_action( 'omniverse_remove_product_from_wishlist', $product_id );
	}
	...
	private function define_constants() {
		if ( ! defined( 'ZS_WISHLIST_DIR' ) ) {
			define( 'ZS_WISHLIST_DIR', OMNIVERSE_THEMEROOT . '/inc/integrations/woocommerce/modules/wishlist/' );
		}
	}
	...
	private function include_files() {
		...
		if ( omniverse_get_opt( 'wishlist_expanded' ) && is_user_logged_in() ) {
			$files[] = 'class-wishlist-group';
		}

		foreach ( $files as $file ) {
			$path = ZS_WISHLIST_DIR . $file . '.php';
			if ( file_exists( $path ) ) {
				require_once $path;
			}
		}
	...
	public function install() {
		if ( $this->is_installed || ! omniverse_get_opt( 'wishlist', 1 ) ) {
			return;
		}
		...
		update_option( 'omniverse_added_column_on_sale_in_product_db', true );
	...
	public function update_localized_settings( $settings ) {
		$settings['wishlist_expanded']          = ( omniverse_get_opt( 'wishlist_expanded' ) && is_user_logged_in() ) ? 'yes' : 'no';
		$settings['wishlist_show_popup']        = omniverse_get_opt( 'wishlist_show_popup' );
		...
		$settings['wishlist_remove_notice']     = esc_html__( 'Do you really want to remove these products?', 'omniverse' );
		$settings['wishlist_hash_name']         = apply_filters( 'omniverse_wishlist_hash_name', 'omniverse_wishlist_hash_' . md5( get_current_blog_id() . '_' . get_site_url( get_current_blog_id(), '/' ) ) );
		$settings['wishlist_fragment_name']     = apply_filters( 'omniverse_wishlist_fragment_name', 'omniverse_wishlist_fragments_' . md5( get_current_blog_id() . '_' . get_site_url( get_current_blog_id(), '/' ) ) );
		$settings['wishlist_save_button_state'] = omniverse_get_opt( 'wishlist_save_button_state', '0' ) ? 'yes' : 'no';

		if ( omniverse_get_opt( 'wishlist_expanded' ) ) {
			$settings['wishlist_current_default_group_text'] = esc_html__( 'Current default group', 'omniverse' );
			$settings['wishlist_default_group_text']         = esc_html__( 'Default group', 'omniverse' );
			$settings['wishlist_rename_group_notice']        = esc_html__( 'Title is empty!', 'omniverse' );
		}
