...
use DN\WC_Wishlist\DB_Storage;
use DN\WC_Wishlist\Cookies_Storage;
...
class Wishlist {
	...
	public function __construct( $id = false, $user_id = false, $read_only = false ) {
		...
		if ( omniverse_get_opt( 'wishlist_page' ) && get_queried_object_id() === (int) omniverse_get_opt( 'wishlist_page' ) ) {
	...
	public function remove_all_user_wishlists( $id ) {
		...
		$wpdb->query(
			"DELETE FROM $wpdb->omniverse_products_table
				WHERE $wpdb->omniverse_products_table.wishlist_id IN ( $where_query )"
		);
	...
	public function remove_unnecessary_products() {
		if ( is_user_logged_in() ) {
			...
			$wishlist_cleared_time = omniverse_get_cookie( 'wishlist_cleared_time' );
		}
		...
		if ( is_user_logged_in() ) {
			...
			omniverse_set_cookie( 'wishlist_cleared_time', time() );
		}
	}
	...
	private function create( $group = '' ) {
		...
		if ( get_option( 'omniverse_upgrade_database_wishlist' ) && omniverse_get_opt( 'wishlist_expanded' ) ) {
			delete_user_meta( $this->get_user_id(), 'omniverse_wishlist_groups' );

			if ( ! $group ) {
				$group = esc_html__( 'My wishlist', 'omniverse' );
			}
	...
	public function update_count_cookie() {
		$cookie_name = 'omniverse_wishlist_count';

		if ( is_multisite() ) {
			$cookie_name .= '_' . get_current_blog_id();
		}

		omniverse_set_cookie( $cookie_name, $this->get_count() );
	}
	...
	public function move_products_if_needed( $user_login, $user ) {
		...
		if ( $db_products ) {
			...
			$cookie_name_count = 'omniverse_wishlist_count';

			if ( is_multisite() ) {
				$cookie_name_count .= '_' . get_current_blog_id();
			}

			omniverse_set_cookie( $cookie_name_count, count( $products ) );
