...
class Wishlists_Group extends Singleton {
	...
	public function init() {
		...
		add_action( 'wp_ajax_omniverse_remove_group_from_wishlist', array( $this, 'remove_group_from_wishlist_action' ) );

		add_action( 'wp_ajax_omniverse_rename_wishlist_group', array( $this, 'rename_wishlist_group' ) );

		add_action( 'wp_ajax_omniverse_save_wishlist_group', array( $this, 'create_wishlist_group' ) );

		add_action( 'wp_ajax_omniverse_move_products_from_wishlist', array( $this, 'move_products_from_wishlist' ) );

		add_action( 'wp_ajax_omniverse_get_wishlist_fragments', array( $this, 'get_wishlist_fragments' ) );

		add_filter( 'omniverse_get_update_wishlist_fragments', array( $this, 'get_wishlist_groups_fragments' ) );
		add_filter( 'omniverse_get_update_wishlist_fragments', array( $this, 'get_wishlist_count' ) );

		add_filter( 'omniverse_get_wishlist_hash', array( $this, 'get_wishlist_hash' ) );

		add_action( 'omniverse_before_wp_footer', array( $this, 'wishlist_create_group_popup' ), 250 );
	}
	...
	public function remove_group_from_wishlist_action() {
		...
		$response = array(
			...
			'fragments' => apply_filters( 'omniverse_get_update_wishlist_fragments', array() ),
			'hash'      => apply_filters( 'omniverse_get_wishlist_hash', '' ),
		);

		add_filter( 'omniverse_is_ajax', '__return_false' );
	...
	public function rename_wishlist_group() {
		...
		$group_id = omniverse_clean( $_GET['group_id'] ); //phpcs:ignore
		$title    = omniverse_clean( $_GET['title'] ); //phpcs:ignore
	...
	public function create_wishlist_group() {
		...
		$wishlist_id = $wishlist->create_group( omniverse_clean( $_GET['group'] ) ); //phpcs:ignore
	...
	public function move_products_from_wishlist() {
		...
		$groups_ids  = omniverse_get_wishlist_groups();
	...
	public function get_wishlist_groups_fragments( $fragments ) {
		...
		if ( omniverse_get_wishlist_groups() ) {
			ob_start();
	...
	public function get_wishlist_hash() {
		return md5( wp_json_encode( omniverse_get_wishlist_groups() ) . omniverse_get_wishlist_count() );
	}
	...
	public function set_cookies() {
		...
		$hash_name = 'omniverse_wishlist_hash';
		...
		$setcookies = apply_filters(
			'omniverse_set_cookies_wishlist',
			array(
		...
		foreach ( $setcookies as $name => $value ) {
			if ( omniverse_get_cookie( $name ) !== $value ) {
				omniverse_set_cookie( $name, $value );
			}
		}
	}
	...
	public function upgrade_database_wishlist() {
		if ( get_option( 'omniverse_upgrade_database_wishlist' ) ) {
			return;
		}

		global $wpdb;

		$default_wishlist_name = esc_html__( 'My wishlist', 'omniverse' );

		$wpdb->query(
			"ALTER TABLE {$wpdb->omniverse_wishlists_table} 
				ADD COLUMN wishlist_group varchar( 255 ) DEFAULT '{$default_wishlist_name}' NOT NULL AFTER user_id"
		);

		update_option( 'omniverse_upgrade_database_wishlist', true );
	}
	...
	public function wishlist_create_group_popup() {
		$page_id = omniverse_get_opt( 'wishlist_page' );

		if ( defined( 'ICL_SITEPRESS_VERSION' ) && function_exists( 'wpml_object_id_filter' ) ) {
			$page_id = wpml_object_id_filter( $page_id, 'page', true );
		}

		if ( 'disable' === omniverse_get_opt( 'wishlist_show_popup', 'disable' ) && get_the_ID() !== (int) $page_id && ! wp_doing_ajax() ) {
			return;
		}

		omniverse_enqueue_inline_style( 'mfp-popup' );
		omniverse_enqueue_inline_style( 'page-wishlist-popup' );
		omniverse_enqueue_js_library( 'magnific' );

		$wishlist_groups = omniverse_get_wishlist_groups();
		...
		<div class="mfp-with-anim wd-popup wd-close-btn-inset wd-popup-wishlist">
			<div class="wd-wishlist-back-btn wd-action-btn wd-style-text">
				<a href="#">
					<?php esc_html_e( 'Back to list', 'omniverse' ); ?>
				</a>
			</div>
			...
			<ul class="wd-wishlist-group-list" data-product-id="" data-nonce="" data-group-count="<?php echo count( $wishlist_groups ); ?>">
				...
				<li data-group-id="add_new">
					<span class="wd-wishlist-add-group wd-action-btn wd-style-text">
						<a href="#">
							<?php esc_html_e( 'Add new wishlist', 'omniverse' ); ?>
						</a>
					</span>
				</li>
			</ul>
			...
			<div class="wd-wishlist-create-group">
				<label for="wd-wishlist-group-name">
					<?php esc_html_e( 'Create wishlist', 'omniverse' ); ?>
				</label>
			...
			<div class="wd-wishlist-add-success set-mb-m reset-last-child">
				<span class="title">
					<?php esc_html_e( 'Product was successfully added to your wishlist.', 'omniverse' ); ?>
				</span>
				<a href="<?php echo esc_url( omniverse_get_wishlist_page_url() ); ?>" class="wd-wishlist-back-to-lists btn btn-full-width">
					<?php esc_html_e( 'Browse wishlist', 'omniverse' ); ?>
				</a>
				<a href="#" class="wd-wishlist-back-to-shop btn btn-style-link btn-color-default">
					<?php esc_html_e( 'Back to shop', 'omniverse' ); ?>
				</a>
			</div>
			<a href="#" data-group-id="save" class="btn btn-color-primary btn-full-width wd-wishlist-save-btn" data-added-text="<?php esc_html_e( 'Add to wishlist', 'omniverse' ); ?>" data-create-text="<?php esc_html_e( 'Create wishlist', 'omniverse' ); ?>" data-move-text="<?php esc_html_e( 'Move to wishlist', 'omniverse' ); ?>">
				<?php esc_html_e( 'Add to wishlist', 'omniverse' ); ?>
			</a>
		</div>
		<?php
	}
	...
	public function output_header_for_wishlist_groups() {
		omniverse_enqueue_inline_style( 'page-wishlist-group' );
		omniverse_enqueue_js_script( 'wishlist-group' );

		?>
		<div class="wd-wishlist-head wd-border-off">
			<h4 class="title">
				<?php esc_html_e( 'Your wishlists', 'omniverse' ); ?>
			</h4>
		...
		foreach ( $wishlist_groups as $key => $wishlist_group ) {
			...
			<div class="wd-wishlist-group" data-group-id="<?php echo esc_attr( $wishlist_group['ID'] ); ?>">
				...
				if ( $wishlist_group_product_ids ) {
					...
					echo omniverse_shortcode_products( apply_filters( 'omniverse_wishlist_products_settings', $args ) ); //phpcs:ignore
				} else {
