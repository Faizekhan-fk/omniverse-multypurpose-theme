...
class Popular_Products extends WP_List_Table {
	...
	public function column_name( $item ) {
		...
		$actions     = array(
			...
			'edit'         => sprintf( '<a href="%s" title="%s">%s</a>', $product_edit_url, esc_html__( 'Edit this item', 'omniverse' ), esc_html__( 'Edit', 'omniverse' ) ),
			'view_users'   => sprintf(
				...
				esc_html__( 'View a list with customers that have added this product to their wishlist', 'omniverse' ),
				esc_html__( 'View customers', 'omniverse' )
			),
			'view_product' => sprintf( '<a href="%s" title="%s" rel="permalink">%s</a>', $product_url, esc_html__( 'View Product', 'omniverse' ), esc_html__( 'View Product', 'omniverse' ) ),
		);
	...
	public function column_create_promotion( $item ) {
		return sprintf(
			'<a href="%s" class="dn-btn dn-color-primary wd-create-promotion">%s</a>',
			esc_url(
				wp_nonce_url(
					add_query_arg(
						array(
							'page'       => 'dn-wishlist-settings-page',
							'tab'        => 'dn-popular-products-in-wishlist',
							'product_id' => $item['product_id'],
						),
						admin_url( 'edit.php?post_type=product' )
					),
	...
	public function get_columns() {
		return array(
			...
			'category'         => esc_html__( 'Category', 'omniverse' ),
			'count'            => esc_html__( 'Wishlist count', 'omniverse' ),
	...
	public function process_bulk_action() {
		...
		if ( 'create-promotion' === $this->current_action() && ! empty( $popular_products_ids ) ) {
			...
			wp_safe_redirect( admin_url( '/edit.php?post_type=product&page=dn-wishlist-settings-page&tab=dn-popular-products-in-wishlist' ) );
			die();
		}
	}
	...
	private function table_data() {
		...
		if ( $_product_id ) {
			$where_query[] = $wpdb->prepare( "$wpdb->omniverse_products_table.product_id = %d", $_product_id );
		}
		...
		if ( ! wp_cache_get( 'popular_products_list_table' ) ) {
			...
			wp_cache_set(
				...
				$wpdb->get_results( //phpcs:ignore;
					"SELECT
						$wpdb->omniverse_products_table.product_id,
						$wpdb->posts.post_title as name,
						count( $wpdb->omniverse_products_table.wishlist_id ) as count
						FROM $wpdb->omniverse_products_table 
							INNER JOIN $wpdb->posts
							    ON $wpdb->omniverse_products_table.product_id = $wpdb->posts.ID"
					. $where_query_text .
					" GROUP BY $wpdb->omniverse_products_table.product_id;",
					ARRAY_A
				)
			);
		}
	...
	public static function get_user_ids_by_product_id( $product_id ) {
		...
		$user_ids_from_db = $wpdb->get_results( //phpcs:ignore;
			"SELECT $wpdb->omniverse_wishlists_table.user_id
				FROM $wpdb->omniverse_wishlists_table
			        INNER JOIN $wpdb->omniverse_products_table
	...
	protected function extra_tablenav( $which ) {
		...
		wp_enqueue_style(
			'dn-jquery-ui',
			OMNIVERSE_ASSETS . '/css/jquery-ui.css',
			array(),
			OMNIVERSE_VERSION
		);
		?>
		...
		<input
			...
			placeholder="<?php esc_html_e( 'From:', 'omniverse' ); ?>"
		...
		<input
			...
			placeholder="<?php esc_html_e( 'To:', 'omniverse' ); ?>"
			style="min-width: 200px; vertical-align:middle;"
		/>
		<?php
		submit_button( esc_html__( 'Filter', 'omniverse' ), 'button', 'filter_action', false, array( 'id' => 'post-query-submit' ) );
