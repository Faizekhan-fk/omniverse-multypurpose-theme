...
class Wishlists extends WP_List_Table {
	...
	public function column_wishlist_group( $item ) {
		...
		if ( '' != get_option('permalink_structure') ) {
			$view_wishlist_page = omniverse_get_wishlist_page_url() . $item['ID'] . '/';
		} else {
		...
		$actions = apply_filters(
			'omniverse_admin_table_column_name_actions',
			array(
				'view'   => sprintf(
					'<a href="%s">%s</a>',
						esc_url( $view_wishlist_page ),
						esc_html__( 'View', 'omniverse' )
					),
				'delete' => sprintf(
					'<a href="%s">%s</a>',
						esc_url( $delete_wishlist_url ),
						esc_html__( 'Delete', 'omniverse' )
					),
			),
			$item,
			$delete_wishlist_url
	...
	public function column_user_name( $item ) {
		...
		if ( isset( $item['user_id'] ) ) {
			...
			if ( ! empty( $user ) ) {
				...
				$row = sprintf( '- %s -', esc_html__( 'guest', 'omniverse' ) );
			}
		}

		return apply_filters( 'omniverse_admin_table_column_username_row', $row, $item );
	}
	...
	public function column_date_created( $item ) {
		...
		if ( isset( $item['date_created'] ) ) {
			...
			if ( $time_diff < DAY_IN_SECONDS ) {
				...
				$row = sprintf( esc_html__( '%s ago', 'omniverse' ), human_time_diff( $date_created ) );
			} else {
	...
	public function get_columns() {
		return array(
			...
			'wishlist_group' => esc_html__( 'Name', 'omniverse' ),
			'user_name'      => esc_html__( 'Customer', 'omniverse' ),
			'product_count'  => esc_html__( 'Items in wishlist', 'omniverse' ),
			'date_created'   => esc_html__( 'Date created', 'omniverse' ),
		);
	}
	...
	public function get_bulk_actions() {
		return array(
			'delete' => esc_html__( 'Delete', 'omniverse' ),
		);
	}
	...
	private function table_data() {
		...
		if ( $search && $wishlist_group_exist ) {
			$where_query[] = $wpdb->prepare( "$wpdb->omniverse_wishlists_table.wishlist_group LIKE %s", '%' . $wpdb->esc_like( $search ) . '%' );
		}
		...
		if ( $_user_id ) {
			$where_query[] = $wpdb->prepare( "$wpdb->omniverse_wishlists_table.user_id = %d", $_user_id );
		}
		...
		if ( ! wp_cache_get( 'wishlists_list_table' ) ) {
			...
			wp_cache_set(
				...
				$wpdb->get_results( //phpcs:ignore;
					"SELECT
							$wpdb->omniverse_wishlists_table.*,
							$wpdb->users.user_login as user_name,
							COUNT( $wpdb->omniverse_products_table.product_id ) as product_count
						FROM $wpdb->omniverse_wishlists_table
						    ...
						    INNER JOIN $wpdb->users
           						 ON $wpdb->omniverse_wishlists_table.user_id = $wpdb->users.ID"
					. $where_query_text .
					" GROUP BY $wpdb->omniverse_wishlists_table.ID;",
					ARRAY_A
				)
			);
		}
