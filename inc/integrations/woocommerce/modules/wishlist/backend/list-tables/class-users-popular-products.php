...
namespace DN\WC_Wishlist\Backend\List_Table;

use WP_List_Table;
use DN\WC_Wishlist\Sends_About_Products_Wishlists;
use DN\WC_Wishlist\Sends_Promotional;
...
class Users_Popular_Products extends WP_List_Table {
	...
	public function column_default( $item, $column_name ) {
		if ( isset( $item[ $column_name ] ) ) {
			return apply_filters( 'omniverse_column_default', esc_html( $item[ $column_name ] ), $item, $column_name );
		} else {
			// Show the whole array for troubleshooting purposes.
			return apply_filters( 'omniverse_column_default', print_r( $item, true ), $item, $column_name ); // phpcs:ignore.
		}
	}
	...
	public function column_name( $item ) {
		if ( ! $item['user_id'] ) {
			return sprintf( '- %s -', esc_html__( 'guest', 'omniverse' ) );
		}
		...
		$actions     = array(
			...
			'edit'    => sprintf( '<a href="%s" title="%s">%s</a>', $user_edit_url, esc_html__( 'Edit this customer', 'omniverse' ), esc_html__( 'Edit', 'omniverse' ) ),
		);
	...
	public function column_wishlist( $item ) {
		...
		$actions = apply_filters(
			'omniverse_admin_table_wishlist_id_actions',
			array(
				'view'   => sprintf( '<a href="%s">%s</a>', esc_url( omniverse_get_wishlist_page_url() . $item['wishlist_id'] . '/' ), esc_html__( 'View', 'omniverse' ) ),
				'delete' => sprintf( '<a href="%s">%s</a>', esc_url( $delete_wishlist_url ), esc_html__( 'Delete', 'omniverse' ) ),
			),
			$item,
			$delete_wishlist_url
		);
		...
		return sprintf(
			"<a href='%s'>%s</a>%s",
			esc_url( omniverse_get_wishlist_page_url() . $item['wishlist_id'] . '/' ),
			( ! empty( $item['wishlist_group'] ) ) ? $item['wishlist_group'] : esc_html__( 'My wishlist', 'omniverse' ),
	...
	public function column_create_promotion( $item ) {
		return sprintf(
			'<a href="%s" class="dn-btn dn-color-primary wd-create-promotion %s">%s</a>',
			esc_url(
				wp_nonce_url(
					add_query_arg(
						array_merge(
					...
					'omniverse_send_promotion_email'
				)
			),
			esc_html( in_array( get_userdata( $item['user_id'] )->user_email, get_option( 'omniverse_wishlist_unsubscribed_users', array() ) ) ? 'dn-disabled' : '' ),
			esc_html__( 'Create promotion', 'omniverse' )
		);
	}
	...
	public function get_columns() {
		return array(
			...
			'thumb'            => sprintf( '<span class="wc-image tips" data-tip="%s">%s</span>', esc_html__( 'Image', 'omniverse' ), esc_html__( 'Image', 'omniverse' ) ),
			'name'             => esc_html__( 'Name', 'omniverse' ),
			'wishlist'         => esc_html__( 'Wishlist', 'omniverse' ),
			'date_added'       => esc_html__( 'Added on', 'omniverse' ),
			'create_promotion' => sprintf(
				'%s<div class="dn-hint">
						<div class="dn-tooltip dn-top dn-top-left"><div class="dn-tooltip-inner">
							%s
						</div>
					</div>',
				esc_html__( 'Create promotion', 'omniverse' ),
				sprintf(
					'%s <a href="%s">%s</a>.',
					esc_html__( 'When you create a promotion, all customers that have a corresponding product in their wishlist will get an email. You can customize this email content in', 'omniverse' ),
					esc_url( admin_url( 'admin.php?page=wc-settings&tab=email&section=omniverse_promotional_email' ) ),
					esc_html__( 'WooCommerce -> Settings -> Emails -> Wishlist “Promotional” email', 'omniverse' )
				)
			),
		);
	}
	...
	public function get_bulk_actions() {
		return array(
			'create-promotion' => esc_html__( 'Create promotion', 'omniverse' ),
		);
	}
	...
	public function process_bulk_action() {
		...
		if ( 'create-promotion' === $this->current_action() && ! empty( $user_ids ) ) {
			if ( ! omniverse_check_this_email_notification_is_enabled( 'woocommerce_omniverse_promotional_email_settings', 'yes' ) ) {
				return;
			}
			...
			wp_safe_redirect( admin_url( '/edit.php?post_type=product&page=dn-wishlist-settings-page&tab=dn-users-popular-products' ) );
			die();
		}
	}
	...
	private function table_data() {
		...
		$wishlist_group_exist = ! empty(
			$wpdb->get_results( //phpcs:ignore;
				"SHOW COLUMNS FROM $wpdb->omniverse_wishlists_table LIKE 'wishlist_group'",
				ARRAY_A
			)
		);

		if ( $product_id ) {
			$where_query[] = $wpdb->prepare( "$wpdb->omniverse_products_table.product_id = %d", $product_id );
		}

		if ( ! wp_cache_get( 'users_popular_products_list_table' ) ) {
			$where_query_text    = ! empty( $where_query ) ? ' WHERE ' . implode( ' AND ', $where_query ) : '';
			$wishlist_group_text = $wishlist_group_exist ? " $wpdb->omniverse_wishlists_table.wishlist_group," : '';

			wp_cache_set(
				'users_popular_products_list_table',
				$wpdb->get_results( //phpcs:ignore;
					"SELECT
						$wpdb->omniverse_wishlists_table.user_id,
						$wpdb->omniverse_products_table.wishlist_id,"
						. $wishlist_group_text .
						"MAX($wpdb->omniverse_products_table.date_added) as date_added
					FROM $wpdb->omniverse_products_table
					INNER JOIN $wpdb->omniverse_wishlists_table
						ON $wpdb->omniverse_wishlists_table.ID = $wpdb->omniverse_products_table.wishlist_id"
					. $where_query_text .
					" GROUP BY $wpdb->omniverse_products_table.wishlist_id;",
					ARRAY_A
				)
			);
		}
