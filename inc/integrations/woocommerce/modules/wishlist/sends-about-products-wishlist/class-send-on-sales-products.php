...
class Send_On_Sales_Products extends Singleton {
	...
	public function init() {
		...
		add_action( 'omniverse_remove_product_from_wishlist', array( $this, 'remove_product_from_lists' ) );
	}
	...
	public function register_crons() {
		...
		$crons['omniverse_wishlist_register_on_sales_products'] = array(
			'schedule' => apply_filters( 'omniverse_schedule_register_on_sales_products', 'daily' ),
			'callback' => array( $this, 'register_on_sales_products' ),
		);
		$crons['omniverse_wishlist_on_sales_products_email']    = array(
			'schedule' => apply_filters( 'omniverse_schedule_on_sales_products_email', 'hourly' ),
	...
	public function load_wc_mailer() {
		add_action( 'omniverse_send_on_sale_products_mail', array( 'WC_Emails', 'send_transactional_email' ), 10, 2 );
	}
	...
	public function register_on_sales_products() {
		...
		$products_on_sales         = get_option( 'omniverse_wishlist_products_on_sale', array() );
		...
		if ( $products_id_on_sale ) {
			...
			update_option( 'omniverse_wishlist_products_on_sale', $products_on_sales, false );
		}
	}
	...
	public function send_on_sales_products_email() {
		$products_on_sales  = get_option( 'omniverse_wishlist_products_on_sale' );
		$unsubscribed_users = get_option( $this->unsubscribed_users, array() );
		$emails_limited     = apply_filters( 'omniverse_wishlist_send_emails_limited', 20 );
		$counter            = 1;
		...
		foreach ( $products_on_sales as $user_id => $products ) {
			...
			do_action( 'omniverse_send_on_sale_products_mail', $user_id, $products );
	...
	public function get_users_id_by_product_id( $product_id ) {
		...
		return $wpdb->get_col(
			$wpdb->prepare(
				"SELECT user_id FROM $wpdb->omniverse_wishlists_table INNER JOIN $wpdb->omniverse_products_table ON wishlist_id = $wpdb->omniverse_wishlists_table.ID WHERE product_id = %d",
				$product_id
			)
		);
	}
	...
	public function get_product_id_which_on_sale() {
		...
		return $wpdb->get_col(
			$wpdb->prepare(
				"SELECT product_id FROM $wpdb->omniverse_products_table WHERE on_sale = %d",
				1
			)
		);
	}
	...
	public function upgrade_database_wishlist() {
		if ( get_option( 'omniverse_added_column_on_sale_in_product_db' ) ) {
			return;
		}

		global $wpdb;

		$wpdb->query( "ALTER TABLE {$wpdb->omniverse_wishlists_table} ADD COLUMN on_sale boolean DEFAULT 0 NOT NULL AFTER date_added" );
	...
	public function remove_product_from_lists( $product_id ) {
		...
		$products_ids_on_sale = get_option( 'omniverse_wishlist_products_on_sale' );
		...
		if ( isset( $products_ids_on_sale[ $user_id ] ) ) {
			...
			if ( $key || 0 === $key ) {
				...
				update_option( 'omniverse_wishlist_products_on_sale', $products_ids_on_sale, false );
			}
		}
	}
}
