...
class Sends_Back_In_Stock extends Singleton {
	...
	public function init() {
		...
		add_action( 'omniverse_wishlist_send_back_in_stock_email', array( $this, 'send_back_in_stock_email' ) );

		if ( ! wp_next_scheduled( 'omniverse_wishlist_send_back_in_stock_email' ) ) {
			wp_schedule_event( time(), apply_filters( 'omniverse_schedule_send_back_in_stock_email', 'hourly' ), 'omniverse_wishlist_send_back_in_stock_email' );
		}
	}
	...
	public function load_wc_mailer() {
		add_action( 'omniverse_send_back_in_stock_mail', array( 'WC_Emails', 'send_transactional_email' ), 10, 2 );
	}
	...
	public function send_back_in_stock_email() {
		$products_back_in_stock = get_option( 'omniverse_wishlist_products_back_in_stock' );
		...
		foreach ( $products_back_in_stock as $user_id => $product_list ) {
			...
			do_action( 'omniverse_send_back_in_stock_mail', $user_id, $product_list );
		...
		update_option( 'omniverse_wishlist_products_back_in_stock', $products_back_in_stock, false );
	}
	...
	public function schedule_back_in_stock_emails( $product_id, $stock_status, $product ) {
		...
		$products_back_in_stock = get_option( 'omniverse_wishlist_products_back_in_stock', array() );
		$unsubscribed_users     = get_option( $this->unsubscribed_users, array() );
	...
	public function remove_product_from_lists( $product_id ) {
		...
		$product_ids_back_in_stock = get_option( 'omniverse_wishlist_products_back_in_stock' );
		...
		if ( isset( $product_ids_back_in_stock[ $user_id ] ) ) {
			...
			if ( $key || 0 === $key ) {
				...
				update_option( 'omniverse_wishlist_products_back_in_stock', $product_ids_back_in_stock, false );
			}
		}
	}
}
