...
class Back_In_Stock_Email extends WC_Email {
	...
	public function __construct() {
		$this->id          = 'omniverse_back_in_stock_email';
		$this->title       = esc_html__( 'Wishlist "Back in stock" email', 'omniverse' );
		$this->description = esc_html__( 'This email is sent to customers when an item of their wishlist is back in stock', 'omniverse' );

		$this->heading = esc_html__( 'An item of your wishlist is back in stock!', 'omniverse' );
		$this->subject = esc_html__( 'An item of your wishlist is back in stock!', 'omniverse' );

		$this->template_html  = 'emails/back-in-stock.php';
		$this->template_plain = 'emails/plain/back-in-stock.php';

		// Triggers for this email.
		add_action( 'omniverse_send_back_in_stock_mail_notification', array( $this, 'trigger' ), 10, 2 );
	...
	public function init_form_fields() {
		...
		$this->form_fields = array(
			...
			'product_exclusions'  => array(
				'title'       => esc_html__( 'Product exclusions', 'omniverse' ),
				'type'        => 'multiselect',
				'description' => esc_html__( 'Select the products for which you don\'t want to send a reminder email', 'omniverse' ),
				'class'       => 'wc-product-search',
				'options'     => $exclusions_options,
			),

			'category_exclusions' => array(
				'title'             => esc_html__( 'Category exclusions', 'omniverse' ),
				'type'              => 'multiselect',
				'class'             => 'wc-category-search',
				'description'       => esc_html__( 'Select the product categories for which you don\'t want to send a reminder email', 'omniverse' ),
