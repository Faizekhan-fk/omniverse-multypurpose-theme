...
class Promotional_Email extends WC_Email {
	...
	public function __construct() {
		$this->id          = 'omniverse_promotional_email';
		$this->title       = esc_html__( 'Wishlist "Promotional" email', 'omniverse' );
		$this->description = esc_html__( 'This email is sent to the customer that has a product on his wishlist. You can send these emails via Dashboard -> Products -> Wishlists -> Popular products -> Create promotion.', 'omniverse' );

		$this->heading = esc_html__( 'There is a deal for you!', 'omniverse' );
		$this->subject = esc_html__( 'A product of your wishlist is on sale!', 'omniverse' );
		...
		add_action( 'omniverse_send_promotional_mail_notification', array( $this, 'trigger' ), 10, 4 );
	...
	public function get_custom_content_html() {
		...
		$image_size = apply_filters( 'omniverse_promotional_email_thumbnail_item_size', array( 32, 32 ) );
		...
		$this->placeholders = array_merge(
			...
			array_merge(
				array(
					...
					'{wishlist_url}'    => omniverse_get_wishlist_page_url(),
				),
				! isset( $this->coupon ) ? array() : array(
		...
		foreach ( $this->items as $id => $product ) {
			$this->placeholders = array_merge(
				...
				array_merge(
					array(
						'{product_image_' . $id . '}'   => $product ? apply_filters( 'omniverse_promotional_email_item_thumbnail', '<div style="margin-bottom: 5px"><img src="' . ( $product->get_image_id() ? current( wp_get_attachment_image_src( $product->get_image_id(), 'thumbnail' ) ) : wc_placeholder_img_src() ) . '" alt="' . esc_attr__( 'Product image', 'omniverse' ) . '" height="' . esc_attr( $image_size[1] ) . '" width="' . esc_attr( $image_size[0] ) . '" style="vertical-align:middle; margin-' . ( is_rtl() ? 'left' : 'right' ) . ': 10px;" /></div>', $product ) : '',
		...
		return apply_filters( 'omniverse_custom_html_content_' . $this->id, $this->format_string( stripcslashes( $this->content_html ) ), $this->object );
	}
	...
	public function get_custom_content_plain() {
		...
		return apply_filters( 'omniverse_custom_text_content_' . $this->id, $this->format_string( stripcslashes( $this->content_text ) ), $this->object );
	}
	...
	public function init_form_fields() {
		$this->form_fields = array(
			'enabled'      => array(
				'title'   => esc_html__( 'Enable/Disable', 'omniverse' ),
				'type'    => 'checkbox',
				'label'   => esc_html__( 'Enable this email notification', 'omniverse' ),
				'default' => 'yes',
			),
			'subject'      => array(
				'title'       => esc_html__( 'Subject', 'omniverse' ),
				'type'        => 'text',
				// translators: 1. Default subject.
				'description' => sprintf( __( 'This field lets you modify the email subject line. Leave blank to use the default subject: <code>%s</code>.', 'omniverse' ), $this->subject ),
				'placeholder' => '',
				'default'     => '',
			),
			'heading'      => array(
				'title'       => esc_html__( 'Email Heading', 'omniverse' ),
				'type'        => 'text',
				// translators: 1. Default email heading.
				'description' => sprintf( __( 'This field lets you modify the main heading contained within the email notification. Leave blank to use the default heading: <code>%s</code>.', 'omniverse' ), $this->heading ),
				'placeholder' => '',
				'default'     => '',
			),
			'email_type'   => array(
				'title'       => esc_html__( 'Email type', 'omniverse' ),
				'type'        => 'select',
				'description' => esc_html__( 'Choose which type of email to send.', 'omniverse' ),
				'default'     => 'html',
				'class'       => 'email_type',
				'options'     => array(
					'plain'     => esc_html__( 'Plain text', 'omniverse' ),
					'html'      => esc_html__( 'HTML', 'omniverse' ),
					'multipart' => esc_html__( 'Multipart', 'omniverse' ),
				),
			),
			'coupon_code'  => array(
				'title'       => esc_html__( 'Coupon code', 'omniverse' ),
				'type'        => 'select',
				'description' => esc_html__( 'Choose which coupon code to send.', 'omniverse' ),
				'default'     => 'html',
				'options'     => array_merge(
					array(
						'' => '',
			...
			'content_html' => array(
				'title'       => esc_html__( 'Email HTML content', 'omniverse' ),
				'type'        => 'textarea',
				'description' => sprintf(
					esc_html__( 'This field lets you modify the main content of the HTML email. You can use the following placeholders: %s. Next placeholders you must use only within %s tags: %s', 'omniverse' ),
			...
			'content_text' => array(
				'title'       => esc_html__( 'Email text content', 'omniverse' ),
				'type'        => 'textarea',
				'description' => sprintf(
					esc_html__( 'This field lets you modify the main content of the text email. You can use the following placeholders: %s. Next placeholders you must use only within %s tags: %s', 'omniverse' ),
