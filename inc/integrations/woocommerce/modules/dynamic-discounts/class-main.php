...
class Main extends Singleton {
	...
	public function add_options() {
		Options::add_field(
			array(
				...
				'name'        => esc_html__( 'Enable "Dynamic discounts"', 'omniverse' ),
				'hint'        => wp_kses( __( '<img data-src="' . OMNIVERSE_TOOLTIP_URL . 'discounts-enabled.jpg" alt="">', 'omniverse' ), true ),
				'description' => esc_html__( 'You can configure your discounts in Dashboard -> Products -> Dynamic Discounts.', 'omniverse' ),
				'group'       => esc_html__( 'Dynamic discounts', 'omniverse' ),
				'type'        => 'switcher',
		...
		Options::add_field(
			array(
				...
				'name'        => esc_html__( 'Show discounts table', 'omniverse' ),
				'description' => esc_html__( 'Dynamic pricing table on the single product page.', 'omniverse' ),
				'group'       => esc_html__( 'Dynamic discounts', 'omniverse' ),
				'type'        => 'switcher',
				'section'     => 'shop_section',
				'default'     => '0',
	...
	public function include_files() {
		...
		foreach ( $files as $file ) {
			require_once get_parent_theme_file_path( OMNIVERSE_FRAMEWORK . '/integrations/woocommerce/modules/dynamic-discounts/' . $file . '.php' );
		}
	}
	...
	public function calculate_discounts( $cart ) {
		...
		foreach ( $cart->get_cart() as $cart_item ) {
			...
			$product_price = apply_filters( 'omniverse_pricing_before_calculate_discounts', (float) $product->get_price(), $cart_item );
			...
			switch ( $discount['_omniverse_rule_type'] ) {
				case 'bulk':
					foreach ( $discount['discount_rules'] as $key => $discount_rule ) {
						if ( $discount_rule['_omniverse_discount_rules_from'] <= $item_quantity && ( $item_quantity <= $discount_rule['_omniverse_discount_rules_to'] || ( array_key_last( $discount['discount_rules'] ) === $key && empty( $discount_rule['_omniverse_discount_rules_to'] ) ) ) ) {
							$discount_type  = $discount_rule['_omniverse_discount_type'];
							$discount_value = $discount_rule[ '_omniverse_discount_' . $discount_type . '_value' ];

							// @codeCoverageIgnoreStart
							// WPML woocommerce-multilingual compatibility.
							if ( function_exists( 'omniverse_wpml_shipping_progress_bar_amount' ) && 'amount' === $discount_type ) {
								$discount_value = omniverse_wpml_shipping_progress_bar_amount( $discount_value );
							}
							// @codeCoverageIgnoreEnd
			...
			$product_price = apply_filters( 'omniverse_pricing_after_calculate_discounts', $product_price, $cart_item );
