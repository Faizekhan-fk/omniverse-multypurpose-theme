...
use WC_Product;
use DN\Modules\Layouts\Main as Layouts;
use DN\Modules\Unit_Of_Measure\Main as Unit_Of_Measure;
use DN\Singleton;
...
class Frontend extends Singleton {
	...
	public function init() {
		...
		if ( omniverse_get_opt( 'show_discounts_table', 0 ) ) {
			add_action( 'woocommerce_single_product_summary', array( $this, 'render_dynamic_discounts_table' ), 25 );

			add_action( 'wp_ajax_omniverse_update_discount_dynamic_discounts_table', array( $this, 'update_dynamic_discounts_table' ) );
			add_action( 'wp_ajax_nopriv_omniverse_update_discount_dynamic_discounts_table', array( $this, 'update_dynamic_discounts_table' ) );
		}
	}
	...
	public function render_dynamic_discounts_table( $product_id = false, $wrapper_classes = '' ) {
		...
		if ( ! Manager::get_instance()->check_discount_exist( $product ) || ( ! Layouts::is_layout_type( 'single_product' ) && ! is_product() && ! is_ajax() ) || in_array( $product_type, array( 'grouped', 'external' ), true ) || 'bulk' !== $discount['_omniverse_rule_type'] ) {
			return false;
		}
		...
		if ( ! empty( $last_rules['_omniverse_discount_rules_to'] ) ) {
			$discount['discount_rules']['last'] = array(
				'_omniverse_discount_rules_from'       => $last_rules['_omniverse_discount_rules_to'] + 1,
				'_omniverse_discount_rules_to'         => '',
				'_omniverse_discount_type'             => 'amount',
				'_omniverse_discount_amount_value'     => 0,
		...
		foreach ( $discount['discount_rules'] as $id => $rules ) {
			// Quantity min.
			$data[ $id ]['min'] = $rules['_omniverse_discount_rules_from'];

			// Quantity max.
			$data[ $id ]['max'] = $rules['_omniverse_discount_rules_to'];

			// Quantity column.
			if ( $rules['_omniverse_discount_rules_from'] === $rules['_omniverse_discount_rules_to'] ) {
				$data[ $id ]['quantity'] = $rules['_omniverse_discount_rules_from'];
			} else {
				$data[ $id ]['quantity'] = sprintf(
					'%s%s%s',
					$rules['_omniverse_discount_rules_from'],
					array_key_last( $discount['discount_rules'] ) !== $id ? '-' : '',
					! empty( $rules['_omniverse_discount_rules_to'] ) ? $rules['_omniverse_discount_rules_to'] : '+'
				);
			}
			...
			$data[ $id ]['discount'] = sprintf(
				'%s%s',
				'amount' === $rules['_omniverse_discount_type'] ? apply_filters( 'omniverse_pricing_amount_discounts_value', $rules['_omniverse_discount_amount_value'] ) : $rules['_omniverse_discount_percentage_value'],
				'amount' === $rules['_omniverse_discount_type'] ? get_woocommerce_currency_symbol() : '%'
			);
			...
			$product_price = Main::get_instance()->get_product_price(
				$product->get_price(),
				array(
					'type'  => $rules['_omniverse_discount_type'],
					'value' => 'amount' === $rules['_omniverse_discount_type'] ? apply_filters( 'omniverse_pricing_amount_discounts_value', $rules['_omniverse_discount_amount_value'] ) : $rules['_omniverse_discount_percentage_value'],
				)
			);
		...
		omniverse_enqueue_inline_style( 'woo-opt-dynamic-discounts' );
		omniverse_enqueue_js_script( 'dynamic-discounts-table' );
		?>
		<?php if ( ! is_ajax() || ( Layouts::is_layout_type( 'single_product' ) && ( ! isset( $_REQUEST['action'] ) || 'omniverse_update_discount_dynamic_discounts_table' !== $_REQUEST['action'] ) ) ) : ?>
	...
	public function update_dynamic_discounts_table() {
		...
		wp_send_json(
			apply_filters( 'omniverse_variation_dynamic_discounts_table', $this->render_dynamic_discounts_table( $variation_id ) )
		);
	}
}
