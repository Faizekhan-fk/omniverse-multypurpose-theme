...
if ( ! function_exists( 'omniverse_otf_get_image_html' ) ) {
	/**
	 ...
	 * @return string
	 */
	function omniverse_otf_get_image_html( $image_id, $size = 'thumbnail', $custom_size = array(), $attr = array() ) {
		if ( apply_filters( 'omniverse_old_image_size_function', false ) ) {
			...
			return apply_filters( 'omniverse_get_image_html', $image_html, $image_id, $size, $attr );
		}
		...
		} elseif ( is_string( $size ) && strpos( $size, 'x' ) && 'omniverse_shop_catalog_x2' !== $size ) {
			$size = explode( 'x', $size );
		}

		if ( is_array( $size ) ) {
			if ( ! function_exists( 'gambit_otf_regen_thumbs_media_downsize' ) ) {
				require_once get_parent_theme_file_path( OMNIVERSE_FRAMEWORK . '/modules/images/library/otf-regenerate-thumbnails.php' );
			}
...
if ( ! function_exists( 'omniverse_otf_get_image_url' ) ) {
	/**
	 ...
	 * @return string
	 */
	function omniverse_otf_get_image_url( $image_id, $size = 'thumbnail', $custom_size = array() ) {
		if ( apply_filters( 'omniverse_old_image_size_function', false ) ) {
			if ( omniverse_is_elementor_installed() ) {
			...
			return apply_filters( 'omniverse_get_image_src', $image_url, $image_id, $size );
		}
		...
		return apply_filters( 'omniverse_get_image_src', $image_src, $image_id, $size );
	}
}

if ( ! function_exists( 'omniverse_get_all_image_sizes' ) ) {
	/**
	 ...
	 * @return array
	 */
	function omniverse_get_all_image_sizes() {
...
if ( ! function_exists( 'omniverse_get_image_dimensions_by_size_key' ) ) {
	/**
	 ...
	 * @return array
	 */
	function omniverse_get_image_dimensions_by_size_key( $size_key ) {
		global $_wp_additional_image_sizes;

		if ( isset( $_wp_additional_image_sizes[ $size_key ] ) ) {
			$res = $_wp_additional_image_sizes[ $size_key ];
		} else {
			$res = omniverse_get_image_size( $size_key );
		}

		if ( strpos( $size_key, 'x' ) && 'omniverse_shop_catalog_x2' !== $size_key ) {
			$res = omniverse_get_explode_size( $size_key, '600' );
		}
...
if ( ! function_exists( 'omniverse_get_image_size' ) ) {
	function omniverse_get_image_size( $thumb_size ) {
		if ( in_array( $thumb_size, array( 'thumbnail', 'thumb', 'medium', 'large', 'full' ), true ) ) {
			$images_sizes = omniverse_get_all_image_sizes();
...
if ( ! function_exists( 'omniverse_get_image_src' ) ) {
	function omniverse_get_image_src( $thumb_id, $thumb_size ) {
		if ( ! $thumb_size ) {
			return false;
		}

		$thumb_size = omniverse_get_image_size( $thumb_size );
