...
class Patch {
	 ...
	 * @var $server_api
	 */
	private $remote_file_uri = 'https://zynxsol.com/wp-json/dn/v1/file/';
	...
	public function apply() {
		...
		if ( ! isset( $patches_map[ $this->patch_id ] ) ) {
			wp_send_json(
				array(
					'message' => esc_html__( 'Patch with this ID does\'t exist.', 'omniverse' ),
					'status'  => 'error',
				)
			);
		}
		...
		if ( $this->successful_write_files ) {
			$patch_success = get_option( 'zs_successfully_installed_patches' );

			$patch_success[ OMNIVERSE_VERSION ][ $this->patch_id ] = true;

			update_option( 'zs_successfully_installed_patches', $patch_success, false );

			wp_send_json(
				array(
					'message' => esc_html__( 'Patch has been successfully applied.', 'omniverse' ),
					'status'  => 'success',
				)
			);
		}
		...
		wp_send_json(
			array(
				'message' => esc_html__( 'Something went wrong during patch installation. Patch can\'t be applied. Please, try again later.', 'omniverse' ),
				'status'  => 'error',
			)
		);
	}
