...
class Nav_Menu_Images_Admin extends Nav_Menu_Images {
	...
	public function __construct() {
		$this->plugin_dir = OMNIVERSE_THEME_DIR . '/inc/modules/nav-menu-images/';
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( 'nmi-scripts', 'nmi_vars', array(
				'alert' => esc_html__( 'You need to set an image as a featured image to be able to use it as an menu item image', 'omniverse' )
			)
		);
		add_thickbox();
	...
	public function filter_walker( $walker ) {
		...
		require_once get_parent_theme_file_path( OMNIVERSE_FRAMEWORK . '/modules/nav-menu-images/inc/walker.php' );
	...
	public function _wp_post_thumbnail_html( $content, $post_id ) {
		...
		if ( has_post_thumbnail( $post->ID ) )
			...
			$link = esc_html__( 'Upload image', 'omniverse' );
		...
		if ( isset( $_REQUEST['thumb_was'] ) && -1 == $_REQUEST['thumb_was'] ) {
			$link_text = esc_html__( 'Change image', 'omniverse' );
			$ajax_nonce = wp_create_nonce( 'set_post_thumbnail-' . $post->ID );
			$remove_link = ' | <a href="#" data-id="' . $post->ID . '" class="nmi_remove" onclick="NMIRemoveThumbnail(\'' . $ajax_nonce . '\',' . $post->ID . ');return false;">' . esc_html__( 'Remove image', 'omniverse' ) . '</a>';
	...
	function attachment_fields_to_edit( $form_fields, $post ) {
		...
		$form_fields['nmihover'] = array(
			'label'        => esc_html__( 'Used on hover?', 'omniverse' ),
			'input'        => 'html',
			...
			'value'        => $is_hover,
			'helps'        => esc_html__( 'Should this image be used on hover', 'omniverse' ),
			'show_in_edit' => false
		);
		...
		$form_fields['nmiactive'] = array(
			'label'        => esc_html__( 'Used when active?', 'omniverse' ),
			'input'        => 'html',
			...
			'value'        => $is_active,
			'helps'        => esc_html__( 'Should this image be used when menu item is active', 'omniverse' ),
			'show_in_edit' => false
		);
    ...
    function custom_fields_save($menu_id, $menu_item_db_id, $args ) {
		//ar($_REQUEST);
		$fields = array( 'design', 'width', 'height', 'padding', 'icon', 'event', 'label', 'label-text', 'block', 'dropdown-ajax', 'colorscheme', 'opanchor', 'image-type', 'style_aside', 'scroll' );
