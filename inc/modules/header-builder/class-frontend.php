...
class Frontend extends Singleton {
	...
	public function get_current_id() {
		...
		$default_header          = omniverse_get_opt( 'default_header' );
		$custom_post_header      = omniverse_get_opt( 'single_post_header' );
		$custom_portfolio_header = omniverse_get_opt( 'single_portfolio_header' );
		$custom_product_header   = omniverse_get_opt( 'single_product_header' );
		$custom                  = get_post_meta( $page_id, '_omniverse_whb_header', true );
		...
		return apply_filters( 'omniverse_get_current_header_id', $id );
	}
	...
	private function render_element( $el ) {
		...
		if ( isset( $el['content'] ) && is_array( $el['content'] ) ) {
			if ( wp_is_mobile() && omniverse_get_opt( 'mobile_optimization', 0 ) && isset( $el['desktop_only'] ) ) {
				return;
			}
...
$GLOBALS['omniverse_hb_frontend'] = Frontend::get_instance();
