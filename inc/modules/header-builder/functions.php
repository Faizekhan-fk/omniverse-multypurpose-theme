...
use DN\Modules\Header_Builder\Frontend;
...
if ( ! function_exists( 'whb_generate_header' ) ) {
	...
	function whb_generate_header() {
		omniverse_enqueue_inline_style( 'header-base' );
...
if ( ! function_exists('omniverse_get_whb_headers_array' ) ) {
	function omniverse_get_whb_headers_array( $get_from_options = false, $new = false ) {
		if ( $get_from_options ) {
...
if ( ! function_exists( 'omniverse_get_theme_settings_headers_array' ) ) {
	/**
	 * Function to get array of HTML Blocks in theme settings array style.
	 *
	 * @return array
	 */
	function omniverse_get_theme_settings_headers_array() {
		$list = get_option( 'whb_saved_headers' );
...
if ( ! function_exists( 'omniverse_get_header_classes' ) ) {
	/**
	 * Header classes.
	 *
	 * @return void
	 */
	function omniverse_get_header_classes() {
		$settings              = whb_get_settings();
		$custom_product_header = omniverse_get_opt( 'single_product_header' );
		...
		omniverse_enqueue_js_script( 'header-builder' );

		if ( ! empty( $custom_product_header ) && 'none' !== $custom_product_header && omniverse_woocommerce_installed() && is_product() ) {
...
if ( ! function_exists( 'omniverse_set_default_header' ) ) {
	/**
	 * Setup default header from theme settings
	 *
	 * @since 1.0.0
	 */
	function omniverse_set_default_header() {
		if ( ! isset( $_GET['settings-updated'] ) || isset( $_GET['preset'] ) ) { // phpcs:ignore
			return;
		}

		$theme_settings_header_id = omniverse_get_opt( 'default_header' );
	...
	add_filter( 'init', 'omniverse_set_default_header', 1000 );
}

if ( ! function_exists( 'omniverse_get_header_body_classes' ) ) {
	/**
	 * Get header body classes.
	 *
	 * @return array
	 */
	function omniverse_get_header_body_classes() {
		$classes  = array();
		$settings = whb_get_settings();

		if ( isset( $settings['overlap'] ) && $settings['overlap'] ) {
			$classes[] = 'wd-header-overlap';
			$classes[] = omniverse_get_old_classes( 'omniverse-header-overcontent' );
		}
