...
abstract class Element {
	...
	public function visual_composer_to_header() {
		$vc_element = 'omniverse_get_' . $this->vc_element . '_shortcode_args';
		$vc_args    = $vc_element();
	...
	private function _get_params_from_vc( $params ) {
		...
		foreach ( $params as $key => $param ) {
			if ( ! $param || $param['type'] == 'omniverse_title_divider' || $param['type'] == 'omniverse_css_id' ) {
				continue;
			}
	...
	private function _get_param_from_vc( $param ) {
		...
		$new_param['value'] = '';
		$dropdown_params    = array( 'dropdown', 'omniverse_dropdown', 'omniverse_button_set', 'omniverse_image_select' );
	...
	private function _get_type_from_vc( $type ) {
		switch ( $type ) {
			case 'omniverse_button':
				return 'button';
				break;

			case 'omniverse_info_box':
				return 'infobox';
				break;
	...
	public function get_menu_options( $empty = false ) {
		...
		if ( $empty ) {
			$array[''] = array(
				'label' => esc_html__( 'Select', 'omniverse' ),
				'value' => '',
			);
		}
	...
	public function get_html_block_options() {
		...
		foreach ( $blocks_posts as $post ) {
			$array[ $post->ID ] = array(
				'label' => $post->post_title ? $post->post_title : esc_html__( '(no title)', 'omniverse' ),
				'value' => $post->ID,
			);
		}
		return $array;
