...
class Manager {
	...
	public function __construct( $factory, $list ) {
		...
		add_action( 'wp_ajax_omniverse_save_header', array( $this, 'save_header' ) );
		add_action( 'wp_ajax_omniverse_duplicate_header', array( $this, 'duplicate_header' ) );
		add_action( 'wp_ajax_omniverse_load_header', array( $this, 'load_header' ) );
		add_action( 'wp_ajax_omniverse_remove_header', array( $this, 'remove_header' ) );
		add_action( 'wp_ajax_omniverse_set_default_header', array( $this, 'set_default_header' ) );
	}
	...
	public function duplicate_header() {
		check_ajax_referer( 'omniverse-builder-save-header-nonce', 'security' );
		...
		$new_name = $name . ' ' . esc_html__( '(Copy)', 'omniverse' );
	...
	public function load_header() {
		check_ajax_referer( 'omniverse-builder-load-header-nonce', 'security' );
	...
	private function new_header( $base = false ) {
		...
		$id   = $this->generate_id();
		$name = esc_html__( 'Header layout', 'omniverse' ) . ' (' . ( count( $list ) + 1 ) . ')';
	...
	private function get_example_json( $file ) {
		...
		include OMNIVERSE_THEMEROOT . '/inc/modules/header-builder/examples/' . $file . '.json';
	...
	public function remove_header() {
		check_ajax_referer( 'omniverse-builder-remove-header-nonce', 'security' );

		$id = sanitize_text_field( stripslashes( $_GET['id'] ) ); //phpcs:ignore

		delete_option( 'whb_' . $id );
		delete_option( 'dn-' . $id . '-file-data' );
		delete_option( 'dn-' . $id . '-css-data' );
		delete_option( 'dn-' . $id . '-version' );
		delete_option( 'dn-' . $id . '-site-url' );
		delete_option( 'dn-' . $id . '-status' );
	...
	public function set_default_header() {
		check_ajax_referer( 'omniverse-builder-set-default-header-nonce', 'security' );
		...
		update_option( 'dn-omniverse-options', $options );
