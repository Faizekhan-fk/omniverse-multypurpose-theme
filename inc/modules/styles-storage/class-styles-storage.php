...
class Styles_Storage {
	...
	public function __construct( $data_name, $storage = 'option', $id = '', $check_credentials = true ) {
		...
		$this->current_theme_version = omniverse_get_theme_info( 'Version' );
		$this->current_site_url      = get_site_url();

		$this->set_data( 'dn-' . $this->data_name . '-file-data' );
		$this->set_css_data( 'dn-' . $this->data_name . '-css-data' );

		$this->css_version = $this->get_data( 'dn-' . $this->data_name . '-version' );
		$this->site_url    = $this->get_data( 'dn-' . $this->data_name . '-site-url' );
	...
	public function check_css_status() {
		$data_status = $this->get_data( 'dn-' . $this->data_name . '-status' );

		if ( 'valid' === $data_status ) {
			if ( isset( $this->data['path'] ) && file_exists( $this->get_file_path( $this->data['path'] ) ) && apply_filters( 'omniverse_styles_storage_file', true ) && isset( $this->data['theme_version'] ) && version_compare( $this->current_theme_version, $this->data['theme_version'], '==' ) && ( isset( $this->data['site_url'] ) && md5( get_site_url() ) === $this->data['site_url'] ) ) {
				$this->is_file_exists = true;
			}

			if ( $this->css && apply_filters( 'omniverse_styles_storage_db_css', true ) && version_compare( $this->current_theme_version, $this->css_version, '==' ) && md5( get_site_url() ) === $this->site_url ) {
	...
	public function print_styles_inline() {
		if ( $this->is_file_exists ) {
			?>
			<link rel="stylesheet" id="dn-style-<?php echo esc_attr( $this->data_name ); ?>-css" href="<?php echo esc_attr( $this->get_file_url( $this->data['path'] ) ); ?>?ver=<?php echo esc_attr( $this->current_theme_version ); ?>" type="text/css" media="all">  <?php // phpcs:ignore ?>
			<?php
		} else {
			$this->inline_css();
		}
	...
	public function file_css() {
		if ( isset( $this->data['path'] ) && $this->data['path'] ) {
			wp_enqueue_style( 'dn-style-' . $this->data_name, $this->get_file_url( $this->data['path'] ), array(), $this->current_theme_version );
		}
	}
	...
	public function reset_data() {
		$this->update_data( 'dn-' . $this->data_name . '-status', 'invalid' );
		$this->delete_data( 'dn-' . $this->data_name . '-credentials' );
	}
	...
	public function write( $css, $is_frontend = false ) {
		...
		$this->update_data( 'dn-' . $this->data_name . '-status', 'valid' );
		$this->update_data( 'dn-' . $this->data_name . '-version', omniverse_get_theme_info( 'Version' ) );
		$this->update_data( 'dn-' . $this->data_name . '-site-url', md5( get_site_url() ) );
	}
	...
	public function delete_file() {
		...
		if ( $this->data && ! empty( $this->data['path'] ) ) {
			...
			$this->delete_data( 'dn-' . $this->data_name . '-file-data' );
		}
	}
	...
	private function write_file( $css ) {
		 ...
		 * File system
		 *
		 * @var \DN\WP_Filesystem_Base $wp_filesystem
		 */
		global $wp_filesystem;
	...
	public function get_file_info( $data_name ) {
		$uploads = wp_upload_dir();
		return set_url_scheme( $uploads['subdir'] . '/' . 'dn-' . $data_name . '-' . time() . '.css' );
	}
	...
	public function check_credentials() {
		$data_status        = $this->get_data( 'dn-' . $this->data_name . '-status' );
		$credentials_status = $this->get_data( 'dn-' . $this->data_name . '-credentials' );

		if ( ( 'valid' === $data_status || 'requested' === $credentials_status ) && ! $_POST ) {
			return false;
		}

		$this->update_data( 'dn-' . $this->data_name . '-credentials', 'requested' );

		echo '<div class="omniverse-request-credentials">';
