...
class Parts_Css_Files extends Singleton {
	...
	public function init() {
		$this->theme_version = omniverse_get_theme_info( 'Version' );
		$this->is_mobile     = wp_is_mobile() && omniverse_get_opt( 'mobile_optimization', 0 );
		...
		add_action( 'save_post_omniverse_slider', array( $this, 'delete_all_meta' ), 10 );
		add_action( 'zs_theme_settings_save', array( $this, 'delete_all_meta' ), 10 );
		...
		add_action( 'woocommerce_single_product_summary', 'omniverse_page_css_files_disable', 59 );
		add_action( 'woocommerce_single_product_summary', 'omniverse_page_css_files_enable', 61 );
	}
	...
	private function get_page_data() {
		...
		if ( omniverse_woocommerce_installed() && $queried_object && ( is_product_tag() || is_product_category() || omniverse_is_product_attribute_archive() ) ) {
			$data = array(
				'type' => 'taxonomy',
				'id'   => $queried_object->term_id,
			);
		...
		if ( is_archive() && 'portfolio' === get_post_type() ) {
			$data = array(
				'type' => 'post',
				'id'   => omniverse_get_portfolio_page_id(),
			);
		}
	...
	private function get_page_css_files() {
		...
		if ( 'post' === $data['type'] ) {
			if ( $this->is_mobile && get_post_meta( $data['id'], '_omniverse_mobile_content', true ) ) {
	...
	public function enqueue_page_css_files() {
		$config     = omniverse_get_config( 'css-files' );
		$version    = omniverse_get_theme_info( 'Version' );
		$page_files = $this->page_css_files;
		$localize   = array();

		if ( omniverse_is_combined_needed( 'combined_css' ) || ! $page_files ) {
			wp_localize_script( 'omniverse-theme', 'omniverse_page_css', array() );
		...
		foreach ( $page_files as $slug ) {
			...
			foreach ( $config[ $slug ] as $file ) {
				if ( isset( $file['wpb_file'] ) && 'wpb' === omniverse_get_current_page_builder() ) {
				...
				$src = OMNIVERSE_THEME_DIR . $file['file'] . '.min.css';

				$localize[ 'wd-' . $file['name'] . '-css' ] = $src;

				wp_enqueue_style( 'wd-' . $file['name'], $src, array( 'omniverse-style' ), $version, $media );
			}
		}

		wp_localize_script( 'omniverse-theme', 'omniverse_page_css', $localize );
	}
	...
	public function enqueue_style( $key, $ignore_combined = false ) {
		$config         = omniverse_get_config( 'css-files' );
		$version        = omniverse_get_theme_info( 'Version' );
		$styles_not_use = omniverse_get_opt( 'styles_not_use' );

		if ( omniverse_is_combined_needed( 'combined_css' ) && ! $ignore_combined || ! isset( $config[ $key ] ) ) {
			return;
		}
		...
		foreach ( $config[ $key ] as $file ) {
			...
			wp_enqueue_style( 'wd-' . $file['name'], $src, array( 'omniverse-style' ), $version );
		}
	}
	...
	public function enqueue_inline_style( $key, $ignore_combined = false ) {
		...
		if ( ! isset( $config[ $key ] ) || in_array( $key, $page_files, true ) || ( omniverse_is_combined_needed( 'combined_css' ) && ! $ignore_combined ) || isset( $GLOBALS['wd_page_css_ignore'] ) ) {
			return;
		}
		...
		foreach ( $config[ $key ] as $data ) {
			...
			if ( isset( $data['wpb_file'] ) && 'wpb' === omniverse_get_current_page_builder() ) {
			...
			$src = OMNIVERSE_THEME_DIR . $data['file'] . '.min.css';
