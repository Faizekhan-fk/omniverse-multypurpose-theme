...
class Mega_Menu_Walker extends Walker_Nav_Menu {
	...
	/**
	 * OMNIVERSE_Mega_Menu_Walker constructor.
	 */
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		if ( 0 === $depth && ! $is_nav_mobile ) {
			...
			$classes .= omniverse_get_old_classes( ' sub-menu-dropdown' );
		...
		if ( 0 === $depth ) {
			if ( ( 'full-width' === $this->design || 'sized' === $this->design || 'full-height' === $this->design ) && ! $is_nav_mobile ) {
				...
				$sub_menu_class .= omniverse_get_old_classes( ' sub-menu' );
			} else {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( 'aside' === $design ) {
			omniverse_enqueue_inline_style( 'dropdown-aside' );
		}

		if ( 'full-height' === $design ) {
			omniverse_enqueue_inline_style( 'dropdown-full-height' );
		}

		if ( 'full-height' === $design || 'yes' === $scroll && ( 'full-width' === $design || 'sized' === $design ) ) {
			omniverse_enqueue_inline_style( 'header-mod-content-calc' );
		}
		...
		if ( 0 === $depth && ! $is_nav_mobile ) {
			$classes[] = omniverse_get_old_classes( 'menu-item-design-' . $design );
		...
		if ( 'enable' === $opanchor ) {
			...
			omniverse_enqueue_js_script( 'one-page-menu' );
		...
		if ( ! empty( $label ) ) {
			omniverse_enqueue_inline_style( 'mod-nav-menu-label' );
		...
		omniverse_enqueue_js_script( 'menu-offsets' );
		omniverse_enqueue_js_script( 'menu-setup' );
		...
		if ( 'yes' === $dropdown_ajax ) {
			omniverse_enqueue_js_script( 'menu-dropdowns-ajax' );
		...
		if ( 'product_cat' === $item->object || ( 'image' === $image_type && has_post_thumbnail( $item->ID ) ) ) {
			...
			$icon_attrs = apply_filters( 'omniverse_megamenu_icon_attrs', false );

			if ( $icon_data ) {
				if ( is_array( $icon_data ) && $icon_data['id'] ) {
					if ( omniverse_is_svg( $icon_data['url'] ) ) {
						$image_output .= omniverse_get_svg_html( $icon_data['id'], apply_filters( 'omniverse_mega_menu_icon_size_svg', '18x18' ), array( 'class' => 'wd-nav-img' ) );
					} else {
						$image_output .= wp_get_attachment_image( $icon_data['id'], apply_filters( 'omniverse_mega_menu_icon_size', 'thumbnail' ), false, array( 'class' => 'wd-nav-img' ) );
					}
				} else {
					...
					if ( $icon_data ) {
						$image_output .= '<img src="' . esc_url( $icon_data ) . '" alt="' . esc_attr( $item->title ) . '" ' . $icon_attrs . ' class="wd-nav-img' . omniverse_get_old_classes( ' category-icon' ) . '" />';
					}
				}
			}
		}
		...
		if ( ! $is_nav_mobile ) {
			if ( $block && ! $args->walker->has_children ) {
				...
				if ( 'yes' === $dropdown_ajax ) {
					...
					$item_output .= omniverse_html_block_shortcode( array( 'id' => $block ) );
				}
				$item_output .= "\n$indent</div>\n";
