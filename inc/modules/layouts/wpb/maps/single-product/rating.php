...
if ( ! function_exists( 'omniverse_get_vc_map_single_product_rating' ) ) {
	/**
	 * Rating map.
	 */
	function omniverse_get_vc_map_single_product_rating() {
		return array(
			'base'        => 'omniverse_single_product_rating',
			'name'        => esc_html__( 'Product rating', 'omniverse' ),
			'category'    => omniverse_get_tab_title_category_for_wpb( esc_html__( 'Single product elements', 'omniverse' ), 'single_product' ),
			'description' => esc_html__( 'Star rating and reviews number', 'omniverse' ),
			'icon'        => OMNIVERSE_ASSETS . '/images/vc-icon/sp-icons/sp-rating.svg',
			'params'      => array(
				array(
