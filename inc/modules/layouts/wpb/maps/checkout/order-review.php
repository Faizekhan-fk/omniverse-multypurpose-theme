...
if ( ! function_exists( 'omniverse_get_vc_map_checkout_order_review' ) ) {
	/**
	 * Order review map.
	 */
	function omniverse_get_vc_map_checkout_order_review() {
		return array(
			'base'        => 'omniverse_checkout_order_review',
			'name'        => esc_html__( 'Order review', 'omniverse' ),
			'category'    => omniverse_get_tab_title_category_for_wpb( esc_html__( 'Checkout', 'omniverse' ), 'checkout_form' ),
			'description' => esc_html__( 'Order review table with totals', 'omniverse' ),
			'icon'        => OMNIVERSE_ASSETS . '/images/vc-icon/ch-icons/ch-order-review.svg',
			'params'      => array(
				array(
