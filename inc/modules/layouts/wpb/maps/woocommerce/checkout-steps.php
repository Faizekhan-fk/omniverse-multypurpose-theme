...
if ( ! function_exists( 'omniverse_get_vc_map_checkout_steps' ) ) {
	/**
	 * Checkout steps map.
	 */
	function omniverse_get_vc_map_checkout_steps() {
		return array(
			'base'        => 'omniverse_woocommerce_checkout_steps',
			'name'        => esc_html__( 'Checkout steps', 'omniverse' ),
			'category'    => omniverse_get_tab_title_category_for_wpb( esc_html__( 'WooCommerce', 'omniverse' ) ),
			'description' => esc_html__( 'Cart -> Checkout -> Complete', 'omniverse' ),
			'icon'        => OMNIVERSE_ASSETS . '/images/vc-icon/wc-icons/wc-checkout-steps.svg',
			'params'      => array(
				array(
