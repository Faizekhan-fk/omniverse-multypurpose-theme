...
if ( ! function_exists( 'omniverse_get_vc_map_shop_archive_filters_area' ) ) {
	/**
	 * Filters area map.
	 */
	function omniverse_get_vc_map_shop_archive_filters_area() {
		return array(
			'base'        => 'omniverse_shop_archive_filters_area',
			'name'        => esc_html__( 'Filters area', 'omniverse' ),
			'category'    => omniverse_get_tab_title_category_for_wpb( esc_html__( 'Products archive', 'omniverse' ), 'shop_archive' ),
			'description' => esc_html__( 'Special widget area for filters', 'omniverse' ),
			'icon'        => OMNIVERSE_ASSETS . '/images/vc-icon/sa-icons/sa-filter-area.svg',
			'params'      => array(
				array(
