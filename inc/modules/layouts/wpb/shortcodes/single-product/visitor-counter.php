...
use DN\Modules\Visitor_Counter\Main as Counter_Visitors;

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Direct access not allowed.
}

if ( ! function_exists( 'omniverse_shortcode_single_product_visitor_counter' ) ) {
	/**
	 * Single product visitor counter shortcode.
	 *
	 * @param array $settings Shortcode attributes.
	 */
	function omniverse_shortcode_single_product_visitor_counter( $settings ) {
		...
		if ( 'icon' === $settings['icon_type'] ) {
			...
			if ( omniverse_is_svg( wp_get_attachment_image_url( $settings['image'] ) ) ) {
				$icon_output = omniverse_get_svg_html(
					$settings['image'],
					$settings['img_size']
				);
			} else {
				$icon_output = omniverse_otf_get_image_html( $settings['image'], $settings['img_size'] );
			}
		...
		Counter_Visitors::get_instance()->output_count_visitors( $wrapper_classes, $icon_html );
