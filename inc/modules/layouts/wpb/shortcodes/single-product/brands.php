...
if ( ! function_exists( 'omniverse_shortcode_single_product_brands' ) ) {
	/**
	 * Product brands shortcode.
	 *
	 * @param array $settings Shortcode attributes.
	 */
	function omniverse_shortcode_single_product_brands( $settings ) {
		$default_settings = array(
			'alignment'  => 'left',
			'css'        => '',
			'label_text' => esc_html__( 'Brands: ', 'omniverse' ),
			'layout'     => 'default',
			'style'      => 'default',
			'show_label' => 'no',
		);
		...
		if ( ! $attr || ( ! omniverse_get_opt( 'product_page_brand' ) && ! Main::get_instance()->has_custom_layout( 'single_product' ) ) ) {
			return '';
		}
