...
class Manager extends Singleton {
	...
	public function add_actions() {
		...
		add_action( 'omniverse_after_import', array( $this, 'save_post' ) );
	...
	public function edit_layout() {
		...
		$post_id = omniverse_clean( $_POST['id'] ); // phpcs:ignore
		$data    = isset( $_POST['data'] ) ? omniverse_clean( $_POST['data'] ) : array(); // phpcs:ignore
	...
	public function create_layout() {
		...
		$type            = omniverse_clean( $_POST['type'] ); // phpcs:ignore
		$title           = omniverse_clean( $_POST['name'] ); // phpcs:ignore
		$data            = isset( $_POST['data'] ) ? omniverse_clean( $_POST['data'] ) : array(); // phpcs:ignore
		$predefined_name = isset( $_POST['predefined_name'] ) ? omniverse_clean( $_POST['predefined_name'] ) : ''; // phpcs:ignore
		...
		$url = add_query_arg(
			array(
				'post'           => $post_id,
				'action'         => 'wpb' === omniverse_get_current_page_builder() ? 'edit' : 'elementor',
				'classic-editor' => '',
			),
			admin_url( 'post.php' )
		);
		...
		switch ( $query_type ) {
			...
			case 'filtered_product_stock_status':
				$items = array(
					array(
						'id'   => 'instock',
						'text' => esc_html__( 'In Stock', 'omniverse' ),
					),
					array(
						'id'   => 'onsale',
						'text' => esc_html__( 'On Sale', 'omniverse' ),
					),
					array(
						'id'   => 'onbackorder',
						'text' => esc_html__( 'On backorder', 'omniverse' ),
					),
				);
				break;
			case 'product_type':
	...
	public function duplicate_action( $actions, $post ) {
		if ( 'omniverse_layout' !== $post->post_type ) {
			return $actions;
		}
