...
class Main extends Singleton {
	...
	public function init() {
		define( 'ZS_LAYOUTS_DIR', OMNIVERSE_THEMEROOT . '/inc/modules/layouts/' );
		define( 'ZS_LAYOUTS_TEMPLATES_DIR', '/inc/modules/layouts/templates/' );
	...
	public function include_classes_files() {
		require_once ZS_LAYOUTS_DIR . 'admin/class-admin.php';
		require_once ZS_LAYOUTS_DIR . 'admin/class-conditions-cache.php';
		require_once ZS_LAYOUTS_DIR . 'admin/class-manager.php';
		require_once ZS_LAYOUTS_DIR . 'admin/class-import.php';
		require_once ZS_LAYOUTS_DIR . 'class-layout-type.php';
		require_once ZS_LAYOUTS_DIR . 'class-checkout.php';
		require_once ZS_LAYOUTS_DIR . 'class-cart.php';
		require_once ZS_LAYOUTS_DIR . 'class-shop-archive.php';
		require_once ZS_LAYOUTS_DIR . 'class-single-product.php';
	}
	...
	public function include_files() {
		$directory       = '';
		$current_builder = omniverse_get_current_page_builder();

		if ( 'wpb' === $current_builder ) {
			foreach ( glob( ZS_LAYOUTS_DIR . 'wpb/**/**/*.php', GLOB_NOSORT ) as $file ) {
				require_once $file;
			}

			require_once ZS_LAYOUTS_DIR . 'wpb/maps/register-maps.php';
	...
	public function register_layout_widgets() {
		foreach ( glob( ZS_LAYOUTS_DIR . 'elementor/**/*.php', GLOB_NOSORT ) as $file ) {
			require_once $file;
		}
	}
	...
	public function register_elementor_categories( $elements_manager ) {
		$elements_manager->add_category(
			...
			array(
				'title' => esc_html__( '[Zynxsol] WooCommerce', 'omniverse' ),
				'icon'  => 'fab fa-plug',
			)
		);
		...
		$elements_manager->add_category(
			...
			array(
				'title' => esc_html__( '[Zynxsol] Single product', 'omniverse' ),
				'icon'  => 'fab fa-plug',
			)
		);
		...
		$elements_manager->add_category(
			...
			array(
				'title' => esc_html__( '[Zynxsol] Products archive', 'omniverse' ),
				'icon'  => 'fab fa-plug',
			)
		);
		...
		$elements_manager->add_category(
			'wd-cart-elements',
			array(
				'title' => esc_html__( '[Zynxsol] Cart', 'omniverse' ),
				'icon'  => 'fab fa-plug',
			)
		);
		...
		$elements_manager->add_category(
			...
			array(
				'title' => esc_html__( '[Zynxsol] Checkout', 'omniverse' ),
				'icon'  => 'fab fa-plug',
			)
		);
	}
	...
	public function has_custom_layout( $type ) {
		do_action( 'omniverse_get_layout_id', $this );
	...
	public function get_layout_id( $type ) {
		if ( isset( $this->layouts_cache[ $type ] ) ) {
			return apply_filters( 'omniverse_layout_id', $this->layouts_cache[ $type ] );
		}
		...
		foreach ( $current_conditions_data as $post_id => $conditions ) {
			$post_id = apply_filters( 'wpml_object_id', $post_id, 'omniverse_layout' );
	...
	private function get_condition_priority( $type ) {
		...
		return apply_filters( 'omniverse_layouts_condition_priority', $priority, $type );
	}
	...
	public static function setup_preview( $shop_args = array(), $force_product_id = false ) {
		...
		if ( 'single_product' === $layout_type || ( wp_doing_ajax() && isset( $_POST['action'] ) && 'wd_layout_create' === $_POST['action'] ) || $force_product_id ) { // phpcs:ignore
			...
			if ( ! $shop_args ) {
				$shop_args = array(
					'post_type'      => 'product',
					'posts_per_page' => omniverse_get_products_per_page(),
				);
			}
	...
	public static function is_layout_type( $type ) {
		...
		if ( Builder_Data::get_instance()->get_data( 'layout_id' ) ) {
			...
			if ( isset( $_GET['post'] ) && $_GET['post'] ) { //phpcs:ignore
				$layout_id = omniverse_clean( $_GET['post'] ); //phpcs:ignore
			} elseif ( isset( $_REQUEST['post_id'] ) && $_REQUEST['post_id'] ) { //phpcs:ignore
				$layout_id = omniverse_clean( $_REQUEST['post_id'] ); //phpcs:ignore
			} elseif ( isset( $_POST['post_ID'] ) && $_POST['post_ID'] ) { //phpcs:ignore
				$layout_id = omniverse_clean( $_POST['post_ID'] ); //phpcs:ignore
			}
		} elseif ( function_exists( 'vc_is_inline' ) && vc_is_inline() ) {
			$layout_id = (int) vc_get_param( 'vc_post_id' );
		} elseif ( 'post-new.php' === $GLOBALS['pagenow'] && ( ! isset( $_REQUEST['post_type'] ) || 'omniverse_layout' !== $_REQUEST['post_type'] ) ) {
			return false;
		} elseif ( is_admin() ) {
			return true;
		} else {
