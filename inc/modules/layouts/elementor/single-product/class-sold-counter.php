...
use DN\Modules\Sold_Counter\Main as Sold_Counter_Module;
 ...
 */
class Sold_Counter extends Widget_Base {
	/**
	 * Get widget name.
	 *
	...
	public function get_title() {
		return esc_html__( 'Product sold counter', 'omniverse' );
	}
	...
	public function get_icon() {
		return 'wd-icon-sp-sold-counter';
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'type'    => Controls_Manager::SELECT,
				'options' => array(
					'default' => esc_html__( 'Default icon', 'omniverse' ),
					'icon'    => esc_html__( 'Custom icon', 'omniverse' ),
					'image'   => esc_html__( 'Custom image', 'omniverse' ),
				),
				'default' => 'default',
			)
		);
	...
	protected function render() {
		...
		if ( 'image' === $settings['icon_type'] && isset( $settings['image']['id'] ) && $settings['image']['id'] ) {
			...
			$icon_output = omniverse_elementor_get_render_icon( $settings['icon'], array( 'class' => 'wd-count-icon' ), 'span' );
		...
		Sold_Counter_Module::get_instance()->render( $wrapper_classes, $icon_output );

		Main::restore_preview();
	}
}

Plugin::instance()->widgets_manager->register( new Sold_Counter() );
