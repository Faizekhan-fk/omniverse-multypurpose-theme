...
use DN\WC_Wishlist\Ui as Wishlist;
use DN\Modules\Compare\Ui as Compare;
use DN\Modules\Linked_Variations\Frontend as Linked_Variations;
use DN\Modules\Shipping_Progress_Bar\Main as Shipping_Progress_Bar;
...
class Hook extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'WooCommerce Hook', 'omniverse' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'hook',
			array(
				'label'       => esc_html__( 'Hook', 'omniverse' ),
				'description' => esc_html__( 'Select which PHP hook do you want to display here.', 'omniverse' ),
		...
		$this->add_control(
			'clean_actions',
			array(
				'label'        => esc_html__( 'Clean actions', 'omniverse' ),
				'description'  => esc_html__( 'You can clean all default WooCommerce PHP functions hooked to this action.', 'omniverse' ),
	...
	protected function render() {
		...
		if ( 'yes' === $settings['clean_actions'] ) {
			if ( 'woocommerce_checkout_billing' === $settings['hook'] ) {
				...
				remove_action( 'woocommerce_before_single_product', 'omniverse_product_extra_content', 20 );
				...
				remove_action( 'woocommerce_single_product_summary', 'omniverse_product_brand', 3 );
				remove_action( 'woocommerce_single_product_summary', 'omniverse_product_brand', 8 );
				remove_action( 'woocommerce_single_product_summary', 'omniverse_product_share_buttons', 62 );
				remove_action( 'woocommerce_single_product_summary', 'omniverse_display_product_attributes', 21 );
				remove_action( 'woocommerce_single_product_summary', 'woocommerce_template_loop_add_to_cart', 30 );
				remove_action( 'woocommerce_single_product_summary', 'omniverse_sguide_display', 38 );
				...
				if ( omniverse_get_opt( 'linked_variations' ) ) {
				...
				remove_action( 'woocommerce_before_add_to_cart_form', 'omniverse_single_product_add_to_cart_scripts' );
				...
				remove_action( 'woocommerce_after_single_product_summary', 'omniverse_wc_comments_template', 50 );
			} elseif ( 'woocommerce_checkout_order_review' === $settings['hook'] ) {
				remove_action( 'woocommerce_checkout_order_review', 'omniverse_open_table_wrapper_div', 7 );
				remove_action( 'woocommerce_checkout_order_review', 'omniverse_close_table_wrapper_div', 13 );
