...
class Shop_Archive extends Layout_Type {
	...
	public function check( $condition, $type = '' ) {
		...
		switch ( $condition['condition_type'] ) {
			case 'all':
				$is_active = is_shop() || is_product_category() || is_product_tag() || omniverse_is_product_attribute_archive();
				break;
			case 'shop_page':
				$is_active = is_shop();
				break;
	...
	public function override_template( $template ) {
		if ( omniverse_woocommerce_installed() && ( is_shop() || is_product_taxonomy() ) && Main::get_instance()->has_custom_layout( 'shop_archive' ) && ( ! function_exists( 'wcfm_is_store_page' ) || ! wcfm_is_store_page() ) ) {
	...
	public function before_template_content() {
		...
		omniverse_enqueue_inline_style( 'woo-shop-builder' );
	}
	...
	public static function switch_to_preview_query( $new_query ) {
		if ( ! is_singular( 'omniverse_layout' ) && ! wp_doing_ajax() ) {
			return;
		}
