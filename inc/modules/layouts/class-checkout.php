...
class Checkout extends Layout_Type {
	...
	public function check( $condition, $type = '' ) {
		...
		if ( 'checkout_form' === $type ) {
			switch ( $condition['condition_type'] ) {
				case 'checkout_form':
					$is_active = ( is_checkout() && ! is_order_received_page() && ! is_wc_endpoint_url( 'order-pay' ) ) || ( is_singular( 'omniverse_layout' ) && Main::is_layout_type( 'checkout_content' ) );
					break;
			}
		} elseif ( 'checkout_content' === $type ) {
			switch ( $condition['condition_type'] ) {
				case 'checkout_content':
					$is_active = ( is_checkout() && ! is_order_received_page() && ! is_wc_endpoint_url( 'order-pay' ) ) || ( is_singular( 'omniverse_layout' ) && Main::is_layout_type( 'checkout_form' ) );
					break;
			}
		}
	...
	public function override_template( $template ) {
		if ( omniverse_woocommerce_installed() && is_checkout() && ! is_order_received_page() && ! is_wc_endpoint_url( 'order-pay' ) && ( Main::get_instance()->has_custom_layout( 'checkout_content' ) || Main::get_instance()->has_custom_layout( 'checkout_form' ) ) ) {
