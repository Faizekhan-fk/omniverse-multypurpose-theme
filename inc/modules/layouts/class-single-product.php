...
class Single_Product extends Layout_Type {
	...
	public function check( $condition, $type = '' ) {
		...
		switch ( $condition['condition_type'] ) {
			case 'all':
				...
				break;
			case 'product':
				$is_active = (int) omniverse_get_the_ID() === (int) $condition['condition_query'];
				break;
			case 'product_type':
				$is_active = WC_Product_Factory::get_product_type( omniverse_get_the_ID() ) === $condition['condition_query'];
				break;
			case 'product_cat':
			case 'product_tag':
	...
	public function override_template( $template ) {
		if ( omniverse_woocommerce_installed() && is_singular( 'product' ) && Main::get_instance()->has_custom_layout( 'single_product' ) ) {
	...
	private function display_template() {
		...
		omniverse_enqueue_inline_style( 'woo-single-prod-builder' );
		?>
	...
	public static function get_preview_product_id() {
		$product_id = omniverse_get_opt( 'single_product_builder_post_data' );
	...
	public static function setup_postdata( $force_product_id = false ) {
		...
		if ( ( Main::is_layout_type( 'single_product') && empty( $product ) ) || is_singular( 'omniverse_layout' ) || wp_doing_ajax() || ( isset( $_POST['action'] ) && 'editpost' === $_POST['action'] ) || $force_product_id ) { // phpcs:ignore
	...
	public static function reset_postdata( $force_product_id = false ) {
		if ( is_singular( 'omniverse_layout' ) || wp_doing_ajax() || $force_product_id ) {
			wp_reset_postdata();
		}
	}
