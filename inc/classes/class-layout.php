...
/**
 * OMNIVERSE_Layout Class set up layout settings
...
class Layout {
	...
	public function set_page_id() {
		$this->page_id = omniverse_get_the_ID( array( 'singulars' => array( 'product' ) ) );
	}
	...
	public function get_sidebar_name() {
		return apply_filters( 'omniverse_get_sidebar_name', $this->sidebar_name );
	}
	...
	private function set_sidebar_name() {
		...
		if ( omniverse_woocommerce_installed() && ( is_product_taxonomy() || is_shop() || is_product_category() || is_product_tag() || omniverse_is_product_attribute_archive() || ( is_search() && isset( $_GET['post_type'] ) && 'product' === $_GET['post_type'] ) ) ) {
		...
		if ( $page_id ) {
			$specific = get_post_meta( $page_id, '_omniverse_custom_sidebar', true );
		}
	...
	private function set_content_class() {
		...
		if ( omniverse_woocommerce_installed() && ( is_checkout() || is_cart() ) ) {
			$this->content_class .= omniverse_get_builder_status_class();
		}
	}
	...
	private function set_sidebar_col_width() {
		...
		$page_id                 = $this->get_page_id();
		$this->sidebar_col_width = omniverse_get_opt( 'sidebar_width' );

		if ( $page_id ) {
			$specific = get_post_meta( $page_id, '_omniverse_sidebar_width', true );
		}

		// Get specific sidebar size for Shop Page.
		if ( omniverse_woocommerce_installed() && ( is_shop() || is_product_category() || is_product_tag() || omniverse_is_product_attribute_archive() || ( is_search() && isset( $_GET['post_type'] ) && 'product' === $_GET['post_type'] ) ) ) {
			$this->sidebar_col_width = omniverse_get_opt( 'shop_sidebar_width' );
		} elseif ( is_singular( 'product' ) ) {
			// Get specific layout for SINGLE PRODUCT PAGE.
			$this->sidebar_col_width = omniverse_get_opt( 'single_sidebar_width' );
		} elseif ( is_home() || is_singular( 'post' ) || is_archive() ) {
			// Get specific sidebar size for Blog Page.
			$this->sidebar_col_width = omniverse_get_opt( 'blog_sidebar_width' );
		} elseif ( is_singular( 'omniverse_layout' ) ) {
	...
	public function get_page_layout() {
		return apply_filters( 'omniverse_get_page_layout', $this->page_layout );
	}
	...
	private function set_page_layout() {
		...
		$this->page_layout = omniverse_get_opt( 'main_layout' );
		...
		if ( $page_id ) {
			$specific = get_post_meta( $page_id, '_omniverse_main_layout', true );
		}

		if ( omniverse_woocommerce_installed() && ( is_shop() || is_product_category() || is_product_tag() || omniverse_is_product_attribute_archive() || ( is_search() && isset( $_GET['post_type'] ) && 'product' === $_GET['post_type'] ) ) ) {
			// Get specific layout for Shop Page.
			$this->page_layout = omniverse_get_opt( 'shop_layout' );
		} elseif ( $this->is_account_pages() ) {
			$this->page_layout = 'full-width';
		} elseif ( is_singular( 'product' ) ) {
			// Get specific layout for SINGLE PRODUCT PAGE.
			$this->page_layout = omniverse_get_opt( 'single_product_layout' );
		} elseif ( isset( $WCMp ) && is_tax( $WCMp->taxonomy->taxonomy_name ) ) {
			$this->page_layout = omniverse_get_opt( 'blog_layout' );
		} elseif ( is_singular( 'post' ) || omniverse_is_blog_archive() ) {
			// Get specific layout for Blog Page.
			$this->page_layout = omniverse_get_opt( 'blog_layout' );

			// Disable sidebar if blog design is Masonry Grid.
			if ( 'masonry' === omniverse_get_opt( 'blog_design' ) && ! is_singular( 'post' ) ) {
				$this->page_layout = 'full-width';
			}
		}
	...
	public function is_account_pages() {
		$wishlist_page_id = omniverse_get_opt( 'wishlist_page' );
	...
	public function get_main_container_class() {
		...
		if ( omniverse_woocommerce_installed() && is_singular( 'product' ) && ! get_query_var( 'edit' ) && ( ( function_exists( 'omniverse_elementor_has_location' ) && ! omniverse_elementor_has_location( 'single' ) ) || ! function_exists( 'omniverse_elementor_has_location' ) ) ) {
			$class = 'container-fluid';

			if ( $is_product_builder && 'enabled' === omniverse_get_opt( 'negative_gap' ) ) {
				$class = 'container';
			}
		}

		if ( ( (int) omniverse_get_portfolio_page_id() === (int) omniverse_page_ID() ) && omniverse_get_opt( 'portfolio_full_width' ) ) {
			$class = 'container-fluid container-no-gutters';
		}

		if ( omniverse_is_elementor_full_width() ) {
			$class = 'container-fluid container-no-gutters';
		}

		return apply_filters( 'omniverse_main_content_classes', $class );
	}
