...
class Theme {
	...
	public function __construct() {
		...
		if ( 'elementor' === omniverse_get_current_page_builder() ) {
			require_once get_parent_theme_file_path( OMNIVERSE_FRAMEWORK . '/integrations/elementor/class-elementor.php' );
		}
	}
	...
	private function wpb_files_include() {
		if ( 'wpb' !== omniverse_get_current_page_builder() || ! defined( 'WPB_VC_VERSION' ) ) {
			return;
		}
	...
	private function core_plugin_classes() {
		if ( class_exists( 'OMNIVERSE_Auth' ) ) {
			$files = array(
			...
			foreach ( $files as $file ) {
				require_once apply_filters( 'omniverse_require', OMNIVERSE_PT_3D . $file . '.php' );
			}
