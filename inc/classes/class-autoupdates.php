...
class Autoupdates {
	...
	private $_theme_name      = '';
	private $old_api_url      = 'https://zynxsol.com/licenses/api/';
	private $_info;

	function __construct() {
		$this->_current_version = omniverse_get_theme_info( 'Version' );
		$this->_theme_name      = OMNIVERSE_SLUG;
		...
		add_filter( 'omniverse_setup_wizard', array( $this, 'update_plugins_version' ), 30 );

		if ( ! omniverse_is_license_activated() ) {
			return;
		}
	...
	protected function check_for_update() {
		...
		// Get data.
		if ( empty( $this->_info ) ) {
			$version_information = get_option( 'omniverse-update-info', false );
		...
		$last_check = get_option( 'omniverse-update-time' );

		if ( ! $last_check ) {
			update_option( 'omniverse-update-time', time() );
		}
		...
		if ( time() - $last_check > 172800 || $force || ! $last_check ) {
			...
			$this->_info->url         = 'https://zynxsol.com/omniverse-changelog.php';
			$this->_info->package     = $this->download_url();

		}

		// Save results.
		update_option( 'omniverse-update-info', $this->_info );
	}
	...
	public function update_plugins_version( $transient ) {
		...
		$force      = false;
		$last_check = get_option( 'omniverse-plugins-update-time' );
		...
		if ( ! $last_check ) {
			update_option( 'omniverse-plugins-update-time', time() );
		}
		...
		if ( time() - $last_check > 172800 || $force || ! $last_check ) {
			...
			foreach ( $plugins as $plugin ) {
				...
				update_option( 'omniverse_' . $plugin . '_version', $response->version );
			}
		}
	...
	public function download_url() {
		return $this->_api->get_url(
			'download',
			array(
				'token' => get_option( 'omniverse_token' ),
