...
class Notices {
	...
	public function show_msgs( $globals = false ) {
		...
		if ( ! empty( $msgs ) ) {
			foreach ( $msgs as $key => $msg ) {
				...
				echo '<div class="omniverse-msg dn-notice dn-' . $msg['type'] . '">';
	...
	public function add_notice() {
		...
		if ( ! empty( $msgs ) ) {
			foreach ( $msgs as $key => $msg ) {
				...
				echo '<div class="dn-notice notice dn-' . esc_attr( $msg['type'] ) . '">';
				echo '<p>' . wp_kses( $msg['msg'], true ) . '</p>';

				if ( 'error' !== $msg['type'] ) {
					echo '<a class="wd-dismiss-link" href="' . esc_url( wp_nonce_url( add_query_arg( 'omniverse-hide-notice', $hash ) ) ) . '">' . esc_html_e( 'Dismiss Notice', 'omniverse' ) . '</a>';
					echo '<a class="notice-dismiss" href="' . esc_url( wp_nonce_url( add_query_arg( 'omniverse-hide-notice', $hash ) ) ) . '"></a>';
				}
	...
	public function nag_ignore() {
		if ( ! isset( $_GET['omniverse-hide-notice'] ) ) {
			return;
		}
		global $current_user;
		$user_id = $current_user->ID;

		$hide_notice = sanitize_text_field( wp_unslash( $_GET['omniverse-hide-notice'] ) );
