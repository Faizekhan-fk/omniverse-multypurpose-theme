...
if( ! function_exists( 'omniverse_shortcode_author_area' ) ) {
	function omniverse_shortcode_author_area($atts, $content) {
		$output = $class = '';
		...
		if ( $img_id ) {
			$image_output = omniverse_otf_get_image_html( $img_id, $img_size, array(), array( 'class' => 'author-area-image' ) );
		}
		...
		ob_start(); ?>
			...
			<div class="author-area set-mb-m reset-last-child<?php echo esc_attr( $class ); ?>">
				...
				<?php if ( $link != '' ) : ?>
					<a <?php echo omniverse_get_link_attributes( $link ); ?> class="btn btn-style-link btn-color-default">
