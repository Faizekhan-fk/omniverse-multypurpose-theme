...
if( ! function_exists( 'omniverse_shortcode_countdown_timer' )) {
	function omniverse_shortcode_countdown_timer($atts, $content) {
		...
		$timer_classes .= ' timer-size-' . $size;
		$timer_classes .= ' timer-style-' . $style;
		$timer_classes .= omniverse_get_old_classes( ' omniverse-timer' );

		$timezone = 'GMT';

		$date = str_replace( '/', '-', apply_filters( 'wd_countdown_timer_end_date', $date ) );

		if ( apply_filters( 'omniverse_wp_timezone_element', false ) ) $timezone = get_option( 'timezone_string' );
		ob_start();
