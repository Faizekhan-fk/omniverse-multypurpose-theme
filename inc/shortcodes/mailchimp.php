...
if ( ! function_exists( 'omniverse_shortcode_mailchimp' ) ) {
	function omniverse_shortcode_mailchimp( $atts ) {
		...
		if ( ! empty( $atts['omniverse_css_id'] ) ) {
			$omniverse_css_id .= $atts['omniverse_css_id'];
		}
		...
		<div id="<?php echo esc_attr( 'wd-' . $omniverse_css_id ); ?>" class="wd-wpb wd-mc4wp-wrapper<?php echo esc_attr( $mc4wp_classes ); ?>">
			<?php
