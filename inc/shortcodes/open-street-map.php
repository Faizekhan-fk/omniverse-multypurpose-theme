...
if ( ! function_exists( 'omniverse_get_settings_coords_for_open_street_map' ) ) {
	/**
	 ...
	 */
	function omniverse_get_settings_coords_for_open_street_map( $markers ) {
		...
		foreach ( $markers as $marker ) {
			...
			$marker['image_size'] = 'full' === $marker['image_size'] ? array() : omniverse_get_image_size( $marker['image_size'] );
...
if ( ! function_exists( 'omniverse_shortcode_open_street_map' ) ) {
	/***
	 ...
	 * @return false|string
	 */
	function omniverse_shortcode_open_street_map( $attr, $content ) {
		...
		$attr = shortcode_atts(
			array(
				...
				'marker_icon'                => OMNIVERSE_IMAGES . '/icons/marker-icon.png',
		...
		$map_id  = 'wd-rs-' . $attr['omniverse_css_id'];
		$markers = vc_param_group_parse_atts( $attr['marker_list'] );
		$coords  = omniverse_get_settings_coords_for_open_street_map( $markers );
		$zoom    = json_decode( omniverse_decompress( $attr['zoom'] ), true );

		$wrapper_classes .= ! empty( $attr['el_class'] ) ? ' ' . $attr['el_class'] : '';
		$wrapper_classes .= omniverse_get_css_animation( $attr['css_animation'] );
		...
		if ( ! empty( $attr['map_init_placeholder'] ) ) {
			$placeholder = omniverse_otf_get_image_html( $attr['map_init_placeholder'], ! empty( $attr['map_init_placeholder_size'] ) ? $attr['map_init_placeholder_size'] : 'full' );
		}
		...
		$map_settings = array_filter(
			array(
				...
				'iconSize'                   => 'full' === $attr['marker_icon_size'] ? array() : omniverse_get_image_size( $attr['marker_icon_size'] ),
