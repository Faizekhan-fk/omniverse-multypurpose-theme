...
if ( ! function_exists( 'omniverse_shortcode_tabs' ) ) {
	/***
	 ...
	 * @return false|string
	 */
	function omniverse_shortcode_tabs( $attr, $content ) {
		...
		$title_id = 'wd-' . $attr['omniverse_css_id'];
		...
		$tabs_data = omniverse_get_tabs_title_from_shortcode( $content );
			...
			<div class="wd-tabs-header text-<?php echo esc_attr( $attr['tabs_title_alignment'] ); ?>">
				<?php if ( $attr['title'] ) : ?>
					<div class="tabs-name title">
						<?php
						if ( $img_id ) {
							echo omniverse_display_icon( $img_id, $attr['img_size'], 30 ); //phpcs:ignore
						}
						?>
						<span class="tabs-text"><?php echo wp_kses( $attr['title'], omniverse_get_allowed_html() ); ?></span>
					</div>
				<?php endif; ?>

				<?php if ( $attr['description'] ) : ?>
					<div class="wd-tabs-desc"><?php echo wp_kses( $attr['description'], omniverse_get_allowed_html() ); ?></div>
				<?php endif; ?>
					...
					<ul class="wd-nav wd-nav-tabs<?php echo esc_attr( $nav_tabs_classes ); ?>">
						<?php foreach ( $tabs_data as $data ) : ?>
							...
							if ( ! empty( $data['tabs_image'] ) ) {
								$icon_output = omniverse_display_icon( $data['tabs_image'], $data['tabs_image_size'], 128 ); // phpcs:ignore

								if ( omniverse_is_svg( wp_get_attachment_image_src( $data['tabs_image'] )[0] ) ) {
									$icon_output = '<div class="img-wrapper">' . omniverse_get_svg_html( $data['tabs_image'], $data['tabs_image_size'] ) . '</div>';
								}
		...
		<?php
		return apply_filters( 'vc_shortcode_output', ob_get_clean(), new WD_WPBakeryShortCodeFix(), $attr, 'omniverse_tabs' );
	}
}

if ( ! function_exists( 'omniverse_shortcode_tab' ) ) {
	/**
	 ...
	 * @return false|string
	 */
	function omniverse_shortcode_tab( $attr, $content ) {
		...
		<div class="wd-tab-content">
			<?php if ( 'html_block' === $content_type ) : ?>
				<?php echo omniverse_get_html_block( $attr['html_block_id'] ); // phpcs:ignore ?>
...
if ( ! function_exists( 'omniverse_get_tabs_title_from_shortcode' ) ) {
	/**
	 ...
	 */
	function omniverse_get_tabs_title_from_shortcode( $content ) {
		preg_match_all( '/omniverse_tab([^\]]+)/i', $content, $matches, PREG_OFFSET_CAPTURE );
