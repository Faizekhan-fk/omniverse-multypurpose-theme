...
if( ! function_exists( 'omniverse_shortcode_blog' ) ) {
	function omniverse_shortcode_blog( $atts ) {
		...
		omniverse_set_loop_prop( 'blog_design', $blog_design );
		omniverse_set_loop_prop( 'img_size', $img_size );
		omniverse_set_loop_prop( 'blog_columns', $blog_columns );
		...
		omniverse_set_loop_prop( 'parts_media', $parts_media );

		if ( 'auto' !== $blog_columns_tablet ) {
			omniverse_set_loop_prop( 'blog_columns_tablet', $blog_columns_tablet );
		}
		if ( 'auto' !== $blog_columns_mobile ) {
			omniverse_set_loop_prop( 'blog_columns_mobile', $blog_columns_mobile );
		}

		$parsed_atts['custom_sizes'] = apply_filters( 'omniverse_blog_shortcode_custom_sizes', false );

		if ( '' === $parsed_atts['blog_spacing'] ) {
			$parsed_atts['blog_spacing'] = omniverse_get_opt( 'blog_spacing' );

			if ( $parsed_atts['blog_spacing_tablet'] ) {
				$parsed_atts['blog_spacing_tablet'] = omniverse_get_opt( 'blog_spacing_tablet' );
			}
			if ( $parsed_atts['blog_spacing_mobile'] ) {
				$parsed_atts['blog_spacing_mobile'] = omniverse_get_opt( 'blog_spacing_mobile' );
			}
		}

//	    if( ! $parts_btn ) omniverse_set_loop_prop( 'parts_btn', false );
		...
		if ( omniverse_is_blog_design_new( $true_blog_design ) ) {
		...
		if ( 'small-images' === $true_blog_design || 'chess' === $true_blog_design ) {
			...
			omniverse_enqueue_inline_style( 'blog-loop-design-' . $true_blog_design );
		}
		...
		if ( 'carousel' === $blog_design ) {
			...
			return omniverse_generate_posts_slider( $parsed_atts, $blog_query );
		} else {
			$attributes = '';
				...
				?>
			    	<div class="wd-loop-footer blog-footer">
			    		<?php if ( $pagination == 'infinit' || $pagination == 'more-btn' ): ?>
							...
							<a href="#" data-holder-id="<?php echo esc_attr( $id ); ?>" rel="nofollow noopener" class="btn wd-load-more wd-blog-load-more load-on-<?php echo 'more-btn' === $pagination ? 'click' : 'scroll'; ?>"><span class="load-more-label"><?php esc_html_e( 'Load more posts', 'omniverse' ); ?></span></a>
