...
if ( ! function_exists( 'omniverse_shortcode_products_tabs' ) ) {
	function omniverse_shortcode_products_tabs( $atts = array(), $content = null ) {
		...
		foreach ( $tab_titles as $tab ) {
			...
			if ( isset( $tab_atts['icon'] ) && $tab_atts['icon'] ) {
				$icon_output = omniverse_display_icon( $tab_atts['icon'], $tab_atts['icon_size'], 25 );

				if ( omniverse_is_svg( wp_get_attachment_image_src( $tab_atts['icon'] )[0] ) ) {
					$icon_output = '<div class="img-wrapper">' . omniverse_get_svg_html( $tab_atts['icon'], $tab_atts['icon_size'] ) . '</div>';
				}
		...
		$class .= omniverse_get_old_classes( ' omniverse-products-tabs' );
		...
		<div id="<?php echo esc_attr( $tabs_id ); ?>" class="wd-tabs wd-products-tabs wd-wpb<?php echo esc_attr( $class ); ?>">
			<div class="wd-tabs-header<?php echo esc_attr( $header_classes ); ?>">
				<?php if ( ! empty( $title ) ) : ?>
					<div class="tabs-name title">
						<?php
						if ( $img_id ) {
							echo omniverse_display_icon( $img_id, $img_size, 30 );}
						?>
						<span class="tabs-text"><?php echo wp_kses( $title, omniverse_get_allowed_html() ); ?></span>
					</div>
				<?php endif; ?>

				<?php if ( $description ) : ?>
					<div class="wd-tabs-desc"><?php echo wp_kses( $description, omniverse_get_allowed_html() ); ?></div>
				<?php endif; ?>
			...
			if ( isset( $tab_titles[0][0] ) ) {
				...
				echo omniverse_shortcode_products_tab( $first_tab_atts );
			}
			?>
			<?php
...
if ( ! function_exists( 'omniverse_shortcode_products_tab' ) ) {
	function omniverse_shortcode_products_tab( $atts ) {
		global $wpdb, $post;
		...
		$parsed_atts = shortcode_atts(
			array_merge(
				...
				omniverse_get_default_product_shortcode_atts()
			),
			$atts
		);
		...
		$class .= omniverse_get_old_classes( ' omniverse-tab-content' );
		...
		<?php
		echo omniverse_shortcode_products( $parsed_atts );
		?>
		<?php if ( ! $is_ajax ) : ?>
			</div>
		<?php endif; ?>
