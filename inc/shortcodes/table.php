...
if ( ! function_exists( 'omniverse_shortcode_table' ) ) {
	function omniverse_shortcode_table( $settings, $content ) {
		...
		$table_classes = 'text-' . omniverse_vc_get_control_data( $settings['alignment'], 'desktop' );
...
if ( ! function_exists( 'omniverse_shortcode_table_row' ) ) {
	function omniverse_shortcode_table_row( $settings, $content ) {
		$default_settings = array(
			'css'          => '',
			'table_column' => '',
		);
		...
		?>
			<tr class="<?php echo esc_attr( $wrapper_classes ); ?>">
				<?php foreach ( $row_items as $item ) : ?>
					...
					<<?php echo esc_attr( $tag ) . omniverse_get_table_attribute( $item ); ?>>
...
if ( ! function_exists( 'omniverse_get_table_attribute' ) ) {
	/**
	 * Get row item css.
	 *
	 ...
	 * @return string
	 */
	function omniverse_get_table_attribute( $attr ) {
		$attributes = '';
		$style      = '';
		$classes    = '';

		if ( ! empty( $attr['row_item_color'] ) ) {
			$style .= 'color: ' . omniverse_vc_get_control_data( $attr['row_item_color'], 'desktop' ) . ';';
		}
		if ( ! empty( $attr['row_item_bg_color'] ) ) {
			$style .= 'background-color: ' . omniverse_vc_get_control_data( $attr['row_item_bg_color'], 'desktop' ) . ';';
		}
