...
if ( ! function_exists( 'omniverse_shortcode_pricing_tables' ) ) {
	function omniverse_shortcode_pricing_tables( $atts = array(), $content = null ) {
		$classes     = '';
		$style_attrs = '';
		$atts        = shortcode_atts(
			array(
...
if ( ! function_exists( 'omniverse_shortcode_pricing_plan' ) ) {
	function omniverse_shortcode_pricing_plan( $atts, $content ) {
		global $wpdb, $post;
		...
		if ( $with_bg_image == 'yes' && $bg_image ) {
			$class   .= ' with-bg-image';
			$image    = omniverse_otf_get_image_url( $bg_image, 'full' );
			...
			<div class="wd-price-table wd-col<?php echo esc_attr( $class ); ?>" >
				<div class="wd-plan">
					<div class="wd-plan-name">
						<span class="wd-plan-title title"><?php echo wp_kses( $name, omniverse_get_allowed_html() ); ?></span>
					</div>
				</div>
				<div class="wd-plan-inner">
					<?php if ( ! empty( $label ) ) : ?>
						<div class="wd-plan-label price-label"><span><?php echo wp_kses( $label, omniverse_get_allowed_html() ); ?></span></div>
					<?php endif ?>
					<div class="wd-plan-price" style="<?php echo esc_attr( $bg_style ); ?>">
						<?php if ( $currency ) : ?>
							<span class="wd-price-currency">
								<?php echo wp_kses( $currency, omniverse_get_allowed_html() ); ?>
							</span>
						<?php endif ?>

						<?php if ( $price_value != '' ) : ?>
							<span class="wd-price-value">
								<?php echo wp_kses( $price_value, omniverse_get_allowed_html() ); ?>
							</span>
						<?php endif ?>

						<?php if ( $price_suffix ) : ?>
							<span class="wd-price-suffix">
								<?php echo wp_kses( $price_suffix, omniverse_get_allowed_html() ); ?>
							</span>
						<?php endif ?>
					</div>
					...
					<div class="wd-plan-footer">
						...
						<?php else : ?>
							<?php if ( $button_label ) : ?>
								<a <?php echo omniverse_get_link_attributes( $link ); ?> class="button price-plan-btn">
									<?php echo wp_kses( $button_label, omniverse_get_allowed_html() ); ?>
								</a>
							<?php endif ?>
						<?php endif ?>
					</div>
