...
if ( ! function_exists( 'omniverse_shortcode_promo_banner' ) ) {
	function omniverse_shortcode_promo_banner( $atts, $content ) {
		...
		$subtitle_class .= ' ' . omniverse_get_new_size_classes( 'banner', $title_size, 'subtitle' );

		$title_class .= ' wd-font-weight-' . $font_weight;
		if ( $title_font ) {
			$title_class .= ' font-' . $title_font;
		}
		$title_class .= ' ' . omniverse_get_new_size_classes( 'banner', $title_size, 'title' );
		...
		$class .= omniverse_get_old_classes( ' banner-vr-align-' . $vertical_alignment );
		$class .= omniverse_get_old_classes( ' banner-hr-align-' . $horizontal_alignment );

		$inner_class .= ' ' . omniverse_get_new_size_classes( 'banner', $content_text_size, 'content' );
		...
		if ( $hover == 'parallax' ) {
			...
			omniverse_enqueue_js_script( 'banner-element' );
		}
		...
		if ( 'image' === $source_type ) {
			$source_output = omniverse_otf_get_image_html( $image, $img_size );
			...
			if ( $video_poster ) {
				if ( in_array( $video_poster_size, array( 'thumbnail', 'thumb', 'medium', 'large', 'full' ), true ) || ( is_string( $video_poster_size ) && preg_match_all( '/\d+/', $video_poster_size ) ) ) {
					$poster_src = omniverse_otf_get_image_url( $video_poster, $video_poster_size );
				} else {
		...
		omniverse_enqueue_inline_style( 'banner' );

		if ( in_array( $style, array( 'mask', 'shadow' ), true ) ) {
			omniverse_enqueue_inline_style( 'banner-style-mask-and-shadow' );
		} elseif ( in_array( $style, array( 'border', 'background' ), true ) ) {
			omniverse_enqueue_inline_style( 'banner-style-bg-and-border' );
		} elseif ( 'content-background' === $style ) {
			omniverse_enqueue_inline_style( 'banner-style-bg-cont' );
		}

		if ( in_array( $hover, array( 'background', 'border' ), true ) ) {
			omniverse_enqueue_inline_style( 'banner-hover-bg-and-border' );
		} elseif ( in_array( $hover, array( 'zoom', 'zoom-reverse' ), true ) ) {
			omniverse_enqueue_inline_style( 'banner-hover-zoom' );
		}

		if ( 'hover' === $btn_position ) {
			omniverse_enqueue_inline_style( 'banner-btn-hover' );
		}
		...
		<div class="promo-banner-wrapper <?php echo esc_attr( $wrapper_class ); ?>">
			<div id="<?php echo esc_attr( $id ); ?>" class="promo-banner <?php echo esc_attr( $class ); ?>"<?php echo wp_kses( $wrapper_style_attr, true ); ?>>
					...
					<div class="content-banner <?php echo esc_attr( $content_banner ); ?>">
						...
						if ( ! empty( $btn_text ) ) {
							echo '<div class="banner-btn-wrapper' . esc_attr( $btn_wrapper_classes ) . '">';
								echo omniverse_shortcode_button(
									array(
										'title'            => $btn_text,
										'color'            => $btn_color,
										'style'            => $btn_style,
				...
				<?php if ( $link_attributes ) : ?>
					<a class="wd-promo-banner-link wd-fill" aria-label="<?php esc_html_e( 'Banner link', 'omniverse' ); ?>" <?php echo wp_kses( $link_attributes, true ); ?>></a>
				<?php endif; ?>

				<?php
				$css = '';
				if ( $custom_title_color && ! omniverse_is_css_encode( $custom_title_color ) ) $css .= '#' . $id . ' .banner-title{color:' . $custom_title_color . '}';
				if ( $custom_subtitle_color && ! omniverse_is_css_encode( $custom_subtitle_color ) ) $css .= '#' . $id . ' .banner-subtitle{color:' . $custom_subtitle_color . '}';
				if ( $custom_text_color && ! omniverse_is_css_encode( $custom_text_color ) ) $css .= '#' . $id . ' .banner-inner{color:' . $custom_text_color . '}';

				// Text size
				if ( $title_desktop_text_size ) $css .= omniverse_responsive_text_size_css( $id, 'banner-title', $title_desktop_text_size, 'return' );
				if ( $subtitle_desktop_text_size ) $css .= omniverse_responsive_text_size_css( $id, 'banner-subtitle', $subtitle_desktop_text_size, 'return' );

				if ( $title_tablet_text_size || $subtitle_tablet_text_size ) {
					$css .= '@media (max-width:1199px){';
					if ( $title_tablet_text_size ) $css .= omniverse_responsive_text_size_css( $id, 'banner-title', $title_tablet_text_size, 'return' );
					if ( $subtitle_tablet_text_size ) $css .= omniverse_responsive_text_size_css( $id, 'banner-subtitle', $subtitle_tablet_text_size, 'return' );
					$css .= '}';
				}

				if ( $title_mobile_text_size || $subtitle_mobile_text_size ) {
					$css .= '@media (max-width:767px){';
					if ( $title_mobile_text_size ) $css .= omniverse_responsive_text_size_css( $id, 'banner-title', $title_mobile_text_size, 'return' );
					if ( $subtitle_mobile_text_size ) $css .= omniverse_responsive_text_size_css( $id, 'banner-subtitle', $subtitle_mobile_text_size, 'return' );
					$css .= '}';
				}
...
if ( ! function_exists( 'omniverse_shortcode_banners_carousel' ) ) {
	function omniverse_shortcode_banners_carousel( $atts = array(), $content = null ) {
		...
		$custom_sizes = apply_filters( 'omniverse_promo_banner_shortcode_custom_sizes', false );
		...
		?>
			...
			<div id="<?php echo esc_attr( $carousel_id ); ?>" class="wd-carousel-container banners-carousel-wrapper <?php echo esc_attr( $wrapper_classes ); ?>">
				<div class="wd-carousel-inner">
					<div class="wd-carousel wd-grid banners-carousel<?php echo esc_attr( $class ); ?>" <?php echo omniverse_get_carousel_attributes( $parsed_atts ); ?>>
