...
if ( ! function_exists( 'omniverse_shortcode_marquee' ) ) {
	/**
	 ...
	 * @return false|string
	 */
	function omniverse_shortcode_marquee( $atts, $content ) {
		$atts = shortcode_atts(
			array(
				'omniverse_css_id'  => '',
		...
		if ( 'image' === $atts['icon_type'] && ! empty( $atts['image'] ) ) {
			if ( omniverse_is_svg( wp_get_attachment_image_url( $atts['image'] ) ) ) {
				$icon_output = omniverse_get_svg_html(
					$atts['image'],
					$atts['img_size']
				);
			} else {
				$icon_output = omniverse_otf_get_image_html( $atts['image'], $atts['img_size'] );
			}
		}
		...
		foreach ( $marquee_contents as $item ) {
			...
			if ( isset( $item['icon_type'] ) && 'image' === $item['icon_type'] && ! empty( $item['image_id'] ) ) {
				if ( omniverse_is_svg( wp_get_attachment_image_url( $item['image_id'] ) ) ) {
					...
					$item_icon_output = omniverse_otf_get_image_html( $item['image_id'], $item['image_size'] );
				}
			}
			...
			<span>
				...
				<?php if ( isset( $item['link'] ) ) : ?>
					<a class="wd-fill" <?php echo $link_attrs; // phpcs:ignore. ?> aria-label="<?php esc_attr_e( 'Marquee item link', 'omniverse' ); ?>"></a>
				<?php endif; ?>
			</span>
			<?php
