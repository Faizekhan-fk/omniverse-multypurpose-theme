...
if( ! function_exists( 'omniverse_ajax_search' ) ) {
	function omniverse_ajax_search( $atts ) {
		...
		$class .= ' wd-color-' . $atts['omniverse_color_scheme'];
		$class .= ' ' . $atts['el_class'];
		if( function_exists( 'vc_shortcode_custom_css_class' ) ) $class .= ' ' . vc_shortcode_custom_css_class( $atts['css'] );

		$class .= omniverse_get_old_classes( ' omniverse-vc-ajax-search' );

		ob_start();
		?>
			<div class="wd-el-search wd-wpb omniverse-ajax-search <?php echo esc_attr( $class ); ?>">
				<?php
