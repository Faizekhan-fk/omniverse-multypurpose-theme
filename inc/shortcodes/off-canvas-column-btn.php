...
if ( ! function_exists( 'omniverse_shortcode_off_canvas_btn' ) ) {
	/***
	 ...
	 * @return false|string
	 */
	function omniverse_shortcode_off_canvas_btn( $attr, $content ) {
		$wrapper_classes = apply_filters( 'vc_shortcodes_css_class', '', 'omniverse_off_canvas_btn', $attr );
		...
		if ( omniverse_is_svg( wp_get_attachment_image_url( $settings['img_id'] ) ) ) {
			$icon_output = omniverse_get_svg_html(
				$settings['img_id'],
				$settings['img_size']
			);
		} else {
			$icon_output = omniverse_otf_get_image_html( $settings['img_id'], $settings['img_size'] );
		}
		...
		<?php
		return apply_filters( 'vc_shortcode_output', ob_get_clean(), new WD_WPBakeryShortCodeFix(), $attr, 'omniverse_shortcode_off_canvas_btn' );
	}
}
