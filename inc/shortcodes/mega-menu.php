<?php

if ( ! defined( 'OMNIVERSE_THEME_DIR' ) ) {
	exit( 'No direct script access allowed' );
}

use DN\Modules\Mega_Menu_Walker;

/**
* ------------------------------------------------------------------------------------------------
* Mega Menu widget
* ------------------------------------------------------------------------------------------------
*/

if ( ! function_exists( 'omniverse_shortcode_mega_menu' ) ) {
	function omniverse_shortcode_mega_menu( $atts, $content ) {
		$output = $title_html = '';
		$class  = apply_filters( 'vc_shortcodes_css_class', '', '', $atts );

		$atts = shortcode_atts(
			array(
				'title'                 => '',
				'nav_menu'              => '',
				'style'                 => 'default',
				'design'                => 'vertical',
				'dropdown_design'       => 'default',
				'items_gap'             => 's',
				'vertical_items_gap'    => 's',
				'alignment'             => 'left',
				'color'                 => '',
				'icon_alignment'        => 'inherit',
				'omniverse_color_scheme' => 'light',
				'el_class'              => '',
				'omniverse_css_id'       => '',
				'css'                   => '',
			),
			$atts
		);

		$title_classes = ' color-scheme-' . esc_attr( $atts['omniverse_color_scheme'] );

		if ( function_exists( 'vc_shortcode_custom_css_class' ) ) {
			$class .= ' ' . vc_shortcode_custom_css_class( $atts['css'] );
		}
		if ( ! empty( $atts['el_class'] ) ) {
			$class .= ' ' . $atts['el_class'];
		}

		$widget_id = 'wd-' . $atts['omniverse_css_id'];

		$menu_classes  = ' wd-nav-' . $atts['design'];
		$menu_classes .= ' wd-style-' . $atts['style'];
		$menu_classes .= omniverse_get_old_classes( ' ' . $atts['design'] . '-navigation' );
		$menu_classes .= omniverse_get_old_classes( ' navigation-style-' . $atts['style'] );

		if ( 'horizontal' === $atts['design'] ) {
			$class        .= ' text-' . $atts['alignment'];
			$menu_classes .= ' wd-gap-' . $atts['items_gap'];
		}

		if ( 'vertical' === $atts['design'] ) {
			$menu_classes .= ' wd-design-' . $atts['dropdown_design'];
			$menu_classes .= ' wd-gap-' . $atts['vertical_items_gap'];
		}

		if ( $atts['icon_alignment'] && 'inherit' !== $atts['icon_alignment'] ) {
			$menu_classes .= ' wd-icon-' . $atts['icon_alignment'];
		}

		ob_start();

		omniverse_enqueue_inline_style( 'widget-nav-mega-menu' );

		if ( 'vertical' === $atts['design'] ) {
			omniverse_enqueue_inline_style( 'mod-nav-vertical' );
			omniverse_enqueue_inline_style( 'mod-nav-vertical-design-' . $atts['dropdown_design'] );
		}
		?>

			<div id="<?php echo esc_attr( $widget_id ); ?>" class="widget_nav_mega_menu <?php echo esc_attr( $class ); ?>">

				<?php if ( $atts['title'] ) : ?>
					<h5 class="widget-title<?php echo esc_attr( $title_classes ); ?>">
						<?php echo wp_kses( $atts['title'], omniverse_get_allowed_html() ); ?>
					</h5>
				<?php endif; ?>

				<?php
					wp_nav_menu(
						array(
							'fallback_cb' => '',
							'container'   => '',
							'menu'        => $atts['nav_menu'],
							'menu_class'  => 'menu wd-nav' . $menu_classes,
							'walker'      => new Mega_Menu_Walker(),
						)
					);
				?>

				<?php
				if ( $atts['color'] && ! omniverse_is_css_encode( $atts['color'] ) ) {
					$atts['css']  = '#' . esc_attr( $widget_id ) . ' .widget-title {';
					$atts['css'] .= 'background-color: ' . esc_attr( $atts['color'] ) . ';';
					$atts['css'] .= '}';

					wp_add_inline_style( 'omniverse-inline-css', $atts['css'] );
				}
				?>
			</div>
		<?php
		$output = ob_get_contents();
		ob_end_clean();

		return $output;
	}
}
