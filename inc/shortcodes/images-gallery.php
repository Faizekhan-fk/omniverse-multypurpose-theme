 ...
 * ------------------------------------------------------------------------------------------------
 */
if ( ! function_exists( 'omniverse_images_gallery_shortcode' ) ) {
	function omniverse_images_gallery_shortcode( $atts ) {
		...
		if ( 'carousel' === $view ) {
			...
			$gallery_attrs         = omniverse_get_carousel_attributes( $parsed_atts );
		...
		if ( 'grid' === $view || 'masonry' === $view ) {
			...
			$gallery_attrs .= ' style="' . omniverse_get_grid_attrs( $parsed_atts ) . '"';
		}
					...
					$i = 0; foreach ( $images as $img_id ) :
						...
						<div class="wd-gallery-item<?php echo esc_attr( $gallery_item_classes ); ?>">
							...
							<a href="<?php echo esc_url( $link ); ?>" data-elementor-open-lightbox="no" data-index="<?php echo esc_attr( $i ); ?>" data-width="<?php echo esc_attr( $width ); ?>" data-height="<?php echo esc_attr( $height ); ?>" 
								...
								<?php echo omniverse_otf_get_image_html( $img_id, $img_size, array(), array( 'class' => 'wd-gallery-image image-' . $i ) ); ?>
