...
if( ! function_exists( 'omniverse_shortcode_info_box' ) ) {
	function omniverse_shortcode_info_box( $atts, $content ) {
		...
		$atts = shortcode_atts( array(
			...
			'omniverse_hover_color_scheme' => 'light',
			'svg_animation' => '',
			'info_box_inline' => '',
			'omniverse_bg_position' => 'none',
		...
		if ( ! $omniverse_css_id ) $omniverse_css_id = uniqid();
		$id = 'wd-' . $omniverse_css_id;

		$class .= ' wd-info-box';
		$class .= omniverse_get_old_classes( ' omniverse-info-box' );
		if ( 'header' !== $source ) {
			$class .= ' wd-wpb';
		}
		...
		$class .= ' color-scheme-' . $omniverse_color_scheme;
		$class .= ' wd-bg-' . $omniverse_bg_position;
		...
		$subtitle_class .= ' ' . omniverse_get_new_size_classes( 'infobox', $title_size, 'subtitle' );

		if ( $style == 'bg-hover' ) $class .= ' color-scheme-hover-' . $omniverse_hover_color_scheme;
		...
		$title_class .= ' ' . omniverse_get_new_size_classes( 'infobox', $title_size, 'title' );
		$class .= omniverse_get_old_classes( ' box-title-' . $title_size );
		...
		$sizes = omniverse_get_explode_size( $img_size, 128 );

		ob_start();

		omniverse_enqueue_inline_style( 'info-box' );

		if ( 'border' === $style ) {
			omniverse_enqueue_inline_style( 'info-box-style-brd' );
		} elseif ( in_array( $style, array( 'shadow', 'bg-hover' ), true ) ) {
			omniverse_enqueue_inline_style( 'info-box-style-shadow-and-bg-hover' );
		}

		if ( ! empty( $btn_text ) && 'hover' === $btn_position ) {
			omniverse_enqueue_inline_style( 'info-box-btn-hover' );
		}
			...
			<div class="info-box-wrapper<?php echo esc_html( $wrapper_class ); ?>">
				<div id="<?php echo esc_attr( $id ); ?>" class="<?php echo esc_attr( $class ); ?>">
							...
							<?php if ( $icon_type == 'icon' ): ?>
								...
								<?php $i=0; foreach ($images as $img_id): $i++; ?>
									<?php
										...
										$image_output = omniverse_otf_get_image_html( $img_id, $img_size );

										if ( omniverse_is_svg( wp_get_attachment_image_url( $img_id ) ) && apply_filters( 'omniverse_show_infobox_svg_by_tag', true ) ) {
											if ( $svg_animation == 'yes' ) {
												omniverse_enqueue_js_library( 'vivus' );

												wp_add_inline_script('omniverse-theme', 'jQuery(document).ready(function($) {
											...
											echo '<div class="info-svg-wrapper info-icon" style="width: ' . $sizes[0] . 'px;height: ' . $sizes[1] . 'px;">' . omniverse_get_any_svg( $src, $rand ) . '</div>';
										} else {
											echo $image_output;
										}
									?>
					...
					<div class="info-box-content">
						...
						<?php
							if( ! empty( $btn_text ) ) {
								...
								echo omniverse_shortcode_button( array(
									'title'            => $btn_text,
									'link'             => $link,
									'color'            => $btn_color,
									'style'            => $btn_style,
					...
					<?php if ( $link_attributes && empty( $btn_text ) ) : ?>
						<a class="wd-info-box-link wd-fill" aria-label="<?php esc_html_e( 'Infobox link', 'omniverse' ); ?>" <?php echo wp_kses( $link_attributes, true ); ?>></a>
					<?php endif; ?>
					...
					if ( $bg_hover_color || $icon_text_color || $icon_bg_color || $icon_bg_hover_color || $icon_border_color || $icon_border_hover_color || $bg_hover_color_gradient || $title_color || $subtitle_custom_color || $subtitle_custom_bg_color || $custom_text_color || $bg_image_box || $bg_hover_image || $bg_color_gradient ) {
						...
						if ( $bg_image_box ) {
							...
							$bg_image_src = omniverse_otf_get_image_url( $bg_image_box, $bg_image_box_size );
						...
						if ( $bg_hover_image ) {
							...
							if ( 'wpb' === omniverse_get_current_page_builder() && ( in_array( $bg_hover_image_size, array( 'thumbnail', 'thumb', 'medium', 'large', 'full' ), true ) || ( is_string( $bg_hover_image_size ) && preg_match_all( '/\d+/', $bg_hover_image_size ) ) ) ) {
								$bg_hover_image_src = omniverse_otf_get_image_url( $bg_hover_image, $bg_hover_image_size );
							} else {
						...
						if ( $bg_hover_color ) {
							...
							if ( ! omniverse_is_css_encode( $bg_hover_color ) ) {
						...
						if ( $icon_text_color ) {
							...
							if ( ! omniverse_is_css_encode( $icon_text_color ) ) {
						...
						if ( $icon_bg_color || $icon_border_color ) {
							...
							if ( ! omniverse_is_css_encode( $icon_bg_color ) ) {
								$style .= 'background-color: ' . $icon_bg_color . ' !important;';
							}

							if ( ! omniverse_is_css_encode( $icon_border_color ) ) {
						...
						if ( $icon_bg_hover_color || $icon_border_hover_color ) {
							...
							if ( ! omniverse_is_css_encode( $icon_bg_hover_color ) ) {
								$style .= 'background-color: ' . $icon_bg_hover_color . ' !important;';
							}

							if ( ! omniverse_is_css_encode( $icon_border_hover_color ) ) {
						...
						if ( $bg_hover_colorpicker == 'gradient' ) {
							if ( $bg_color_gradient ) {
								$style .= '#' . $id . ' {' . omniverse_get_gradient_css( $bg_color_gradient ) . ' !important;}';
							}
							if ( $bg_hover_color_gradient ) {
								$style .= '#' . $id . ':after {' . omniverse_get_gradient_css( $bg_hover_color_gradient ) . ' !important;}';
							}
						}
						...
						if ( $title_color ) {
							...
							if ( ! omniverse_is_css_encode( $title_color ) ) {
						...
						if ( $subtitle_custom_color || $subtitle_custom_bg_color ) {
							...
							if ( ! omniverse_is_css_encode( $subtitle_custom_color ) ) {
								$style .= 'color: ' . $subtitle_custom_color . ' !important;';
							}

							if ( ! omniverse_is_css_encode( $subtitle_custom_bg_color ) ) {
						...
						if ( $custom_text_color ) {
							...
							if ( ! omniverse_is_css_encode( $custom_text_color ) ) {
					...
					echo apply_filters( 'omniverse_infobox_style', $style );
					?>
				</div>
			</div>
		<?php

		return apply_filters( 'vc_shortcode_output', ob_get_clean(), new WD_WPBakeryShortCodeFix(), $atts, 'omniverse_info_box' );
	}
}


if( ! function_exists( 'omniverse_shortcode_info_box_carousel' ) ) {
	function omniverse_shortcode_info_box_carousel( $atts = array(), $content = null ) {
		...
		$custom_sizes = apply_filters( 'omniverse_info_box_shortcode_custom_sizes', false );
		...
		$content = str_replace( '[omniverse_info_box', '[omniverse_info_box wrapper_classes="wd-carousel-item"', $content );
