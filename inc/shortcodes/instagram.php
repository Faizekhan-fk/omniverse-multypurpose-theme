...
if ( ! function_exists( 'omniverse_shortcode_instagram' ) ) {
	function omniverse_shortcode_instagram( $atts, $content = '' ) {
		$output          = '';
		$pics_classes    = '';
		$picture_classes = '';
		$carousel_atts   = '';
		...
		if ( ! empty( $parsed_atts['omniverse_css_id'] ) ) {
			$class .= ' wd-rs-' . $parsed_atts['omniverse_css_id'];
		}
		...
		if ( $design == 'slider' ) {
			...
			$custom_sizes = apply_filters( 'omniverse_instagram_shortcode_custom_sizes', false );
		...
		if ( 'images' === $data_source ) {
			$media_array = omniverse_get_instagram_custom_images( $images, $images_size, $images_link, $images_likes, $images_comments );
		} else {
			$media_array = omniverse_scrape_instagram( $username, $number, $ajax_body, $data_source );
		}
		...
		if ( is_wp_error( $media_array ) && ( $media_array->get_error_code() === 'invalid_response_429' || apply_filters( 'omniverse_intagram_user_ajax_load', false ) || 'ajax' === $data_source ) ) {
			omniverse_enqueue_js_script( 'instagram-element' );
			$class      .= ' wd-error';
			$media_array = array();
			$hide_mask   = true;
			for ( $i = 0; $i < $number; $i++ ) {
				$media_array[] = array(
					$size      => OMNIVERSE_ASSETS . '/images/settings/instagram/insta-placeholder.jpg',
					'link'     => '#',
					'likes'    => '0',
					'comments' => '0',
				);
		...
		omniverse_enqueue_inline_style( 'instagram' );
			...
			?>
				...
				<?php foreach ( $media_array as $item ) : ?>
					...
					<div class="wd-insta-item<?php echo esc_attr( $picture_classes ); ?>">
						<a href="<?php echo esc_url( $item['link'] ); ?>" target="<?php echo esc_attr( $target ); ?>" aria-label="<?php esc_attr_e( 'Instagram picture', 'omniverse' ); ?>"></a>
						...
						if ( isset( $item['image_id'] ) && $item['image_id'] ) {
							...
							echo apply_filters( 'omniverse_image', '<img src="' . esc_url( $image ) . '" alt="' . esc_attr__( 'Instagram image', 'omniverse' ) . '"/>' );
						}
						?>

						<?php if ( $hide_mask == 0 ) : ?>
							<div class="wd-insta-meta wd-grid-g">
								<span class="wd-insta-likes instagram-likes"><span><?php echo esc_attr( omniverse_pretty_number( $item['likes'] ) ); ?></span></span>
								<span class="wd-insta-comm instagram-comments"><span><?php echo esc_attr( omniverse_pretty_number( $item['comments'] ) ); ?></span></span>
							</div>
						<?php endif; ?>
					</div>
...
if ( ! function_exists( 'omniverse_pretty_number' ) ) {
	function omniverse_pretty_number( $x = 0 ) {
		$x = (int) $x;
...
if ( ! function_exists( 'omniverse_scrape_instagram' ) ) {
	function omniverse_scrape_instagram( $username, $slice = 9, $ajax_body = false, $data_source = 'scrape' ) {
		...
		if ( false === $instagram ) {
			if ( 'scrape' === $data_source || 'ajax' === $data_source ) {
				$instagram = omniverse_get_scrape_insta_images(
					array(
						'username'  => $username,
						'ajax_body' => $ajax_body,
					)
				...
				$instagram = omniverse_get_api_insta_images();
			}
			...
			if ( ! empty( $instagram ) ) {
				$instagram = function_exists( 'omniverse_compress' ) ? omniverse_compress( maybe_serialize( $instagram ) ) : '';
				set_transient( $transient_name, $instagram, apply_filters( 'null_instagram_cache_time', HOUR_IN_SECONDS * 2 ) );
			}
		}

		if ( ! empty( $instagram ) ) {
			$instagram = function_exists( 'omniverse_decompress' ) ? maybe_unserialize( omniverse_decompress( $instagram ) ) : array();
			return array_slice( $instagram, 0, $slice );
		} else {
			return new WP_Error( 'no_images', esc_html__( 'Instagram did not return any images.', 'omniverse' ) );
		}
	}
}

if ( ! function_exists( 'omniverse_get_api_insta_images' ) ) {
	function omniverse_get_api_insta_images() {
		$instagram_account_id   = get_option( 'instagram_account_id' );
		$instagram_access_token = get_option( 'instagram_access_token' );

		if ( ! $instagram_access_token || ! $instagram_account_id ) {
			return new WP_Error( 'no_token', esc_html__( 'You need connect your Instagram account in Theme Options -> General -> Connect instagram account', 'omniverse' ) );
		}
		...
		if ( is_object( $images_data_decoded ) ) {
			...
			return new WP_Error( 'no_images', esc_html__( 'Instagram API did not return any images.', 'omniverse' ) );
		}

		$instagram = array();

		foreach ( $images_data_decoded->data as $image ) {
			$caption = esc_html__( 'Instagram Image', 'omniverse' );
			...
			$instagram[] = array(
				...
				'image_id'    => zs_insert_image_from_url( $image_url ),
		...
		if ( $instagram && omniverse_get_opt( 'insta_delete_outdated_images' ) ) {
			...
			$attachments = new WP_Query(
				array(
					...
					'meta_query'     => array(
						array(
							'key'     => '_omniverse_instagram_image_name',
							'compare' => 'EXISTS',
						),
					),
				)
...
if ( ! function_exists( 'zs_insert_image_from_url' ) ) {
	/**
	 ...
	 * @return int|WP_Error
	 */
	function zs_insert_image_from_url( $url ) {
		...
		$img_by_slug = omniverse_get_image_id_by_slug( $img_name );

		if ( ! $img_by_slug ) {
			add_action( 'image_sideload_extensions', 'omniverse_get_instagram_image_sideload_extensions' );
			add_filter( 'intermediate_image_sizes', 'omniverse_get_instagram_insert_image_sizes', 10 );
			$upload = media_sideload_image( $url, 0, $img_name, 'id' );
			remove_action( 'intermediate_image_sizes', 'omniverse_get_instagram_insert_image_sizes', 10 );
			remove_action( 'image_sideload_extensions', 'omniverse_get_instagram_image_sideload_extensions' );

			if ( is_wp_error( $upload ) ) {
				return $upload->get_error_message();
			}

			update_post_meta( $upload, '_omniverse_instagram_image_name', $img_name );
...
if ( ! function_exists( 'omniverse_get_image_id_by_slug' ) ) {
	/**
	 ...
	 * @return int
	 */
	function omniverse_get_image_id_by_slug( $slug ) {
		if ( ! $slug ) {
			return '';
		}
		...
		$args = array(
			'post_type'      => 'attachment',
			'posts_per_page' => 1,
			'meta_key'       => '_omniverse_instagram_image_name',
			'meta_value'     => $slug,
		);
...
if ( ! function_exists( 'omniverse_get_instagram_insert_image_sizes' ) ) {
	/**
	 ...
	 * @return array
	 */
	function omniverse_get_instagram_insert_image_sizes() {
		return array( 'medium' );
	}
}

if ( ! function_exists( 'omniverse_get_instagram_image_sideload_extensions' ) ) {
	/**
	 ...
	 * @return array
	 */
	function omniverse_get_instagram_image_sideload_extensions( $allowed_extensions ) {
...
if ( ! function_exists( 'omniverse_get_scrape_insta_images' ) ) {
	function omniverse_get_scrape_insta_images( $data ) {
		...
		if ( ! $data['ajax_body'] ) {
			...
			if ( is_wp_error( $remote ) ) {
				return new WP_Error( 'site_down', esc_html__( 'Unable to communicate with Instagram.', 'omniverse' ) );
			}

			if ( 200 != wp_remote_retrieve_response_code( $remote ) ) {
				return new WP_Error( 'invalid_response_' . wp_remote_retrieve_response_code( $remote ), esc_html__( 'Instagram did not return a 200.', 'omniverse' ) );
			}
		...
		if ( ! isset( $shards[1] ) ) {
			return new WP_Error( 'bad_json', esc_html__( 'Instagram has returned invalid data.', 'omniverse' ) );
		}
		...
		if ( isset( $insta_array['entry_data']['ProfilePage'][0]['graphql']['user']['edge_owner_to_timeline_media']['edges'] ) ) {
			...
			return new WP_Error( 'bad_json_2', esc_html__( 'Instagram has returned invalid data.', 'omniverse' ) );
		}

		if ( ! is_array( $images ) ) {
			return new WP_Error( 'bad_array', esc_html__( 'Instagram has returned invalid data.', 'omniverse' ) );
		}
...
if ( ! function_exists( 'omniverse_instagram_ajax_query' ) ) {
	function omniverse_instagram_ajax_query() {
		if ( ! empty( $_POST['atts'] ) && ! empty( $_POST['body'] ) ) {
			$atts = omniverse_clean( $_POST['atts'] );

			$atts['ajax_body'] = trim( $_POST['body'] );
			$data              = omniverse_shortcode_instagram( $atts );

			wp_send_json( $data );
		}
	}

	add_action( 'wp_ajax_omniverse_instagram_ajax_query', 'omniverse_instagram_ajax_query' );
	add_action( 'wp_ajax_nopriv_omniverse_instagram_ajax_query', 'omniverse_instagram_ajax_query' );
}


if ( ! function_exists( 'omniverse_get_instagram_custom_images' ) ) {
	function omniverse_get_instagram_custom_images( $images, $size, $link, $likes, $comments ) {
		if ( ! $images ) {
			return new WP_Error( 'no_images', esc_html__( 'You need to upload your images manually to the element if you want to load them from your website. Otherwise you will need to connect your real Instagram account via API.', 'omniverse' ) );
		}
