...
if ( ! function_exists( 'omniverse_shortcode_accordion' ) ) {
	/***
	 ...
	 * @return false|string
	 */
	function omniverse_shortcode_accordion( $args, $content ) {
		...
		$content_data = explode( '[/omniverse_accordion_item]', $content );

		ob_start();

		omniverse_enqueue_js_script( 'accordion-element' );
		omniverse_enqueue_inline_style( 'accordion' );
		...
		<div id="<?php echo esc_attr( $id ); ?>" class="wd-accordion wd-wpb<?php echo esc_attr( $wrapper_classes ); ?>" data-state="<?php echo esc_attr( $args['state'] ); ?>">
			<?php foreach ( $content_data as $index => $data ) : ?>
				...
				<?php
				$shortcode         = $data . '[/omniverse_accordion_item]';
				$shortcode_data    = omniverse_get_accordion_data( $shortcode );
				...
				if ( ! empty( $shortcode_data[0]['image'] ) ) {
					$icon_output = omniverse_display_icon( $shortcode_data[0]['image'], $shortcode_data[0]['image_size'], 128 ); // phpcs:ignore

					if ( omniverse_is_svg( wp_get_attachment_image_src( $shortcode_data[0]['image'] )[0] ) ) {
						$icon_output = omniverse_get_svg_html( $shortcode_data[0]['image'], $shortcode_data[0]['image_size'] );
					}
				...
				<div class="wd-accordion-item">
					...
					<div class="wd-accordion-content reset-last-child<?php echo esc_attr( $loop_content_classes_wrapper ); ?>" data-accordion-index="<?php echo esc_attr( $index ); ?>">
						<?php if ( 'html_block' === $content_type ) : ?>
							<?php echo omniverse_get_html_block( $shortcode_data[0]['html_block_id'] ); // phpcs:ignore ?>
...
if ( ! function_exists( 'omniverse_shortcode_accordion_item' ) ) {
	/**
	 ...
	 * @return false|string
	 */
	function omniverse_shortcode_accordion_item( $args, $content ) {
		ob_start();
...
if ( ! function_exists( 'omniverse_get_accordion_data' ) ) {
	/**
	 ...
	 */
	function omniverse_get_accordion_data( $content ) {
		preg_match_all( '/omniverse_accordion([^\]]+)/i', $content, $matches, PREG_OFFSET_CAPTURE );
