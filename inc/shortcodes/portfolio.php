...
if ( ! function_exists( 'omniverse_shortcode_portfolio' ) ) {
	function omniverse_shortcode_portfolio( $atts ) {
		...
		omniverse_set_loop_prop( 'portfolio_style', $style );
		omniverse_set_loop_prop( 'portfolio_column', $columns );
		omniverse_set_loop_prop( 'portfolio_image_size', $image_size );

		if ( 'auto' !== $columns_tablet ) {
			omniverse_set_loop_prop( 'portfolio_columns_tablet', $columns_tablet );
		}
		if ( 'auto' !== $columns_mobile ) {
			omniverse_set_loop_prop( 'portfolio_columns_mobile', $columns_mobile );
		}
		...
		$parsed_atts['custom_sizes'] = apply_filters( 'omniverse_portfolio_shortcode_custom_sizes', false );
		...
		if ( 'carousel' === $layout ) {
			...
			return omniverse_generate_posts_slider( $parsed_atts, $query );
		}
			...
			<?php if ( ! $is_ajax ) : ?>
				<div class="wd-portfolio-element<?php echo esc_attr( $wrapper_classes ); ?>">
					...
					<?php if ( ! is_tax() && $filters && ! $s ) : ?>
						<?php omniverse_portfolio_filters( $categories, $filters_type ); ?>
					<?php endif ?>
					...
					<?php if ( $query->max_num_pages > 1 && ! $is_ajax && $pagination != 'disable' && $layout != 'carousel' ) : ?>
						...
						<div class="wd-loop-footer portfolio-footer">
							<?php if ( $pagination == 'infinit' || $pagination == 'load_more' ) : ?>
								...
								<a href="#" rel="nofollow noopener" class="btn wd-load-more wd-portfolio-load-more load-on-<?php echo 'load_more' === $pagination ? 'click' : 'scroll'; ?>"><span class="load-more-label"><?php esc_html_e( 'Load more projects', 'omniverse' ); ?></span></a>
