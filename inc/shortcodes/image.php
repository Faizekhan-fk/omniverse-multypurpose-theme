...
if ( ! function_exists( 'omniverse_shortcode_image' ) ) {
	function omniverse_shortcode_image( $atts ) {
		...
		$title_id = 'wd-' . $atts['omniverse_css_id'];
		...
		$image_html = omniverse_otf_get_image_html( $atts['img_id'], $atts['img_size'] );
		...
		if ( isset( $image_data[0] ) && omniverse_is_svg( $image_data[0] ) ) {
			$image_html = omniverse_get_svg_html(
				$atts['img_id'],
				$atts['img_size']
			);
		}
		...
		return apply_filters( 'vc_shortcode_output', ob_get_clean(), new WD_WPBakeryShortCodeFix(), $atts, 'omniverse_image' );
	}
}
