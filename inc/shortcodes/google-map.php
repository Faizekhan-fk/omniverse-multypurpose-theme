...
if ( ! function_exists( 'omniverse_get_settings_coords_for_google_map' ) ) {
	/**
	 ...
	 */
	function omniverse_get_settings_coords_for_google_map( $markers ) {
...
if ( ! function_exists( 'omniverse_shortcode_google_map' ) ) {
	/***
	 ...
	 * @return false|string
	 */
	function omniverse_shortcode_google_map( $atts, $content ) {
		...
		if ( isset( $atts['multiple_markers'] ) && 'yes' === $atts['multiple_markers'] ) {
			$markers = array_map(
				function ( $marker ) {
					...
					if ( is_numeric( $marker['image'] ) ) {
						$thumb_size = omniverse_get_image_size( $marker['image_size'] );
			...
			$coords = omniverse_get_settings_coords_for_google_map( $markers );
		}
		...
		wp_enqueue_script( 'wd-google-map-api', 'https://maps.google.com/maps/api/js?libraries=geometry&callback=omniverseThemeModule.googleMapsCallback&v=weekly&key=' . $google_key, array(), $version, true );
		...
		$el_class                .= omniverse_get_css_animation( $css_animation );
		...
		$map_args = array(
			...
			'elementor'          => false,
			'json_style'         => rawurldecode( omniverse_decompress( $style_json ) ),
		...
		if ( $marker_icon ) {
			$thumb_size = omniverse_get_image_size( $marker_icon_size );
		...
		if ( $image_id ) {
			$placeholder = omniverse_otf_get_image_html( $image_id, $image_size );
		}

		$style_attr = ! $new_height && 0 < $height ? 'style="height: ' . esc_attr( $height ) . 'px"' : '';
		$el_class  .= ' wd-rs-' . $omniverse_css_id;
		ob_start();
