 ...
 * ------------------------------------------------------------------------------------------------
 */
if ( ! function_exists( 'omniverse_shortcode_categories' ) ) {
	function omniverse_shortcode_categories( $atts, $content ) {
		...
		if ( ! empty( $img_size ) ) {
			omniverse_set_loop_prop( 'product_categories_image_size', $img_size );
		}
		...
		if ( 'navigation' === $type ) {
			$wrapper_classes  = ' text-' . omniverse_vc_get_control_data( $nav_alignment, 'desktop' );
			...
			<div class="<?php echo esc_attr( $extra_wrapper_classes ); ?>">
				<div class="<?php echo esc_attr( $wrapper_classes ); ?>">
					<?php omniverse_product_categories_nav( $args, $parsed_atts ); ?>
				</div>
			</div>
			<?php
			return ob_get_clean();
		...
		if ( omniverse_is_compressed_data( $columns ) ) {
			$columns_desktop = omniverse_vc_get_control_data( $columns, 'desktop' );
			$columns_tablet  = omniverse_vc_get_control_data( $columns, 'tablet' );
			$columns_mobile  = omniverse_vc_get_control_data( $columns, 'mobile' );
		} else {
			$columns_desktop = absint( $columns );
		}

		omniverse_set_loop_prop( 'product_categories_color_scheme', $color_scheme );
		...
		if ( 'masonry' === $style || 'masonry-first' === $style ) {
			...
			if ( ! empty( $grid_different_sizes ) ) {
				omniverse_set_loop_prop( 'grid_items_different_sizes', explode( ',', $grid_different_sizes ) );
			}
		}
		...
		omniverse_set_loop_prop( 'product_categories_design', $categories_design );
		omniverse_set_loop_prop( 'product_categories_shadow', $categories_with_shadow );
		omniverse_set_loop_prop( 'product_categories_style', $style );

		if ( isset( $columns_desktop ) ) {
			omniverse_set_loop_prop( 'products_columns', $columns_desktop );
		}
		...
		if ( $product_categories ) {
			...
			if ( 'carousel' === $style ) {
				...
				omniverse_set_loop_prop( 'category_extra_classes', 'wd-carousel-item' );
				...
				<div id="<?php echo esc_attr( $carousel_id ); ?>" class="products woocommerce wd-carousel-container <?php echo esc_attr( $extra_wrapper_classes . $extra_class ); ?>">
					<div class="wd-carousel-inner">
						<div class="wd-carousel wd-grid<?php echo esc_attr( $carousel_classes ); ?>" <?php echo omniverse_get_carousel_attributes( $parsed_atts ); ?>>
