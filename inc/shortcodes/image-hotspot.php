...
if ( ! function_exists( 'omniverse_image_hotspot_shortcode' ) ) {
	function omniverse_image_hotspot_shortcode( $atts, $content ) {
		$image   = '';
		$video_attr = '';
		...
		$classes .= ' color-scheme-' . $atts['omniverse_color_scheme'];
		...
		if ( 'video' === $atts['source_type'] && $atts['video_poster'] ) {
			$video_attr .= ' poster="' . omniverse_otf_get_image_url( $atts['video_poster'], $atts['video_poster_size'] ) . '"';
		}
		...
		<div class="wd-image-hotspot-wrapper<?php echo esc_attr( $classes ); ?>">
			<div class="wd-image-hotspot-hotspots">
				...
				if ( 'image' === $atts['source_type'] && $atts['img'] ) {
					echo omniverse_otf_get_image_html( $atts['img'], $atts['img_size'], array(), array( 'class' => 'wd-image-hotspot-img' ) ); //phpcs:ignore
...
if ( ! function_exists( 'omniverse_hotspot_shortcode' ) ) {
	function omniverse_hotspot_shortcode( $atts, $content ) {
		...
		if ( $product_id && omniverse_woocommerce_installed() ) {
		...
		if ( $hotspot_type == 'product' && $product ) {
			...
			if ( wc_review_ratings_enabled() ) {
				...
				$output .= '<div class="hotspot-content-text wd-more-desc set-cont-mb-s reset-last-child' . omniverse_get_old_classes( ' omniverse-more-desc' ) . '"><div class="wd-more-desc-inner' . omniverse_get_old_classes( ' omniverse-more-desc-inner' ) . '">' . do_shortcode( $product->get_short_description() ) . '</div></div>';
		...
		if ( $hotspot_type == 'text' && ( $title || $content || $link_text ) ) {
			if ( $link ) {
				$attributes = omniverse_get_link_attributes( $link );
			}

			$image = omniverse_otf_get_image_html( $img, $img_size, array(), array( 'class' => 'wd-image-hotspot-img' ) );
			...
			if ( $content ) {
				$output .= '<div class="hotspot-content-text set-cont-mb-s reset-last-child">' . $content . '</div>';
			}
			if ( $link_text && $link ) {
				$output .= '<a class="btn btn-color-primary btn-size-small" ' . $attributes . '>' . esc_html( $link_text ) . '</a>';
			}
			$output .= '</div>';
		}
		...
		echo '<div class="wd-image-hotspot' . esc_attr( $classes ) . '" style="left: ' . esc_attr( $left ) . '%; top: ' . esc_attr( $top ) . '%;">';
			...
			echo apply_filters( 'omniverse_hotspot_content', $output );
		echo '</div>';

	}
}

if ( ! function_exists( 'omniverse_get_hotspot_image' ) ) {
	function omniverse_get_hotspot_image() {
		check_ajax_referer( 'omniverse-get-hotspot-image-nonce', 'security' );

		$source_id = sanitize_text_field( $_GET[ 'image_id' ] ); //phpcs:ignore
		$html      = '';

		if ( false !== strpos( get_post_mime_type( $source_id ), 'video' ) ) {
			$html = '<video class="dn-hotspot-img" src="' . wp_get_attachment_url( $source_id ) . '" autoplay loop muted playsinline></video>';
		} else {
			$background_image = wp_get_attachment_image_src( $source_id, 'full' );

			if ( $background_image ) {
				$html = '<img class="dn-hotspot-img" src="' . esc_url( $background_image[0] ) . '" width="' . esc_attr( $background_image[1] ) . '" height="' . esc_attr( $background_image[2] ) . '">';
			}
		}
		...
		if ( ! $html ) {
			$response = array(
				'status' => 'warning',
				'html'   => '<div class="omniverse-warning">' . esc_html__( 'You need to upload an image for the parent element first.', 'omniverse' ) . '</div>',
			);
	...
	add_action( 'wp_ajax_omniverse_get_hotspot_image', 'omniverse_get_hotspot_image' );
}
