...
if ( ! function_exists( 'omniverse_shortcode_button' ) ) {
	function omniverse_shortcode_button( $atts, $popup = false ) {
		...
		$attributes = omniverse_get_link_attributes( $link, $popup );
		...
		$wrap_class .= omniverse_get_old_classes( ' omniverse-button-wrapper' );
		$wrap_class .= omniverse_get_css_animation( $css_animation );
		...
		if ( 'yes' === $button_smooth_scroll ) {
			omniverse_enqueue_js_script( 'button-element' );
		...
		if ( 'yes' === $wd_button_collapsible_content ) {
			omniverse_enqueue_js_script( 'button-show-more' );
		...
		if ( 'icon' === $icon_type && ${'icon_' . $icon_library} ) {
			...
			if ( omniverse_is_svg( wp_get_attachment_image_url( $image ) ) ) {
				$image_output = omniverse_get_svg_html( $image, $img_size );
			} elseif ( ! empty( $image['id'] ) ) {
				$image_output = omniverse_otf_get_image_html( $image['id'], $img_size );
			} else {
				$image_output = omniverse_otf_get_image_html( $image, $img_size );
			}
		...
		if ( $bg_color && ! omniverse_is_css_encode( $bg_color ) || $bg_color_hover && ! omniverse_is_css_encode( $bg_color_hover ) ) {
			$css = '';
			// Custom Color
			$css .= '#' . $id . ' a {';
