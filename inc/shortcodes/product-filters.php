...
if ( ! function_exists( 'omniverse_product_filters_shortcode' ) ) {
	function omniverse_product_filters_shortcode( $atts, $content ) {
		global $wp;
		...
		$atts['space_between_tablet']     = omniverse_vc_get_control_data( $atts['space_between'], 'tablet' );
		$atts['space_between_mobile']     = omniverse_vc_get_control_data( $atts['space_between'], 'mobile' );
		$atts['space_between']            = omniverse_vc_get_control_data( $atts['space_between'], 'desktop' );
		$atts['display_grid_col_desktop'] = omniverse_vc_get_control_data( $atts['display_grid_col'], 'desktop' );
		$atts['display_grid_col_tablet']  = omniverse_vc_get_control_data( $atts['display_grid_col'], 'tablet' );
		$atts['display_grid_col_mobile']  = omniverse_vc_get_control_data( $atts['display_grid_col'], 'mobile' );

		Global_Data::get_instance()->set_data( 'omniverse_product_filters_attr', $atts );
		...
		if ( ! empty( $omniverse_color_scheme ) ) {
			$classes .= ' color-scheme-' . $omniverse_color_scheme;
		}
		...
		$classes .= omniverse_get_old_classes( ' omniverse-product-filters' );
			...
			<form action="<?php echo esc_url( $form_action ); ?>" class="wd-product-filters<?php echo esc_attr( $classes ); ?>" method="GET" style="<?php echo esc_attr( $style_attrs ); ?>">
				...
				<?php if ( 'click' === $submit_form_on ) : ?>
					<div class="wd-pf-btn wd-col">
						<button type="submit">
							<?php echo esc_html__( 'Filter', 'omniverse' ); ?>
						</button>
					</div>
				<?php endif; ?>
			</form>
		...
		return apply_filters( 'vc_shortcode_output', $output, new WD_WPBakeryShortCodeFix(), $atts, 'omniverse_product_filters' );
	}
}
...
* ------------------------------------------------------------------------------------------------
*/
if ( ! function_exists( 'omniverse_filters_categories_shortcode' ) ) {
	function omniverse_filters_categories_shortcode( $atts, $content ) {
		global $wp_query, $post;

		$omniverse_product_filters_attr = Global_Data::get_instance()->get_data( 'omniverse_product_filters_attr' );
		$classes                       = '';
		...
		ob_start();
		?>
			<div class="wd-pf-checkboxes wd-col wd-pf-categories wd-event-<?php echo esc_attr( $omniverse_product_filters_attr['show_dropdown_on'] ); ?>">
			<div class="wd-pf-title">
				<span class="title-text">
					<?php echo esc_html( $title ); ?>
				</span>
				<?php if ( 'yes' === $omniverse_product_filters_attr['show_selected_values'] ) : ?>
...
if ( ! function_exists( 'omniverse_stock_status_shortcode' ) ) {
	function omniverse_stock_status_shortcode( $atts ) {
		$omniverse_product_filters_attr = Global_Data::get_instance()->get_data( 'omniverse_product_filters_attr' );
		...
		ob_start();
		?>
			<div class="wd-pf-checkboxes wd-col wd-pf-stock multi_select wd-event-<?php echo esc_attr( $omniverse_product_filters_attr['show_dropdown_on'] ); ?>">
...
if ( ! function_exists( 'omniverse_filters_attribute_shortcode' ) ) {
	function omniverse_filters_attribute_shortcode( $atts, $content ) {
		$omniverse_product_filters_attr = Global_Data::get_instance()->get_data( 'omniverse_product_filters_attr' );
		$classes                       = '';
		...
		extract(
			shortcode_atts(
				array(
					'title'        => esc_html__( 'Filter by', 'omniverse' ),
					'attribute'    => '',
					'categories'   => '',
					'query_type'   => 'and',
					'size'         => 'normal',
		...
		the_widget(
			...
			array(
				...
				'show_selected_values' => isset( $omniverse_product_filters_attr['show_selected_values'] ) ? $omniverse_product_filters_attr['show_selected_values'] : 'yes',
				'show_dropdown_on'     => isset( $omniverse_product_filters_attr['show_dropdown_on'] ) ? $omniverse_product_filters_attr['show_dropdown_on'] : 'yes',
			),
			array(
				'before_widget' => '',
				'after_widget'  => '',
...
* ------------------------------------------------------------------------------------------------
*/
if ( ! function_exists( 'omniverse_filters_price_slider_shortcode' ) ) {
	function omniverse_filters_price_slider_shortcode( $atts, $content ) {
		global $wpdb;
		...
		extract(
			shortcode_atts(
				array(
					'title'    => esc_html__( 'Filter by price', 'omniverse' ),
					'el_class' => '',
				),
				$atts
			)
		...
		if ( function_exists( 'WC' ) && version_compare( WC()->version, '3.6.0', '<' ) ) {
			$prices = omniverse_get_filtered_price();
		} else {
		...
		?>
		<div class="wd-pf-checkboxes wd-col wd-pf-price-range multi_select widget_price_filter wd-event-<?php echo esc_attr( $omniverse_product_filters_attr['show_dropdown_on'] ); ?>">
			<div class="wd-pf-title">
				<span class="title-text">
					<?php echo esc_html( $title ); ?>
				</span>
			...
			<div class="wd-pf-dropdown wd-dropdown">
				<div class="price_slider_wrapper">
					...
					<div class="filter_price_slider_amount">
						...
						<?php if ( 'select' === $omniverse_product_filters_attr['submit_form_on'] ) : ?>
...
if ( ! function_exists( 'omniverse_get_filtered_price' ) ) {
	function omniverse_get_filtered_price() {
		global $wpdb;
...
if ( ! function_exists( 'omniverse_orderby_filter_template' ) ) {
	function omniverse_orderby_filter_template() {
		$omniverse_product_filters_attr = Global_Data::get_instance()->get_data( 'omniverse_product_filters_attr' );
		...
		ob_start();
		?>
		<div class="wd-pf-checkboxes wd-col wd-pf-sortby wd-event-<?php echo esc_attr( $omniverse_product_filters_attr['show_dropdown_on'] ); ?>">
