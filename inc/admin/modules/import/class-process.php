...
class Process {
	 ...
	 */
	public function __construct( $version, $current_process, $type, $business_type ) {
	...
	public function run_import() {		
		if ( $this->need_process( 'xml' ) && 'xml' === $this->current_process && ( ! $this->is_version_imported || ( 'page' === $this->type || 'element' === $this->type ) ) ) {
			...
			if ($this->business_type === 'ecommerce') {
				if ( 'base' === $this->type ) {
					Before::get_instance()->init($this->business_type);
				}
			}
		...
		if ( 'other' === $this->current_process ) {
			...
			update_option( 'wd_import_theme_version', omniverse_get_theme_info( 'Version' ), false );
			...
			do_action( 'omniverse_after_import' );
		}
	}
