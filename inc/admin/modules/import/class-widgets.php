...
class Widgets {
	...
	private function import_widgets() {
		...
		if ( 'elementor' === omniverse_get_current_page_builder() && file_exists( $this->helpers->get_version_folder_path( $this->version ) . 'widgets-elementor.json' ) ) {
	...
	private function widgets_replace_html_block_and_menu_ids( $widgets_data ) {
		...
		foreach ( $widgets_data as $area_name => $area_settings ) {
			foreach ( $area_settings['widgets'] as $widget_name => $widget_settings ) {
				if ( strstr( $widget_name, 'omniverse-html-block' ) ) {
				...
				if ( strstr( $widget_name, 'omniverse-author-information' ) || strstr( $widget_name, 'omniverse-banner' ) ) {
				...
				if ( strstr( $widget_name, 'omniverse-instagram' ) ) {
