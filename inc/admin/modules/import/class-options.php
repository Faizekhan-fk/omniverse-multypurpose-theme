...
class Options {
	...
	private function import_options_presets() {
		...
		if ( 'elementor' === omniverse_get_current_page_builder() && file_exists( $this->helpers->get_version_folder_path( $this->version ) . 'options_presets-elementor.json' ) ) {
		...
		$new_options = $zs_omniverse_options + json_decode( $new_options_json, true );
	...
	private function import_presets() {
		...
		if ( 'elementor' === omniverse_get_current_page_builder() && file_exists( $this->helpers->get_version_folder_path( $this->version ) . 'presets-elementor.json' ) ) {
	...
	private function import_options() {
		...
		if ( 'elementor' === omniverse_get_current_page_builder() && file_exists( $this->helpers->get_version_folder_path( $this->version ) . 'options-elementor.json' ) ) {
		...
		$zs_omniverse_options['variation_gallery_storage_method'] = 'new';

		// Merge new options with other existed ones.
		$new_options = $new_options + $zs_omniverse_options;
		...
		foreach ( Presets::get_all() as $preset ) {
			delete_option( 'dn-theme_settings_' . $preset['id'] . '-file-data' );
			delete_option( 'dn-theme_settings_' . $preset['id'] . '-css-data' );
			delete_option( 'dn-theme_settings_' . $preset['id'] . '-version' );
			delete_option( 'dn-theme_settings_' . $preset['id'] . '-site-url' );
			delete_option( 'dn-theme_settings_' . $preset['id'] . '-status' );
			delete_option( 'dn-theme_settings_' . $preset['id'] . '-credentials' );
		}

		delete_option( 'dn-theme_settings_default-file-data' );
		delete_option( 'dn-theme_settings_default-css-data' );
		delete_option( 'dn-theme_settings_default-version' );
		delete_option( 'dn-theme_settings_default-site-url' );
		delete_option( 'dn-theme_settings_default-status' );
		delete_option( 'dn-theme_settings_default-credentials' );
	...
	private function get_reset_options() {
		...
		$reset_options_keys = omniverse_get_config( 'reset-options-' . $version_type );
	...
	private function theme_settings_replace_post_ids() {
		...
		update_option( 'dn-omniverse-options', $theme_options );
	}
