...
class Remove extends Singleton {
	...
	public function init() {
		...
		add_action( 'wp_ajax_omniverse_import_remove_action', array( $this, 'remove_action' ) );
	}
	...
	private function set_versions() {
		$versions = omniverse_get_config( 'versions' );
	...
	public function remove_action() {
		check_ajax_referer( 'omniverse-import-remove-nonce', 'security' );

		if ( ! isset( $_GET['data'] ) || ! $_GET['data'] ) { // phpcs:ignore
			return;
		}

		$selected_categories_raw = omniverse_clean( wp_unslash( $_GET['data'] ) ); // phpcs:ignore
		...
		foreach ( $this->versions as $version ) {
			...
			if ( in_array( 'omniverse_layout', $selected_categories, true ) ) {
				$imported_data = $this->delete_posts( array( 'omniverse_layout' ), $imported_data );

				$this->categories['omniverse_layout']['data'] = array();
			}
			...
			if ( in_array( 'omniverse_slider', $selected_categories, true ) ) {
				$imported_data = $this->delete_terms( array( 'omniverse_slider' ), $imported_data );
				$imported_data = $this->delete_posts( array( 'omniverse_slide' ), $imported_data );

				$this->categories['omniverse_slider']['data'] = array();
			}
	...
	private function delete_headers( $imported_data ) {
		...
		foreach ( $imported_data['headers'] as $key => $header_id ) {
			...
			delete_option( 'dn-' . $header_id . '-file-data' );
			delete_option( 'dn-' . $header_id . '-css-data' );
			delete_option( 'dn-' . $header_id . '-version' );
			delete_option( 'dn-' . $header_id . '-site-url' );
			delete_option( 'dn-' . $header_id . '-status' );
			delete_option( 'zs_' . $header_id );
		}
	...
	public function render() {
		?>
		<div class="dn-popup-holder dn-import-remove">
			<a href="#" class="dn-popup-opener dn-import-remove-opener dn-bordered-btn dn-color-warning dn-i-trash">
				<?php esc_html_e( 'Remove content', 'omniverse' ); ?>
			</a>
			...
			<div class="dn-popup dn-theme-style">
				<div class="dn-popup-inner">
					...
					<div class="dn-popup-content dn-import-remove-content ">
						<div class="dn-notices-wrapper dn-import-notices dn-import-remove-notices"></div>

						<div class="dn-import-remove-form-wrap">
							<?php $this->popup_content( false, 'import' ); ?>
						</div>
					</div>

					<div class="dn-popup-actions"> </div>
				</div>
			</div>
		</div>
		<?php
	...
	public function popup_content( $ajax, $action ) {
		...
		?>
		<form class="dn-import-remove-form" method="post" action="#">
			<div class="dn-import-select-btns">
				<button class="dn-import-remove-select dn-inline-btn dn-i-check-all">
					<?php esc_html_e( 'Check all', 'omniverse' ); ?>
				</button>

				<button class="dn-import-remove-deselect dn-inline-btn dn-i-uncheck-all">
					<?php esc_html_e( 'Uncheck all', 'omniverse' ); ?>
				</button>
			</div>
			...
			<?php foreach ( $this->categories as $key => $category ) : ?>
				...
				?>
				<div class="dn-import-remove-items">
			...
			<div class="dn-import-remove-alert">
				<p>
					<?php echo wp_kses( __( '<strong>Warning</strong>. You are going to completely delete the content imported with our theme. This process cannot be undone. Any changes you made in import pages, posts, or products will be lost.', 'omniverse' ), omniverse_get_allowed_html() ); ?>
				</p>

				<button type="submit" class="dn-import-remove-btn dn-btn dn-color-warning dn-i-trash">
					<?php esc_html_e( 'Remove', 'omniverse' ); ?>
				</button>
			</div>
		</form>

		<div class="dn-import-remove-img">
			<img src="<?php echo esc_url( OMNIVERSE_ASSETS_IMAGES . '/dn-import-image.jpg' ); ?>" alt="<?php esc_attr_e( 'Remove image', 'omniverse' ); ?>">
		</div>
		<?php
		if ( $ajax ) {
			return ob_get_clean();
	...
	private function set_categories() {
		$this->categories = array(
			'page'               => array(
				'title' => esc_html__( 'Pages', 'omniverse' ),
				'data'  => array(),
			),
			'rev_sliders'        => array(
				'title' => esc_html__( 'Revolution sliders', 'omniverse' ),
				'data'  => array(),
			),
			'product'            => array(
				'title' => esc_html__( 'Products', 'omniverse' ),
				'data'  => array(),
			),
			'mc4wp-form'         => array(
				'title' => esc_html__( 'Mailchimp forms', 'omniverse' ),
				'data'  => array(),
			),
			'post'               => array(
				'title' => esc_html__( 'Posts', 'omniverse' ),
				'data'  => array(),
			),
			'omniverse_layout'    => array(
				'title' => esc_html__( 'Layouts', 'omniverse' ),
				'data'  => array(),
			),
			'omniverse_slider'    => array(
				'title' => esc_html__( 'Omniverse sliders', 'omniverse' ),
				'data'  => array(),
			),
			'portfolio'          => array(
				'title' => esc_html__( 'Projects', 'omniverse' ),
				'data'  => array(),
			),
			'presets'            => array(
				'title' => esc_html__( 'Settings presets', 'omniverse' ),
				'data'  => array(),
			),
			'cms_block'          => array(
				'title' => esc_html__( 'HTML blocks', 'omniverse' ),
				'data'  => array(),
			),
			'headers'            => array(
				'title' => esc_html__( 'Headers', 'omniverse' ),
				'data'  => array(),
			),
			'attachment'         => array(
			...
			'wpcf7_contact_form' => array(
				'title' => esc_html__( 'Contact forms', 'omniverse' ),
				'data'  => array(),
			),
		);
	}
	...
	private function set_categories_data() {
		...
		if ( $presets ) {
			...
			$this->categories['presets']['data'] = get_option( 'dn-options-presets' );
		}
		...
		foreach ( $this->versions as $version ) {
			...
			if ( ! empty( $imported_data['omniverse_layout'] ) ) {
				$this->has_data_to_remove                    = true;
				$this->categories['omniverse_layout']['data'] = $this->categories['omniverse_layout']['data'] + $imported_data['omniverse_layout'];
			}
			...
			if ( ! empty( $imported_data['term']['omniverse_slider'] ) ) {
				$this->has_data_to_remove                    = true;
				$this->categories['omniverse_slider']['data'] = $this->categories['omniverse_slider']['data'] + $imported_data['term']['omniverse_slider'];
			}
