...
class After extends Singleton {
	...
	public function change_header_on_pages() {
		...
		foreach ( $pages as $page_slug ) {
			...
			if ( isset( $page_data['ID'] ) ) {
				update_post_meta( $page_data['ID'], '_omniverse_whb_header', $header );
			}
		}
	}
	...
	public function set_pages_sidebar() {
		$pages = apply_filters(
			...
			array(
				'cart'      => array(
					'name'    => _x( 'cart', 'Page slug', 'omniverse' ),
					'title'   => _x( 'Cart', 'Page title', 'omniverse' ),
					'content' => '[' . apply_filters( 'woocommerce_cart_shortcode_tag', 'woocommerce_cart' ) . ']',
				),
				'checkout'  => array(
					'name'    => _x( 'checkout', 'Page slug', 'omniverse' ),
					'title'   => _x( 'Checkout', 'Page title', 'omniverse' ),
		...
		foreach ( $pages as $key => $page ) {
			...
			update_post_meta( $page_id, '_omniverse_main_layout', 'full-width' );
		}
	...
	public function enable_wpb_on_custom_post_types() {
		...
		$vc_roles->save(
			array(
				'administrator' => json_decode( '{"post_types":{"_state":"custom","post":"1","page":"1","omniverse_slide":"1","omniverse_layout":"1","omniverse_size_guide":"1","cms_block":"1","omniverse_sidebar":"0","portfolio":"1","product":"1","omniverse_woo_lv":"0"},"backend_editor":{"_state":"1","disabled_ce_editor":"0"},"frontend_editor":{"_state":"1"},"post_settings":{"_state":"1"},"settings":{"_state":"1"},"templates":{"_state":"1"},"shortcodes":{"_state":"1"},"grid_builder":{"_state":"1"},"presets":{"_state":"1"}}' ),
			)
		);
	}
