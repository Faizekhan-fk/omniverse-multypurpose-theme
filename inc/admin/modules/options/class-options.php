...
class Options extends Singleton {
	 ...
	 * @var array
	 */
	public static $opt_name = 'omniverse';
	...
	public static $_controls_classes = array(
		'select'            => 'DN\Admin\Modules\Options\Controls\Select',
		'text_input'        => 'DN\Admin\Modules\Options\Controls\Text_Input',
		'switcher'          => 'DN\Admin\Modules\Options\Controls\Switcher',
		'color'             => 'DN\Admin\Modules\Options\Controls\Color',
		'checkbox'          => 'DN\Admin\Modules\Options\Controls\Checkbox',
		'buttons'           => 'DN\Admin\Modules\Options\Controls\Buttons',
		'upload'            => 'DN\Admin\Modules\Options\Controls\Upload',
		'upload_list'       => 'DN\Admin\Modules\Options\Controls\Upload_List',
		'background'        => 'DN\Admin\Modules\Options\Controls\Background',
		'textarea'          => 'DN\Admin\Modules\Options\Controls\Textarea',
		'image_dimensions'  => 'DN\Admin\Modules\Options\Controls\Image_Dimensions',
		'typography'        => 'DN\Admin\Modules\Options\Controls\Typography',
		'custom_fonts'      => 'DN\Admin\Modules\Options\Controls\Custom_Fonts',
		'range'             => 'DN\Admin\Modules\Options\Controls\Range',
		'editor'            => 'DN\Admin\Modules\Options\Controls\Editor',
		'import'            => 'DN\Admin\Modules\Options\Controls\Import',
		'notice'            => 'DN\Admin\Modules\Options\Controls\Notice',
		'instagram_api'     => 'DN\Admin\Modules\Options\Controls\Instagram_Api',
		'reset'             => 'DN\Admin\Modules\Options\Controls\Reset',
		'sorter'            => 'DN\Admin\Modules\Options\Controls\Sorter',
		'select_with_table' => 'DN\Admin\Modules\Options\Controls\Select_With_Table',
		'responsive_range'  => 'DN\Admin\Modules\Options\Controls\Responsive_Range',
		'icons_font'        => 'DN\Admin\Modules\Options\Controls\Icons_Font',
		'dimensions'        => 'DN\Admin\Modules\Options\Controls\Dimensions',
		'group'             => 'DN\Admin\Modules\Options\Controls\Group',
	);
	...
	public function include_files() {
		...
		foreach ( $classes_files as $file ) {
			require_once get_parent_theme_file_path( OMNIVERSE_FRAMEWORK . '/admin/modules/options/' . $file . '.php' );
		}

		foreach ( self::$_controls_classes as $key => $class_name ) {
			$control_name = str_replace( '_', '-', $key );

			require_once get_parent_theme_file_path( OMNIVERSE_FRAMEWORK . '/admin/modules/options/controls/' . $control_name . '/class-' . $control_name . '.php' );
		}
	}
	...
	public function set_options_for_post() {
		global $omniverse_options;

		$custom_options = json_decode( get_post_meta( omniverse_page_ID(), '_omniverse_options', true ), true );

		if ( ! empty( $custom_options ) ) {
			$omniverse_options = wp_parse_args( $custom_options, $omniverse_options );
		}

		$omniverse_options = apply_filters( 'omniverse_global_options', $omniverse_options );
	}
	...
	public function set_custom_meta_for_post() {
		global $zs_omniverse_options, $omniverse_transfer_options, $omniverse_prefix;
		if ( ! empty( $omniverse_transfer_options ) ) {
			foreach ( $omniverse_transfer_options as $field ) {
				$meta = get_post_meta( omniverse_page_ID(), $omniverse_prefix . $field, true );
				if ( isset( $zs_omniverse_options[ $field ] ) ) {
					$zs_omniverse_options[ $field ] = ( isset( $meta ) && $meta != '' && $meta != 'inherit' && $meta != 'default' ) ? $meta : $zs_omniverse_options[ $field ];
				}
			}
		}
	}
	...
	public function specific_options() {
		...
		$rules = omniverse_get_config( 'specific-options' );

		foreach ( $rules as $option => $rule ) {
			if ( ! empty( $rule['will-be'] ) && ! isset( $rule['if'] ) ) {
				$zs_omniverse_options[ $option ] = $rule['will-be'];
			} elseif ( isset( $zs_omniverse_options[ $rule['if'] ] ) && in_array( $zs_omniverse_options[ $rule['if'] ], $rule['in_array'] ) ) {
				$zs_omniverse_options[ $option ] = $rule['will-be'];
			}
		}
	}
	...
	public function specific_taxonomy_options() {
		...
		if ( is_category() ) {
			...
			$category_blog_design = get_term_meta( $current_category->term_id, '_omniverse_' . $option_key, true );

			if ( ! empty( $category_blog_design ) && $category_blog_design != 'inherit' ) {
				$zs_omniverse_options[ $option_key ] = $category_blog_design;
			}
		}
	}
	...
	public function load_options() {
		if ( get_option( 'dn-' . self::$opt_name . '-options' ) ) {
			self::$_options = get_option( 'dn-' . self::$opt_name . '-options' );
		}
	}
	...
	public function load_presets( $preset_id = '' ) {
		...
		$_options = apply_filters( 'zs_presets_options', self::$_options );
	...
	public function setup_globals() {
		$GLOBALS[ 'zs_' . self::$opt_name . '_options' ] = self::$_options;
	}
	...
	public function register_settings() {
		register_setting( 'dn-options-group', 'dn-' . self::$opt_name . '-options', array( 'sanitize_callback' => array( $this, 'sanitize_before_save' ) ) );
	}
	...
	public static function override_options_from_meta() {
		foreach ( self::$_override_options as $key ) {
			$meta = get_post_meta( omniverse_get_the_ID(), '_omniverse_' . $key, true );
	...
	public function get_css_output( $is_preset_active ) {
		...
		foreach ( self::$_fields as $key => $field ) {
			...
			if ( ! $is_preset_active || omniverse_is_opt_changed( $field->args['id'] ) || 'group' === $field->args['type'] ) {
	...
	public function update_options( $new_options ) {
		update_option( 'dn-' . self::$opt_name . '-options', $new_options );
	}
	...
	public function sanitize_before_save( $options ) {
		...
		$default_options = get_option( 'dn-' . self::$opt_name . '-options' );

		$presets = Presets::get_all();

		if ( ! $presets ) {
			$presets = get_option( 'dn-' . self::$opt_name . '-options-presets' );
		}
		...
		if ( isset( $options['import-btn'] ) && $options['import-btn'] ) {
			...
			if ( isset( $imported_options['presets'] ) ) {
				...
				update_option( 'dn-options-presets', $presets );
			}
