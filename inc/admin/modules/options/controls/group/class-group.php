...
use DN\Admin\Modules\Options\Page;
...
class Group extends Field {
	...
	public function render_control() {
		...
		if ( 'dropdown' === $design ) {
			$btn_settings = array(
				'label'   => esc_html__( 'Edit', 'omniverse' ),
				'classes' => '',
			);
			...
			?>
			<a class="dn-btn dn-color-primary dn-dropdown-open <?php echo esc_attr( $btn_settings['classes'] ); ?>">
				<?php echo esc_html( $btn_settings['label'] ); ?>
			</a>
			<div class="dn-dropdown-options dn-hidden">
				<?php $this->render_inner_fields(); ?>
			</div>
			<?php
		} else {
			?>
			<div class="dn-fields-group">
				<?php $this->render_inner_fields(); ?>
			</div>
			<?php
		}
	...
	public function get_field_label() {
		...
		if ( Presets::get_current_preset() && $this->inner_fields ) {
			...
			?>
			<input type="hidden" class="dn-group-settings" data-inputs-id='<?php echo wp_json_encode( $inner_input_ids ); ?>'>
			<?php
		}
		...
		?>
		<a href="#" class="dn-reset-group dn-i-round-left<?php echo $show_reset_btn ? ' dn-show' : ' dn-hidden'; ?>" data-settings='<?php echo wp_json_encode( $inner_input_name ); ?>' title="<?php esc_html_e( 'Reset', 'omniverse' ); ?>"></a>
		<?php
	}
	...
	public function css_output() {
		...
		foreach ( $this->inner_fields as $inner_field ) {
			...
			if ( 'metabox' === $this->_type || ! $is_active_preset || omniverse_is_opt_changed( $inner_field->args['id'] ) ) {
		...
		if ( empty( $this->args['selectors'] ) || ! function_exists( 'omniverse_decompress' ) ) {
			return $output_css;
		}
		...
		foreach ( $this->args['selectors'] as $selector => $css_data ) {
			foreach ( $css_data as $css ) {
				...
				foreach ( $this->inner_fields as $field ) {
					if ( ! $generate_preset_css && $is_active_preset && str_contains( $css, '{{' . strtoupper( $field->get_id() ) . '}}' ) && omniverse_is_opt_changed( $field->args['id'] ) ) {
					...
					} elseif ( omniverse_is_compressed_data( $field_value ) ) {
						$decompressed_value = json_decode( omniverse_decompress( $field_value ), true );
