<?php
/**
 * Upload media library control.
 *
 * @package dn
 */

namespace DN\Admin\Modules\Options\Controls;

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Direct access not allowed.
}

use DN\Admin\Modules\Options\Field;
/**
 * Upload list button.
 */
class Upload_List extends Field {
	/**
	 * Displays the field control HTML.
	 *
	 * @since 1.0.0
	 *
	 * @return void.
	 */
	public function render_control() {
		$images = $this->get_field_value();
		?>
		<div class="dn-upload-preview">
			<?php foreach ( explode( ',', $images ) as $image_id ) : ?>
				<?php if ( $image_id ) : ?>
					<div data-attachment_id="<?php echo esc_attr( $image_id ); ?>">
						<?php echo wp_get_attachment_image( $image_id, 'thumbnail' ); // phpcs:ignore ?>
						<a href="#" class="dn-remove">
							<span class="dn-i-close"></span>
						</a>
					</div>
				<?php endif; ?>
			<?php endforeach; ?>
		</div>

		<div class="dn-upload-btns">
			<a class="dn-btn dn-upload-btn dn-i-import"><?php esc_html_e( 'Upload', 'omniverse' ); ?></a>
			<a class="dn-btn dn-color-warning dn-clear-all dn-remove-upload-btn dn-i-trash"><?php esc_html_e( 'Clear all', 'omniverse' ); ?></a>
			<input type="hidden" class="dn-upload-input-id" name="<?php echo esc_attr( $this->get_input_name() ); ?>" value="<?php echo esc_attr( $images ); ?>" />
		</div>
		<?php
	}

	/**
	 * Check value URl and ID fields.
	 *
	 * @since 1.0.0
	 *
	 * @param string or array $value Field value.
	 *
	 * @return mixed
	 */
	public function validate( $value ) {
		if ( isset( $value['id'] ) ) {
			$attachment = wp_get_attachment_url( $value['id'] );

			if ( $attachment ) {
				$value['url'] = $attachment;
			}
		}

		return $value;
	}

	/**
	 * Enqueue media lib.
	 *
	 * @since 1.0.0
	 */
	public function enqueue() {
		wp_enqueue_media();
	}
}
