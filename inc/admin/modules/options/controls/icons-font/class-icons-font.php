...
class Icons_Font extends Field {
	...
	public function render_control() {
		...
		<div class="dn-fields-group dn-group">
			<div class="dn-fields">
				<div class="dn-field dn-col-6">
					<?php if ( ! empty( $options['font'] ) ) : ?>
						<div class="dn-option-title">
							<label>
								<span>
									<?php esc_html_e( 'Icon design', 'omniverse' ); ?>
								</span>
							</label>
						</div>

						<div class="dn-option-control">
							<select class="dn-select dn-icon-font-select" name="<?php echo esc_attr( $this->get_input_name( 'font' ) ); ?>" aria-label="<?php echo esc_attr( $this->get_input_name( 'font' ) ); ?>">
				...
				<div class="dn-field dn-col-6">
					<?php if ( ! empty( $options['weight'] ) ) : ?>
						<div class="dn-option-title">
							<label>
								<span>
									<?php esc_html_e( 'Icon weight', 'omniverse' ); ?>
								</span>
							</label>
						</div>

						<div class="dn-option-control">
							<select class="dn-select dn-icon-weight-select" name="<?php echo esc_attr( $this->get_input_name( 'weight' ) ); ?>" aria-label="<?php echo esc_attr( $this->get_input_name( 'weight' ) ); ?>">
	...
	public function preview_icons() {
		wp_enqueue_style( 'wd-icon-preview', OMNIVERSE_ASSETS . '/css/icon-preview.css', array(), OMNIVERSE_VERSION );

		?>
		<div class="dn-field dn-icons-preview">
			<?php foreach ( $this->icons_config as $key => $name ) : ?>
				<div class="wd-icon-<?php echo esc_attr( $name ); ?>" tabindex="0">
					<div class="dn-tooltip dn-top" tabindex="0">
						<div class="dn-tooltip-inner">
							content: <span class="dn-icon-cont">"\<?php echo esc_attr( $key ); ?>"</span>;<br>
							font-family: <span class="dn-icon-font">"omniverse-font"</span>;<br>
							font-weight: <span class="dn-icon-weight">400</span>;
						</div>
					</div>
				</div>
			<?php endforeach; ?>
