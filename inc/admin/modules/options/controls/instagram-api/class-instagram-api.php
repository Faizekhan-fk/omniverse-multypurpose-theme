...
class Instagram_Api extends Field {
	...
	public function render_control() {
		if ( isset( $_GET['instagram_account_id'] ) && isset( $_GET['instagram_access_token'] ) ) {
			echo '<div class="dn-notice dn-success">Access token generated</div>';
			$this->connect();
		}
		...
		?>
			<div class="dn-upload-btns">
				<a href="<?php echo esc_url( $this->get_link() ); ?>" class="dn-btn dn-color-primary">
					<?php esc_html_e( 'Connect', 'omniverse' ); ?>
				</a>

				<?php if ( $instagram_access_token ) : ?>
					<a href="<?php echo $this->get_return_url() . '&instagram_account_disconnect'; ?>" class="dn-btn dn-color-warning dn-remove-upload-btn dn-i-trash">
						<?php esc_html_e( 'Disconnect', 'omniverse' ); ?>
					</a>
				<?php endif; ?>
			</div>
		<?php
	...
	public function get_return_url() {
		return admin_url( 'admin.php' ) . '?page=zs_theme_settings&tab=api_integrations_section';
	}
	...
	public function get_link() {
		...
		$redirect_uri  = urlencode( 'https://zynxsol.com/instagram.php' );
		$response_type = 'code';
		$scope         = apply_filters( 'omniverse_instagram_token_scope', 'manage_pages,instagram_basic,public_profile,pages_read_engagement' );
	...
	public function show_connected_account() {
		...
		?>
		<div class="dn-instagram-profile">
			<div class="dn-instagram-pic"><img src="<?php echo $instagram_account_data['image']; ?>" alt=""></div>
			<div class="dn-instagram-name"><?php echo $instagram_account_data['name']; ?> : <span>@<?php echo $instagram_account_data['username']; ?></span></div>
		</div>
		<?php
	}
