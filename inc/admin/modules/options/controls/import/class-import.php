...
class Import extends Field {
	...
	public function __construct( $args, $options, $type = 'options', $object = 'post' ) {
		...
		add_action( 'wp_ajax_zs_download_export_options', array( $this, 'download_options_export' ), 10 );
	}
	...
	public function render_control() {
		?>
			<textarea class="dn-import-area" name="<?php echo esc_attr( $this->get_input_name() ); ?>"></textarea>
			<button class="dn-save-options-btn dn-btn dn-color-primary dn-i-import" type="submit" name="dn-<?php echo esc_attr( $this->opt_name ); ?>-options[import-btn]" value="1"><?php esc_html_e( 'Import', 'omniverse' ); ?></button>

			<?php if ( defined( 'AUTH_KEY' ) && defined( 'SECURE_AUTH_KEY' ) ) : ?>
				<a class="dn-save-options-btn dn-bordered-btn dn-color-primary dn-i-export" href="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>?action=zs_download_export_options&secret=<?php echo esc_attr( $this->generete_secret() ); ?>"><?php esc_html_e( 'Export options', 'omniverse' ); ?></a>
			<?php endif; ?>
		<?php
	}
	...
	public function download_options_export() {
		...
		header( 'Content-Disposition: attachment; filename="zs_options_backup_' . date( 'd-m-Y' ) . '.json"' );
	...
	public function generete_secret() {
		...
		return md5( md5( AUTH_KEY . SECURE_AUTH_KEY ) . '-omniverse' );
	}
