...
abstract class Field {
	...
	public function __construct( $args, $options, $type = 'options', $object = 'post' ) {
		...
		$this->extra_css_class  = 'dn-' . $this->args['type'] . '-control';
		$this->extra_css_class .= ' dn-' . $this->args['id'] . '-field';
		...
		if ( isset( $this->args['tabs'] ) ) {
			$this->extra_css_class .= ' dn-tabs dn-style-' . $this->args['tabs'];
		}
	}
	...
	public function render( $object = null, $preset = false, $is_inner_control = false ) {
		...
		if ( $preset && $this->is_inherit_value() && ! isset( $this->args['tabs'] ) && ! $is_inner_control ) {
			$this->extra_css_class .= ' dn-field-disabled';
		}
		...
		echo '<div class="dn-option-control">';
		$this->render_control();
		echo '</div>';
	...
	public function before( $is_inner_control = false ) {
		?>
			<div class="dn-field dn-settings-field <?php echo esc_attr( $this->extra_css_class ); ?>" <?php $this->get_dependency_data_attribute(); ?>>
					...
					<div class="dn-option-title">
						<label>
							<?php if ( false !== $this->_presets && 'notice' !== $this->args['type'] && ! isset( $this->args['tabs'] ) && isset( $_GET['preset'] ) && ! $is_inner_control ) : // phpcs:ignore ?>
								<div class="dn-inherit-checkbox-wrapper">
									<?php esc_html_e( 'Inherit', 'omniverse' ); ?>
						...
						<?php if ( ! empty( $this->args['status'] ) ) : ?>
							<div class="dn-field-status dn-status-<?php echo esc_attr( $this->args['status'] ); ?><?php echo ! empty( $this->args['status_description'] ) ? ' dn-status-hint' : ''; ?>">
								...
								<?php if ( ! empty( $this->args['status_description'] ) ) : ?>
									<span class="dn-status-icon dn-i-help-question"></span>
									<div class="dn-tooltip dn-top"><div class="dn-tooltip-inner"><?php echo $this->args['status_description']; // phpcs:ignore ?></div></div>
								<?php endif; ?>
							</div>
						<?php endif; ?>

						<?php if ( ! empty( $this->args['hint'] ) && omniverse_get_opt( 'white_label_theme_hints', true ) ) : ?>
							<div class="dn-hint">
								<div class="dn-tooltip dn-top"><div class="dn-tooltip-inner"><?php echo $this->args['hint']; // phpcs:ignore ?></div></div>
							</div>
						<?php endif; ?>
					</div>
				<?php endif; ?>
	...
	private function dependency_class() {
		...
		return ( $shown ) ? 'dn-shown' : 'dn-hidden';
	}
	...
	public function after() {
		?>
				<?php if ( isset( $this->args['description'] ) && ! empty( $this->args['description'] ) ) : ?>
					<p class="dn-field-description"><?php echo $this->args['description']; // phpcs:ignore ?></p>
				<?php endif; ?>
			</div>
		<?php
	}
	...
	public function get_default_value( $value = '' ) {
		...
		} elseif ( ! empty( $this->args['devices'] ) && function_exists( 'omniverse_compress' ) ) {
			$value = omniverse_compress( wp_json_encode( array( 'devices' => $this->args['devices'] ) ) );
		}
