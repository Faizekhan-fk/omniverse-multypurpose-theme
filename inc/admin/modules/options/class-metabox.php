...
class Metabox {
			...
			<div class="dn-box-content">
				<div class="dn-row dn-sp-20">
					<?php if ( count( $this->get_sections() ) > 1 ) : ?>
						<div class="dn-col-12 dn-col-xl-2">
	...
	private function display_sections_tree() {
		foreach ( $this->get_sections() as $key => $section ) {
			...
			?>
				<li class="<?php echo ( $key === $this->get_last_tab() ) ? 'dn-active-nav' : ''; ?>">
						...
						foreach ( $section['fields'] as $field ) {
							...
							if ( isset( $field->args['t_tab'] ) && $printed_tabs !== $field->args['t_tab']['id'] ) {
								...
								foreach ( $tabs[ $field->args['t_tab']['id'] ] as $tab ) {
									...
									echo '<li><a href="#" class="wd-nav-link dn-nav-link' . $classes . '" title="' . $tab['title'] . '"><span class="nav-link-text wd-tabs-title dn-tabs-title">' . $tab['title'] . '</span></a></li>'; // phpcs:ignore
								}
								echo '</ul>';
								echo '</div>';
								echo '</div>';
	...
	public function save_posts_fields( $post_id ) {
		...
		foreach ( $this->_fields as $key => $field ) {
			...
			do_action( 'omniverse_metabox_before_update_metadata', $post_id, $field->get_input_name(), $value );
