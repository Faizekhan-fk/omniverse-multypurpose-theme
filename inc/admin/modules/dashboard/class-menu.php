...
class Menu {
	...
	protected function get_menu( $config, $child_menu = false ) {
		...
		if ( $child_menu ) {
			$classes = wd_add_cssclass( 'dn-dropdown dn-sub-menu', $classes );
		} else {
			$classes = wd_add_cssclass( 'dn-nav', $classes );
		}
	...
	protected function get_menu_item( $item_data ) {
		...
		if ( ! empty( $item_data['icon'] ) ) {
			$link_classes = wd_add_cssclass( 'dn-i-' . $item_data['icon'], $link_classes );
		}
		...
		if ( isset( $item_data['child_menu'] ) && $this->check_show_items( array_column( $item_data['child_menu']['items'], 'condition' ) ) ) {
			$classes = wd_add_cssclass( 'dn-has-child', $classes );

			foreach ( $item_data['child_menu']['items'] as $child_item_data ) {
				$classes .= $this->get_active_classes( $child_item_data, 'dn-active' );
			}
		}

		$classes .= $this->get_active_classes( $item_data, 'dn-active' );

		$link_target = ! empty( $item_data['link']['new_window'] ) ? '_blank' : '_self';

		?>
		<li class="<?php echo esc_attr( $classes ); ?> new-check">
