(function($) {
	...
	$panel.on('vcPanel.shown', function() {
		...
		$('.vc_wrapper-param-type-omniverse_title_divider').each(function() {
			var $divider = $(this);
			...
			var $fields = $divider.nextUntil('.vc_wrapper-param-type-omniverse_title_divider, .vc_shortcode-param.omniverse-vc-no-wrap');
			var $wrapper = $('<div class="omniverse-td-wrapper"></div>');
			var $content = $('<div class="omniverse-td-content dn-group"></div>');
		...
		$('.vc_wrapper-param-type-wd_fonts').each(function() {
			...
			var $content   = $('<div class="wd-typography-content dn-hidden"></div>');
			var $btn       = $('<button class="wd-typography-btn dn-btn dn-i-typography">Edit</button>');
			var $btn_reset = $('<div class="wd-typography-btn-reset dn-i-round-left"><div class="dn-tooltip dn-right">Reset typography</div></div>');
			...
			$btn.on('click', function(){
				...
				$this.addClass('dn-changed');

				$this.siblings('.wd-typography-content').toggleClass('dn-hidden');
				...
				$(document).on('mouseup', function(e) {
					...
					if (!$typographyContent.is(e.target) && $typographyContent.has(e.target).length === 0 && !$this.is(e.target) && !$('.vc_ui-panel-content-container').is(e.target)) {
						$this.siblings('.wd-typography-content').addClass('dn-hidden');
			...
			$btn_reset.on( 'click', function () {
				...
				$(document).trigger('omniverseResetTypography');
				$this.parent().siblings('.wd-typography-btn').removeClass('dn-changed');
				$this.parent().addClass('dn-hidden');
			});
		});
	...
	function hideDividerWrapper($divider) {
		var $wrapper = $divider.parent('.omniverse-td-wrapper');
	...
	$panel.on('change', '.wpb_el_type_omniverse_title_divider', function() {
