(function($) {
	...
	$saveButton.on('click', function() {
		...
		var cssId = $('.omniverse-css-id').val();
		...
		$('.wpb_el_type_omniverse_responsive_spacing').each(function() {
			var $control = $(this);
		...
		if ($styleTag.length === 0) {
			$vcIframe.contents().find('body').prepend('<style id="new' + cssId + '" data-type="omniverse_shortcodes-custom-css">' + results + '</style>');
	...
	$saveButton.on('click', function() {
		...
		$('.omniverse-rs-wrapper, .omniverse-vc-colorpicker').each(function() {
			dataSorting($(this));
		});
		...
		if ($styleTag.length == 0) {
			$vcIframe.contents().find('body').prepend('<style id="' + cssId + '" data-type="omniverse_shortcodes-custom-css">' + results + '</style>');
		} else if ($styleTag.length > 0) {
			$styleTag.html(results);
		}
	...
	if (typeof window.InlineShortcodeView != 'undefined') {
		window.InlineShortcodeView_omniverse_brands = window.InlineShortcodeView.extend({
			rendered: function() {
				$(document).trigger('FrontendEditorCarouselInit', [this.$el.find('.brands-items-wrapper')]);
				window.InlineShortcodeView_omniverse_brands.__super__.rendered.call(this);
			}
		});

		window.InlineShortcodeView_omniverse_categories = window.InlineShortcodeView.extend({
			rendered: function() {
				$(document).trigger('FrontendEditorCarouselInit', [this.$el.find('.categories-style-carousel')]);
				window.InlineShortcodeView_omniverse_categories.__super__.rendered.call(this);
			}
		});

		window.InlineShortcodeView_omniverse_blog = window.InlineShortcodeView.extend({
			rendered: function() {
				$(document).trigger('FrontendEditorCarouselInit', [this.$el.find('.slider-type-post')]);
				window.InlineShortcodeView_omniverse_blog.__super__.rendered.call(this);
			}
		});

		window.InlineShortcodeView_omniverse_gallery = window.InlineShortcodeView.extend({
			rendered: function() {
				$(document).trigger('FrontendEditorCarouselInit', [this.$el.find('.wd-images-gallery')]);
				window.InlineShortcodeView_omniverse_gallery.__super__.rendered.call(this);
			}
		});

		window.InlineShortcodeView_omniverse_instagram = window.InlineShortcodeView.extend({
			rendered: function() {
				$(document).trigger('FrontendEditorCarouselInit', [this.$el.find('.instagram-pics')]);
				window.InlineShortcodeView_omniverse_instagram.__super__.rendered.call(this);
			}
		});

		window.InlineShortcodeView_omniverse_products = window.InlineShortcodeView.extend({
			rendered: function() {
				$(document).trigger('FrontendEditorCarouselInit', [this.$el.find('.slider-type-product')]);
				window.InlineShortcodeView_omniverse_products.__super__.rendered.call(this);
			}
		});
	}
