...
(function($) {
	...
	function htmlBlockEditLink() {
		$('.dn-edit-block-link').each(function() {
			var $link = $(this);
			...
			function changeLink() {
				...
				if ( $('body').hasClass('elementor-editor-active') || 'undefined' !== typeof omniverseConfig && 'elementor' === omniverseConfig.current_page_builder) {
					newHref += 'elementor';
				} else {
					newHref += 'edit';
				}
				...
				if (!selectValue || '0' === selectValue || 0 === selectValue) {
					$link.hide();
					$link.siblings('.dn-add-block-link').show();
				} else {
					$link.attr('href', newHref).show();
					$link.siblings('.dn-add-block-link').hide();
				}
			}
		});
	}
