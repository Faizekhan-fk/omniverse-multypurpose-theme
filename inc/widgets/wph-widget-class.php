    ...
    class WPH_Widget extends WP_Widget
    {
		...
		public function get_layered_get_categories_options() {
			$categories_array = array(
				esc_html__( 'All categories', 'omniverse' ) => 'all',
			);
		...
		public function is_widget_preview() {
			if ( apply_filters( 'omniverse_hide_content_on_widget_preview', true ) && defined( 'REST_REQUEST' ) && REST_REQUEST ) {
				return true;
			}
        ...
        */
        ...
        function create_field( $key, $out = "" ) {
            ...
            $key['type'] = $this->omniverse_change_type($key['type']);
        ...
        private function omniverse_change_type($key) {
            switch ($key) {
                case 'textfield':
                    $key = 'text';
                break;
                ...
                case 'textarea_html':
                    $key = 'textarea';
                break;
                case 'omniverse_switch':
                    $key = 'dropdown';
                break;
                case 'omniverse_button_set':
                    $key = 'dropdown';
                break;
                case 'omniverse_slider':
                    $key = 'dropdown';
                break;
                case 'omniverse_image_select':
                    $key = 'dropdown';
                break;
	            case 'omniverse_dropdown':
		            $key = 'dropdown';
		            break;
            }
            return $key;
		...
		function create_field_select2( $key, $out = "" ) {
			...
			?>
				...
				<select name="<?php echo esc_attr( $key['_name'] ); ?>[]" id="<?php echo esc_attr( $key['_id'] ); ?>" class="dn-select dn-select2" multiple>
        ...
        */
        ...
        function create_field_attach_image( $key, $out = "" )
        {
            ...
            $out .= '<img src="'.$url.'" class="omniverse-image-src" style="'.$style.'" />';

            $out .= '<input type="hidden" ';

            if ( isset( $key['class'] ) )
                $out .= 'class="omniverse-image-upload ' . esc_attr( $key['class'] ) . '" ';
            ...
            $out .= '<button class="button omniverse-image-upload-btn">' .__('Upload image', 'omniverse') . '</button>';
	    ...
	    function create_field_attach_images( $key, $out = '' ) {
		    ...
		    ?>
		    <div class="dn-upload_list-control">
			    <?php echo $this->create_field_label( $key['name'], $key['_id'] ); ?>
			    ...
			    <div class="dn-upload-btns">
				    <button class="dn-btn dn-upload-btn dn-i-import">
					    <?php esc_html_e( 'Upload', 'omniverse' ); ?>
				    </button>

				    <button class="dn-btn dn-btn-remove dn-color-warning">
					    <?php esc_html_e( 'Clear all', 'omniverse' ); ?>
				    </button>

				    <input type="hidden" class="dn-upload-input-id" name="<?php echo esc_attr( $key['_name'] ); ?>" value="<?php echo esc_attr( $value ); ?>" />
			    </div>
