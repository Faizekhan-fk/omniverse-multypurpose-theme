...
if ( ! class_exists( 'OMNIVERSE_Widget_Layered_Nav' ) ) {
	class OMNIVERSE_Widget_Layered_Nav extends WPH_Widget {
		function __construct() {
			if ( ! omniverse_woocommerce_installed() || ! function_exists( 'wc_get_attribute_taxonomies' ) ) {
				return;
			}
			...
			$args = array(
				...
				'label'       => esc_html__( 'OMNIVERSE WooCommerce Layered Nav', 'omniverse' ),
				// Widget Backend Description
				'description' => esc_html__( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'omniverse' ),
				'slug'        => 'omniverse-woocommerce-layered-nav',
			);
			...
			$args['fields'] = array(
				array(
					'id'   => 'title',
					'type' => 'text',
					'std'  => esc_html__( 'Filter by', 'omniverse' ),
				...
				array(
					...
					'type'   => 'dropdown',
					'std'    => '',
					'name'   => esc_html__( 'Attribute', 'omniverse' ),
				...
				array(
					...
					'type'    => 'select2',
					'default' => array( 'all' ),
					'name'    => esc_html__( 'Show on category', 'omniverse' ),
				...
				array(
					...
					'type'   => 'dropdown',
					'std'    => 'and',
					'name'   => esc_html__( 'Query type', 'omniverse' ),
					'fields' => array(
				...
				array(
					...
					'fields'     => array(
						...
						esc_html__( 'Dropdown', 'omniverse' ) => 'dropdown'
					)
				),
				array(
					'id'     => 'size',
					'type'   => 'dropdown',
					'std'    => 'normal',
					'name'   => esc_html__( 'Swatches size', 'omniverse' ),
					'fields' => array(
				...
				array(
					...
					'type'   => 'dropdown',
					'std'    => 'inherit',
					'name'   => esc_html__( 'Swatch style', 'omniverse' ),
					'fields' => array(
				...
				array(
					...
					'type'   => 'dropdown',
					'std'    => 'round',
					'name'   => esc_html__( 'Swatches shape', 'omniverse' ),
					'fields' => array(
						esc_html__( 'Inherit', 'omniverse' ) => 'inherit',
				...
				array(
					...
					'type'   => 'dropdown',
					'std'    => 'on',
					'name'   => esc_html__( 'Show labels', 'omniverse' ),
					'fields' => array(
				...
				array(
					...
					'type'   => 'dropdown',
					'std'    => 'on',
					'name'   => esc_html__( 'Show tooltips', 'omniverse' ),
					'fields' => array(
						esc_html__( 'OFF', 'omniverse' ) => 'off',
						esc_html__( 'ON', 'omniverse' )  => 'on',
					),
				...
				array(
					...
					'type'   => 'dropdown',
					'std'    => 'off',
					'name'   => esc_html__( 'Show checkboxes', 'omniverse' ),
					'fields' => array(
						esc_html__( 'OFF', 'omniverse' ) => 'off',
						esc_html__( 'ON', 'omniverse' )  => 'on',
					),
				...
				array(
					...
					'type'       => 'checkbox',
					'name'       => esc_html__( 'Show search input for this attribute', 'omniverse' ),
					'dependency' => array(
						'element'            => 'display',
		...
		function widget( $args, $instance ) {
			...
			if ( 'layered-nav' === omniverse_get_opt( 'shop_widgets_collapse' ) ) {
				$wrapper_classes .= ' wd-widget-collapse';
			}

			if ( 'disable' !== omniverse_get_opt( 'shop_widgets_collapse', 'disable' ) && is_array( $_chosen_attributes ) && array_key_exists( $taxonomy, $_chosen_attributes ) ) {
			...
			if ( isset( $instance['search_by_filters'] ) && $instance['search_by_filters'] ) {
				omniverse_enqueue_inline_style( 'filter-search' );
				omniverse_enqueue_js_script( 'search-by-filters' );
			}
		...
		protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
			...
			if ( $taxonomy !== $this->get_current_taxonomy() ) {
				...
				$any_label      = apply_filters( 'woocommerce_layered_nav_any_label', sprintf( __( 'Any %s', 'omniverse' ), $taxonomy_label ), $taxonomy_label, $taxonomy );
				...
				omniverse_enqueue_js_script( 'filter-dropdowns' );

				echo '<form method="get" action="' . esc_url( $form_action ) . '" class="wd-widget-layered-nav-dropdown-form">';
				echo '<select class="wd-widget-layered-nav-dropdown omniverse_dropdown_layered_nav_' . esc_attr( $taxonomy_filter_name ) . '"' . ( $multiple ? 'multiple="multiple"' : '' ) . ' data-placeholder="' . esc_attr( $any_label ) . '" data-noResults="' . esc_html__( 'No matches found', 'omniverse' ) . '" data-slug="' . esc_attr( $taxonomy_filter_name ) . '">';
				...
				if ( $multiple ) {
					echo '<button class="wd-widget-layered-nav-dropdown__submit" type="submit" value="' . esc_attr__( 'Apply', 'omniverse' ) . '">' . esc_html__( 'Apply', 'omniverse' ) . '</button>';
				}
		...
		protected function get_filtered_term_product_counts( $term_ids, $taxonomy, $query_type ) {
			...
			';
				...
				if ( $search = WC_Query::get_main_search_query_sql() ) {
					...
					if ( omniverse_get_opt( 'search_by_sku' ) ) {
		...
		protected function layered_nav_list( $terms, $taxonomy, $query_type, $instance ) {
			...
			$scroll_for_widget = omniverse_get_opt( 'widgets_scroll' );

			$is_brand = ( omniverse_get_opt( 'brands_attribute' ) == $taxonomy );

			if ( 'inherit' === $style ) {
				$style = omniverse_wc_get_attribute_term( $taxonomy, 'swatch_style' );
			}
			if ( 'inherit' === $shape ) {
				$shape = omniverse_wc_get_attribute_term( $taxonomy, 'swatch_shape' );
			...
			if ( $style ) {
				...
				omniverse_enqueue_inline_style( 'woo-mod-swatches-style-' . $style );
			} else {
				$class .= ' wd-text-style-1';

				omniverse_enqueue_inline_style( 'woo-mod-swatches-style-1' );
			}
			...
			if ( $is_brand ) {
				...
				if ( $style ) {
					...
					omniverse_enqueue_inline_style( 'woo-mod-swatches-style-4' );
				}
			...
			if ( in_array( $size, array( 'small', 'normal', 'large' ), true ) ) {
				$class .= omniverse_get_old_classes( ' swatches-' . $size );
			}
			...
			if ( $search_by_filters ) {
				...
				$placeholder = sprintf( esc_html__( 'Find a %s', 'omniverse' ), $label );
				...
				<div class="wd-filter-wrapper">
					<div class="wd-filter-search wd-search">
						...
						<span class="wd-filter-search-clear wd-action-btn wd-style-icon wd-cross-icon">
							<a href="#" aria-label="<?php esc_attr__( 'Clear search', 'omniverse' ); ?>"></a>
						</span>
					</div>
				<?php
			}
			...
			foreach ( $terms as $term ) {
				...
				$filter_classes = omniverse_get_old_classes( ' filter-swatch' );
				...
				if ( ! empty( $swatch_color ) ) {
					$filter_classes .= omniverse_get_old_classes( ' with-bg' );
				...
				if ( is_array( $swatch_image ) ) {
					...
					$swatch_image = '<img src="' . $swatch_image . '" alt="' . esc_attr__( 'Swatch image', 'omniverse' ) . '">';
				}

				if ( ! empty( $swatch_text ) && ! $swatch_style && ! $swatch_image ) {
					$filter_classes .= omniverse_get_old_classes( ' with-text' );
		...
		protected function layered_nav_checkbox_list( $terms, $taxonomy, $query_type, $instance ) {
			$query_type           = isset( $instance['query_type'] ) ? $instance['query_type'] : 'and';
			$title                = isset( $instance['filter-title'] ) ? $instance['filter-title'] : esc_html__( 'Filter by', 'omniverse' );
			...
			if ( in_array( $size, array( 'small', 'normal', 'large' ), true ) ) {
				$classes .= omniverse_get_old_classes( ' swatches-' . $size );
			}
