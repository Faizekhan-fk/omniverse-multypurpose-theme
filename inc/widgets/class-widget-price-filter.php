...
if( ! class_exists( 'OMNIVERSE_Widget_Price_Filter' ) ) {
	class OMNIVERSE_Widget_Price_Filter extends WPH_Widget {

		private $_check_ranges = false;
	
		function __construct() {
			if( ! omniverse_woocommerce_installed() ) return;

			// Configure widget array
			$args = array( 
				// Widget Backend label
				'label' => esc_html__( 'OMNIVERSE Price filter', 'omniverse' ),
				// Widget Backend Description								
				'description' =>esc_html__( 'Price filter list', 'omniverse' ),
				'slug' => 'omniverse-price-filter',
			 );
			...
			$args['fields'] = array(
				array(
					'id'	=> 'title',
					'type'  => 'text',
					'std'   => esc_html__( 'Price filter', 'omniverse' ),
					'name' 	=> esc_html__( 'Title', 'omniverse' )
				),
				array(
					'id'	      => 'show_ranges',
					'type'        => 'checkbox',
					'name' 	      => esc_html__( 'Show empty price ranges', 'omniverse' ),
					'description' => esc_html__( 'May increase this widget performance if you have a lot of products in your store.', 'omniverse' ),
				),
			);

			$this->_check_ranges = apply_filters('omniverse_check_ranges_price_filter', true);
		...
		function widget( $args, $instance )	{
			...
			if( ! empty( $links ) ) {
				?>
					<div class="omniverse-price-filter">
						<ul>
							<?php foreach ($links as $link) : ?>
								<li>
									<a rel="nofollow noopener" href="<?php echo esc_url( $link['href'] ); ?>" class="<?php echo esc_attr( $link['class'] ); ?>"><?php echo wp_kses( $link['title'], omniverse_get_allowed_html() ); ?></a>
								</li>
							<?php endforeach; ?>
						</ul>
					</div>
		...
		private function generate_price_links( $min, $max, $min_price, $max_price, $show_ranges ) {
			...
			$link = omniverse_shop_page_link( true );
			...
			$links[] = array(
				'href' 	=> $link_no_price,
				'title' => esc_html__('All', 'omniverse'),
