<?php
/**
 * The template for displaying all dn templates.
 *
 * @package Omniverse
 */

use DN\Modules\Layouts\Main;
use DN\Modules\Layouts\Single_Product;
	...
	<div class="woocommerce-checkout">
		...
		<?php if ( $checkout_form_id ) : ?>
			<form name="checkout" method="post" class="checkout woocommerce-checkout" action="<?php echo esc_url( wc_get_checkout_url() ); ?>" enctype="multipart/form-data">
				<?php if ( omniverse_is_elementor_installed() && Elementor\Plugin::$instance->documents->get( $checkout_form_post->ID )->is_built_with_elementor() ) : ?>
					<?php echo omniverse_elementor_get_content( $checkout_form_post->ID ); // phpcs:ignore ?>
				<?php else : ?>
					<?php
					$shortcodes_custom_css = get_post_meta( $checkout_form_post->ID, '_wpb_shortcodes_custom_css', true );
					$omniverse_shortcodes_custom_css = get_post_meta( $checkout_form_post->ID, 'omniverse_shortcodes_custom_css', true );
...
<?php elseif ( 'checkout_form' === $layout_type ) : ?>
	<div class="woocommerce-checkout">
		<?php if ( $checkout_content_id ) : ?>
			<?php if ( omniverse_is_elementor_installed() && Elementor\Plugin::$instance->documents->get( $checkout_content_post->ID )->is_built_with_elementor() ) : ?>
				<?php echo omniverse_elementor_get_content( $checkout_content_post->ID ); // phpcs:ignore ?>
			<?php else : ?>
				<?php
				$shortcodes_custom_css = get_post_meta( $checkout_content_post->ID, '_wpb_shortcodes_custom_css', true );
				$omniverse_shortcodes_custom_css = get_post_meta( $checkout_content_post->ID, 'omniverse_shortcodes_custom_css', true );
