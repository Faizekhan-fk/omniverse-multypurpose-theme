<?php
	...
	omniverse_enqueue_inline_style( 'header-search' );

if ( 'form' === $params['display'] || 'full-screen-2' === $params['display'] ) {
	omniverse_enqueue_inline_style( 'header-search-form' );
...
$extra_class .= omniverse_get_old_classes( ' search-button' );

?>
<div class="wd-header-search wd-tools-element<?php echo esc_attr( $extra_class ); ?>" title="<?php echo esc_attr__( 'Search', 'omniverse' ); ?>">
	<a href="javascript:void(0);" aria-label="<?php esc_html_e( 'Search', 'omniverse' ); ?>">
		...
		<?php endif; ?>
			...
			<span class="wd-tools-icon<?php echo omniverse_get_old_classes( ' search-button-icon' ); ?>">
				<?php
					if ( $icon_type == 'custom' ) {
			...
			<span class="wd-tools-text">
				<?php echo esc_html__( 'Search', 'omniverse' ); ?>
			</span>
