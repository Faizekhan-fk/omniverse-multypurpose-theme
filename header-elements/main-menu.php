...
$classes     .= omniverse_get_old_classes( ' navigation-style-' . $menu_style );
$extra_class .= omniverse_get_old_classes( ' full-screen-burger-icon omniverse-burger-icon' );
		...
		<div class="wd-tools-element wd-header-fs-nav<?php echo esc_attr( $extra_class ); ?>">
			<a href="#" rel="nofollow noopener">
				...
				<?php endif; ?>
					...
					<span class="wd-tools-icon<?php echo omniverse_get_old_classes( ' omniverse-burger' ); ?>">
						<?php if ( $icon_type == 'custom' ) : ?>
							<?php echo whb_get_custom_icon( $params['custom_icon'] ); ?>
						<?php endif; ?>
					</span>

					<span class="wd-tools-text"><?php esc_html_e( 'Menu', 'omniverse' ); ?></span>
...
?>
<div class="wd-header-nav wd-header-main-nav <?php echo esc_attr( $classes ); ?>" role="navigation" aria-label="<?php esc_html_e( 'Main navigation', 'omniverse' ); ?>">
	<?php
	$args = array(
		'container'  => '',
	...
	if ( has_nav_menu( $location ) ) {
		...
		?>
			...
			<?php
				printf(
					wp_kses(
						__( 'Create your first <a href="%s"><strong>navigation menu here</strong></a> and add it to the "Main menu" location.', 'omniverse' ),
						array(
							'a' => array(
								'href' => array(),
							),
