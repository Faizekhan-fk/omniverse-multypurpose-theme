...
if ( ! empty( $params['menu_layout'] ) ) {
	if ( 'dropdown' === $params['menu_layout'] ) {
		omniverse_enqueue_inline_style( 'header-mobile-nav-dropdown' );
	} elseif ( 'drilldown' === $params['menu_layout'] ) {
		omniverse_enqueue_inline_style( 'header-mobile-nav-drilldown' );

		if ( 'slide' === $params['drilldown_animation'] ) {
			omniverse_enqueue_inline_style( 'header-mobile-nav-drilldown-slide' );
		} elseif ( 'fade-in' === $params['drilldown_animation'] ) {
			omniverse_enqueue_inline_style( 'header-mobile-nav-drilldown-fade-in' );
		}
	}
}
...
$extra_class .= omniverse_get_old_classes( ' omniverse-burger-icon' );

?>
<div class="wd-tools-element wd-header-mobile-nav<?php echo esc_attr( $extra_class ); ?>">
	<a href="#" rel="nofollow" aria-label="<?php esc_html_e( 'Open mobile menu', 'omniverse' ); ?>">
