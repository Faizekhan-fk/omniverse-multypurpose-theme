...
<div class="wd-header-nav wd-header-secondary-nav <?php echo esc_attr( $classes ); ?>" role="navigation" aria-label="<?php esc_html_e( 'Secondary navigation', 'omniverse' ); ?>">
	<?php
	if ( wp_get_nav_menu_object( $params['menu_id'] ) && wp_get_nav_menu_items( $params['menu_id'] ) ) {
		...
		?>
		<span>
			<?php esc_html_e( 'Wrong menu selected', 'omniverse' ); ?>
		</span>
		<?php
	} else {
		?>
		<span>
			<?php esc_html_e( 'Choose menu', 'omniverse' ); ?>
		</span>
		<?php
	}
	?>
