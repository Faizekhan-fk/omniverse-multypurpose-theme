...
$links = omniverse_get_header_links( $params );
...
if ( $my_account_style ) {
	...
	$classes .= omniverse_get_old_classes( ' my-account-with-' . $my_account_style );
}
...
$classes .= omniverse_get_old_classes( ' omniverse-header-links omniverse-navigation item-event-hover menu-simple-dropdown' );
...
<div class="wd-header-my-account wd-tools-element wd-event-hover<?php echo esc_attr( $classes ); ?>">
	<?php foreach ($links as $key => $link): ?>
		<a href="<?php echo esc_url( $link['url'] ); ?>" title="<?php echo esc_attr__( 'My account', 'omniverse' ); ?>">
		...
		<?php if( ! empty( $link['dropdown'] ) ) echo apply_filters( 'omniverse_account_element_dropdown', $link['dropdown'] ); ?>
	<?php endforeach; ?>
</div>
