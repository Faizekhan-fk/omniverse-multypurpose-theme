<?php

$classes        = array();
$omniverse_loop  = omniverse_loop_prop( 'omniverse_loop' );
$blog_design    = omniverse_loop_prop( 'blog_design' );
$blog_style     = omniverse_get_opt( 'blog_style', 'shadow' );
$excerpt_length = apply_filters( 'omniverse_get_excerpt_length', omniverse_get_opt( 'blog_excerpt_length' ) );
$classes[]      = 'wd-post';
$classes[]      = 'blog-design-' . $blog_design;
$classes[]      = 'blog-post-loop';
$post_format    = get_post_format();
$thumb_classes  = '';
$gallery_slider = apply_filters( 'omniverse_gallery_slider', true );
$gallery        = array();

if ( ! is_single() && 'quote' === $post_format ) {
	omniverse_enqueue_inline_style( 'blog-loop-format-quote' );
}

if ( 'shadow' === $blog_style ) {
	$classes[] = 'blog-style-bg';

	if ( omniverse_get_opt( 'blog_with_shadow', true ) ) {
		$classes[] = 'wd-add-shadow';
	}
} else {
	$classes[] = 'blog-style-' . $blog_style;
}

if ( ! get_the_title() ) {
	$classes[] = 'post-no-title';
}

$gallery_slider = apply_filters( 'omniverse_gallery_slider', true );
$gallery        = array();

if( 'gallery' === $post_format && $gallery_slider ) {
	$gallery = get_post_gallery( false, false );

	if ( ! empty( $gallery['src'] ) ) {
		$thumb_classes .= ' wd-carousel-container wd-post-gallery color-scheme-light';
	}
}
?>

<article id="post-<?php the_ID(); ?>" <?php post_class( $classes ); ?>>
	<div class="wd-post-inner">
		<div class="wd-post-thumb<?php echo esc_attr( $thumb_classes ); ?>">
			<?php if ( 'gallery' === $post_format && $gallery_slider && ! empty( $gallery['src'] ) ) : ?>
				<?php
				omniverse_enqueue_js_library( 'swiper' );
				omniverse_enqueue_js_script( 'swiper-carousel' );
				omniverse_enqueue_inline_style( 'swiper' );
				?>
				<div class="wd-carousel-inner">
					<div class="wd-carousel wd-grid"<?php echo omniverse_get_carousel_attributes( array( 'autoheight' => 'yes' ) ); ?>>
						<div class="wd-carousel-wrap">
							<?php
								foreach ( $gallery['src'] as $src ) {
									if ( preg_match( "/data:image/is", $src ) ) {
										continue;
									}
									?>
									<div class="wd-carousel-item">
										<?php echo apply_filters( 'omniverse_image', '<img src="' . esc_url( $src ) . '" />' ); ?>
									</div>
									<?php
								}
							?>
						</div>
					</div>
					<?php omniverse_get_carousel_nav_template( ' wd-post-arrows wd-pos-sep wd-custom-style' ); ?>
				</div>
			<?php else : ?>
				<div class="wd-post-img">
					<?php echo omniverse_get_post_thumbnail( 'large' ); // phpcs:ignore ?>
				</div>
			<?php endif; ?>
			<?php
			omniverse_post_date(
				array(
					'style' => 'wd-style-with-bg',
				)
			);
			?>
			<?php /* translators: %s: Post title */ ?>
			<a class="wd-post-link wd-fill" href="<?php echo esc_url( get_permalink() ); ?>" rel="bookmark" aria-label="<?php echo esc_attr( sprintf( __( 'Link on post %s', 'omniverse' ), esc_attr( get_the_title() ) ) ); ?>"></a>
			<?php if ( omniverse_loop_prop( 'parts_meta' ) && get_the_category_list( ', ' ) ) : ?>
				<div class="wd-post-cat wd-style-with-bg">
					<?php echo get_the_category_list( ', ' ); // phpcs:ignore ?>
				</div>
			<?php endif ?>
		</div>

		<div class="wd-post-content">
			<div class="wd-post-content-inner wd-scroll">
				<?php if ( omniverse_loop_prop( 'parts_meta' ) ) : ?>
					<div class="wd-post-header">
						<?php if ( is_sticky() ) : ?>
							<div class="wd-featured-post">
								<?php esc_html_e( 'Featured', 'omniverse' ); ?>
							</div>
						<?php endif; ?>

						<div class="wd-meta-author">
							<?php omniverse_post_meta_author( true, 'long' ); ?>
						</div>

						<div class="wd-post-actions">
							<?php if ( omniverse_is_social_link_enable( 'share' ) ) : ?>
								<div class="wd-post-share wd-tltp wd-tltp-top">
									<div class="wd-tooltip-label">
										<?php
										if ( function_exists( 'omniverse_shortcode_social' ) ) {
                                            echo omniverse_shortcode_social( // phpcs:ignore
												array(
													'size' => 'small',
													'color' => 'light',
												)
											);}
										?>
									</div>
								</div>
							<?php endif ?>

							<?php if ( comments_open() ) : ?>
								<div class="wd-meta-reply">
									<?php omniverse_post_meta_reply(); ?>
								</div>
							<?php endif; ?>
						</div>
					</div>
				<?php endif; ?>
				<?php if ( omniverse_loop_prop( 'parts_title' ) ) : ?>
					<h3 class="wd-entities-title title post-title">
						<a href="<?php echo esc_url( get_permalink() ); ?>" rel="bookmark">
							<?php the_title(); ?>
						</a>
					</h3>
				<?php endif ?>
				<?php if ( omniverse_loop_prop( 'parts_text' ) ) : ?>
					<div class="wd-entry-content wd-scroll-content">
						<?php if ( is_search() ) : ?>
							<div class="entry-summary">
								<?php the_excerpt(); ?>
							</div><!-- .entry-summary -->
						<?php else : ?>
							<?php omniverse_get_content( false ); ?>
							<?php
							wp_link_pages(
								array(
									'before'      => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'omniverse' ) . '</span>',
									'after'       => '</div>',
									'link_before' => '<span>',
									'link_after'  => '</span>',
								)
							);
							?>
						<?php endif; ?>
					</div>
				<?php endif; ?>
			</div>
			<?php if ( omniverse_loop_prop( 'parts_btn' ) ) : ?>
				<?php omniverse_render_read_more_btn(); ?>
			<?php endif; ?>
		</div>
	</div>
</article>
