...
$thumbs_position             = omniverse_get_opt( 'thums_position' );
$image_action                = omniverse_get_opt( 'image_action' );
...
$thumbnails_settings         = omniverse_get_product_gallery_settings();
...
$gallery_columns_desktop     = omniverse_get_opt( 'single_product_gallery_column_desktop', 1 );
$gallery_columns_tablet      = omniverse_get_opt( 'single_product_gallery_column_tablet', 1 );
$gallery_columns_mobile      = omniverse_get_opt( 'single_product_gallery_column_mobile', 1 );
$grid_columns_desktop        = omniverse_get_opt( 'single_product_grid_column_desktop', 1 );
$grid_columns_tablet         = omniverse_get_opt( 'single_product_grid_column_tablet', 1 );
$grid_columns_mobile         = omniverse_get_opt( 'single_product_grid_column_mobile', 1 );
$pagination_controls         = omniverse_get_opt( 'pagination_main_gallery' );
$carousel_on_tablet          = omniverse_get_opt( 'main_gallery_on_tablet', true );
$carousel_on_mobile          = omniverse_get_opt( 'main_gallery_on_mobile', true );
$gallery_center_mode         = 'without' === $thumbs_position && omniverse_get_opt( 'main_gallery_center_mode' );
$thumbnails_wrap_in_mobile   = omniverse_get_opt( 'single_product_thumbnails_wrap_in_mobile_devices', true );
...
if ( omniverse_get_opt( 'product_slider_auto_height' ) ) {
...
if ( 'zoom' === $image_action ) {
	omniverse_enqueue_js_script( 'init-zoom' );
}
...
if ( 'left' === $thumbs_position ) {
	if ( $thumbnails_wrap_in_mobile ) {
		...
		omniverse_enqueue_inline_style( 'woo-single-prod-el-gallery-opt-thumb-left-desktop', true );
	} else {
		omniverse_enqueue_inline_style( 'woo-single-prod-el-gallery-opt-thumb-left', true );
	}
}

if ( 'left' !== $thumbs_position && 'bottom' !== $thumbs_position && 'without' !== $thumbs_position ) {
	if ( $carousel_on_tablet && $carousel_on_mobile ) {
		omniverse_enqueue_inline_style( 'woo-single-prod-el-gallery-opt-thumb-grid-lg', true );
	} elseif ( $carousel_on_tablet ) {
		omniverse_enqueue_inline_style( 'woo-single-prod-el-gallery-opt-thumb-grid-sm', true );
	} elseif ( $carousel_on_mobile ) {
		omniverse_enqueue_inline_style( 'woo-single-prod-el-gallery-opt-thumb-grid-md', true );
	} else {
		omniverse_enqueue_inline_style( 'woo-single-prod-el-gallery-opt-thumb-grid', true );
	}
}
...
<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?> images image-action-<?php echo esc_attr( $image_action ); ?>">
	<div class="<?php echo esc_attr( $main_gallery_classes ); ?>">
		...
		<?php do_action( 'omniverse_before_single_product_main_gallery' ); ?>
		...
		<figure class="woocommerce-product-gallery__wrapper<?php echo esc_attr( $gallery_classes ); ?>" <?php echo wp_kses( implode( ' ', $main_gallery_attrs ), true ); ?>>
			...
			$attributes = array(
				...
				'class'                   => apply_filters( 'omniverse_single_product_gallery_image_class', 'wp-post-image' ),
			);
			...
			<?php omniverse_get_carousel_nav_template( ' wd-pos-sep wd-hover-1 wd-custom-style' ); ?>

		<?php do_action( 'omniverse_on_product_image' ); ?>

		</div>

		<?php if ( $pagination_controls ) : ?>
			<?php omniverse_get_carousel_pagination_template( array(), ' wd-style-shape wd-custom-style' ); ?>
		<?php endif; ?>
	</div>
	...
	<?php if ( 'sticky' !== $product_design && ( 'bottom' === $thumbs_position || 'left' === $thumbs_position ) ) : ?>
		...
		if ( omniverse_get_opt( 'single_product_thumbnails_gallery_image_width' ) ) {
			$image_size = array( omniverse_get_opt( 'single_product_thumbnails_gallery_image_width' ), 0 );
		}
		...
		<div class="wd-carousel-container wd-gallery-thumb">
			<div class="wd-carousel-inner">
					...
					<div class="wd-carousel-wrap">
						<?php if ( $attachment_ids ) : ?>
							<?php foreach ( $attachment_ids as $attachment_id ) : ?>
								<div class="wd-carousel-item <?php echo esc_attr( apply_filters( 'omniverse_single_product_thumbnail_classes', '', $attachment_id ) ); ?>">
				...
				<?php omniverse_get_carousel_nav_template( $thumb_classes ); ?>
			</div>
		</div>
	<?php endif; ?>
</div>
