...
if ( 'yes' === omniverse_loop_prop( 'hide_out_of_stock_products' ) && $available_variations ) {
...
$is_quick_shop                 = wp_doing_ajax() && isset( $_REQUEST['action'] ) && 'omniverse_quick_shop' === $_REQUEST['action'] && omniverse_get_opt( 'quick_shop_variable' ); // phpcs:ignore
$is_quick_shop2                = isset( $wd_swatches_limits );
$is_quick_view                 = omniverse_loop_prop( 'is_quick_view' );
$swatches_use_variation_images = omniverse_get_opt( 'swatches_use_variation_images' );
$grid_swatches_attribute       = omniverse_grid_swatches_attribute();
$form_classes                  = '';
$wd_reset_classes              = '';
$swatch_limit                  = false;
...
if ( $is_quick_shop2 ) {
	...
	$form_classes     .= ' wd-clear-' . omniverse_get_opt( 'quick_shop_clear_action', 'none' );
} elseif ( ! $is_quick_shop && omniverse_get_opt( 'single_product_swatches_limit' ) ) {
	$swatch_limit = (int) omniverse_get_opt( 'single_product_swatches_limit_count', 10 );
}
...
if ( omniverse_get_opt( 'swatches_labels_name' ) ) {
	$form_classes .= ' wd-swatches-name';
}

if ( ! $is_quick_shop && ! $is_quick_shop2 ) {
	if ( omniverse_get_opt( 'single_product_variations_price' ) ) {
		omniverse_enqueue_js_script( 'variations-price' );
...
if ( $is_quick_view ) {
	...
	if ( 'default' === omniverse_get_opt( 'product_design' ) && omniverse_get_opt( 'swatches_labels_name' ) ) {
		$form_classes .= ' wd-label-top-lg';
	}
}
...
omniverse_enqueue_inline_style( 'woo-mod-variation-form' );
omniverse_enqueue_inline_style( 'woo-mod-swatches-base' );

if ( is_product() || $is_quick_view ) {
	omniverse_enqueue_inline_style( 'woo-mod-variation-form-single' );
}
	...
	<?php else : ?>
		<table class="variations" cellspacing="0">
			...
			<?php foreach ( $attributes as $attribute_name => $options ) : ?>
				...
				// start Omniverse code.
				$loop++;
				$swatches            = omniverse_has_swatches( $product->get_id(), $attribute_name, $options, $available_variations, $swatches_use_variation_images );
				$active_variations   = omniverse_get_active_variations( $attribute_name, $available_variations );
				...
				$swatch_change_image = omniverse_wc_get_attribute_term( $attribute_name, 'change_image' );
				$wrapper_class       = omniverse_get_old_classes( ' swatches-on-single' );
								...
								if ( is_array( $options ) ) {
									...
									if ( taxonomy_exists( $attribute_name ) ) {
										...
										foreach ( $terms as $term ) {
											...
											$image  = '';
											$class  = 'wd-swatch';
											$class .= omniverse_get_old_classes( ' swatch-on-single omniverse-swatch' );
											...
											if ( ! empty( $swatches[ $key ]['color'] ) ) {
												...
												if ( is_array( $swatches[ $key ]['image'] ) ) {
													...
													$image = '<img src="' . $swatches[ $key ]['image'] . '" alt="' . esc_attr__( 'Swatch image', 'omniverse' ) . '">';
												}
											...
											$title = omniverse_get_opt( 'swatches_labels_name' ) ? ' title="' . $term->name . '"' : '';
